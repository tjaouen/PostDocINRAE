...
for (HER_h_ in HER_){
  ...
  if (HER_h_ %in% colnames(tab_1_)){
    ...
    tab_allModels_2070_2100_ = tab_allModels_[which(tab_allModels_$Date >= as.Date("2020-08-01") & tab_allModels_$Date <= as.Date("2050-07-31")),]
    ...
    p_mean <- ggplot() +
      ...
      labs(title = paste0("Moyenne des probabilités d'assecs lissées sur 5 jours (2021-2050) - HER ",HER_h_),
           x = "Date",
    ...
    p_median <- ggplot() +
      ...
      # labels = Legend) +
      # geom_point(data = first_days, aes(x = Date, y = 0), size = 0, alpha = 0) +
      labs(title = paste0("Médianne des probabilités d'assecs lissées sur 5 jours (2021-2050) - HER ",HER_h_),
           x = "Date",
