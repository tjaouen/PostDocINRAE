print("J2000 - 8_Chroniques_ProjectionsAssecs_20412070_7local_20240229.R")
...
for (HER_h_ in HER_){
  ...
  if (HER_h_ %in% colnames(tab_1_)){
    ...
    if (!(nrow(tab_allModels_2070_2100_[grepl("rcp",tab_allModels_2070_2100_$Type),]) %in% c(as.Date(date_intervalle_[2])-as.Date(date_intervalle_[1])+1,0))){
      stop(paste0("Error with rcp content. File :", list_files_chroniquesProba_[l],
                  " - HER : ", HER_h_,
                  " - nrow rcp : ",nrow(tab_allModels_2070_2100_[grepl("rcp",tab_allModels_2070_2100_$Type),])))
    }
