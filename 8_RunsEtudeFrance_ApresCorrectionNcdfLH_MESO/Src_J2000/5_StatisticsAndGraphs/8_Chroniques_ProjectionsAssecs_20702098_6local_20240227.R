print("J2000 - 8_Chroniques_ProjectionsAssecs_19752005_6local_20240227.R")
...
for (HER_h_ in HER_[1]){
  ...
  if (HER_h_ %in% colnames(tab_1_)){
    ...
    tab_allModels_2070_2100_ = tab_allModels_[which(tab_allModels_$Date >= as.Date("2070-08-01") & tab_allModels_$Date <= as.Date("2099-07-31") & grepl("rcp",tab_allModels_$Type)),]
    
    if (!(nrow(tab_allModels_2070_2100_[which(tab_allModels_2070_2100_$Type == "Historical"),]) %in% c(as.Date("2099-07-31")-as.Date("2070-08-01")+1,0))){
      stop(paste0("Error with Historical content. File :", list_files_chroniquesProba_[l], " - HER : ", HER_h_))
    }
    if (!(nrow(tab_allModels_2070_2100_[which(tab_allModels_2070_2100_$Type == "Safran"),]) %in% c(as.Date("2099-07-31")-as.Date("2070-08-01")+1,0))){
      stop(paste0("Error with Safran content. File :", list_files_chroniquesProba_[l], " - HER : ", HER_h_))
    }
    if (!(nrow(tab_allModels_2070_2100_[grepl("rcp",tab_allModels_2070_2100_$Type),]) %in% c(as.Date("2099-07-31")-as.Date("2070-08-01")+1,0))){
