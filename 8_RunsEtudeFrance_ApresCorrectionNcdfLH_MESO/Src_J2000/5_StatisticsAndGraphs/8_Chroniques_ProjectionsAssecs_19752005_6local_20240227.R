...
for (HER_h_ in HER_[1]){
  ...
  if (HER_h_ %in% colnames(tab_1_)){
    ...
    if (!(nrow(tab_allModels_2070_2100_[which(tab_allModels_2070_2100_$Type == "Historical"),]) %in% c(as.Date("2005-07-31")-as.Date("1976-08-01")+1,0))){
      stop(paste0("Error with Historical content. File :", list_files_chroniquesProba_[l], " - HER : ", HER_h_))
    }
    if (!(nrow(tab_allModels_2070_2100_[which(tab_allModels_2070_2100_$Type == "Safran"),]) %in% c(as.Date("2005-07-31")-as.Date("1976-08-01")+1,0))){
      stop(paste0("Error with Safran content. File :", list_files_chroniquesProba_[l], " - HER : ", HER_h_))
    }
    if (!(nrow(tab_allModels_2070_2100_[grepl("rcp",tab_allModels_2070_2100_$Type),]) %in% c(as.Date("2005-07-31")-as.Date("1976-08-01")+1,0))){
