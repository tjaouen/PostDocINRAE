...
for (HER_h_ in HER_){
  ...
  if (HER_h_ %in% colnames(tab_1_)){
    ...
    # year_labels <- rep(c('Janv.','','','Avril','','','Juil.','','','Oct.','',''))
    ...
    # year_labels_eng <- rep(c('Jan.','','','Apr.','','','Jul.','','','Oct.','',''))
    ...
    p_mean <- ggplot() +
      ...
      #                    values = levels(df_mean_nonNar_$Color))+#,
      # guide = F)+#+
    ...
    scale_x_date("Date",
      ...
      guides(alpha = guide_legend(override.aes = list(lwd = 2.5,color =levels(df_mean_nonNar_$Color)),
                                  # color = levels(df_mean_nonNar_$Color)),
                                  keyheight = 3.2,
                                  keywidth = unit(2,"cm"),
                                  order = 1),
             color = guide_legend(
               # title = element_text(face = "normal"),
               override.aes = list(lwd = 2.5),
               #   # override.aes = list(lwd = 2.5, alpha = c(rep(1,length(levels(df_mean_nar_$Color))))),
               keyheight = 3.2,
               keywidth = unit(1,"cm"),
               title.hjust = 0.2,
               order = 2))
    ...
    # x11()
    # p_mean$layers[[1]]$aes_params$alpha = c(1,0.15)
    # p_mean$layers[[2]]$aes_params$alpha = c(1,1,1,1)
    p_mean
    ...
    p_mean <- ggplot() +
      ...
      geom_line(data = df_mean_nonNar_, aes(x = Jour_annee, y = Mean_ValueLissee, group = Variable, color = Legend, lineend = "round", alpha = Alpha), lwd = 1) + # linetype = Type,
      scale_color_manual(name = "Modèles GCM-RCM",
                         values = levels(df_mean_nonNar_$Color),
                         guide = F)+#+
      scale_alpha_manual(name = "Modèles GCM-RCM 3",
                         labels = levels(df_mean_nonNar_$Legend))#+
    p_mean  
    #   new_scale_color()+
    #   geom_line(data = df_mean_nar_, aes(x = Jour_annee, y = Mean_ValueLissee, group = Variable, color = Legend, lineend = "round"), alpha = 1, lwd = 1) + # linetype = Type,
    #   scale_color_manual(name = "Modèles GCM-RCM 2",
    #                      values = levels(df_mean_nar_$Color),
    #                      guide = "legend")+
    #   
    #   # new_scale_color()+
    ...
    #         legend.text = element_text(color = "#2f2f32", size = 18)) +
    #   guides(alpha = guide_legend(override.aes = list(lwd = 2.5,
    #                                                   color = levels(df_mean_nonNar_$Color)),
    #                               keyheight = 3.2,
    #                               keywidth = unit(2,"cm")),
    #          color = guide_legend(
    ...
    p_mean <- ggplot() +
      ...
      theme(plot.title = element_text(color = "#060403", size = 30, face = "bold"),
            ...
            axis.title.y = element_blank(),
            axis.text.x = element_text(color = "#2f2f32",vjust = 0, hjust=0.5), # angle = 90
            axis.text.y = element_text(color = "#2f2f32"), # angle = 90
            axis.line.x = element_line(color = "#bebdbb", size = 1), # Trait des abscisses plus épais
            ...
            panel.grid.major.y = element_line(color = "#dcdad9", size = 0.5), # Lighter horizontal lines
            panel.grid.minor.y = element_line(color = "#dcdad9", size = 0.5), # Lighter horizontal lines
