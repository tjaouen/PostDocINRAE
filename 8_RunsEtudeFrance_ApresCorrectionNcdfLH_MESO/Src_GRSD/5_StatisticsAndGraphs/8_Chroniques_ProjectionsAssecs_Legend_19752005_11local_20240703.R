...
for (HER_h_ in HER_){
  for (nom_categorieSimu_ in nom_categorieSimu_list_){
    ...
    translation_dict <- c("Modéré en réchauffement et\nen changement de précipitations" = "Moderate warming and\nprecipitation change",
                          ...
                          "Safran" = "Safran",
                          "Projections Explore2" = "Explore2 projections, including storylines:")
    df_mean <- df_mean %>%
    ...
    desired_order_english <- c("Moderate warming and\nprecipitation change",
                               ...
                               "Hot and humid\nall seasons",
                               "Safran",
                               "Explore2 projections, including storylines:")
    
    desired_order_lty <- c("Historical",
                           "Safran")
    
    df_mean$Color <- factor(df_mean$Color, levels = intersect(c("#E5E840", "#E2A138", "#70194E", "#447C57","black","#253494"), levels(factor(df_mean$Color))))
    ...
    df_mean_nonNar_$Alpha <- factor(df_mean_nonNar_$Alpha, levels = c(1,0.15))
    ...
    ### Legend Guides ###
    guide_1_ <- guides(alpha = c(1,0.15),
                       keyheight = 3.2,
                       keywidth = unit(2,"cm"),
                       linetype = guide_legend(keyheight = 1.5,
                                               keywidth = unit(2,"cm"),
                                               override.aes = list(linetype = c("solid", "dotdash"))))
    
    guide_2_ <- guides(alpha = c(1,1,1,1),
                       keyheight = 3.2,
                       keywidth = unit(2,"cm"),
                       linetype = guide_legend(keyheight = 1.5,
                                               keywidth = unit(2,"cm"),
    ...
    p_mean <- ggplot() +
      ...
      theme(plot.title = element_text(color = "#060403", size = 30, face = "bold"),
            ...
            legend.title = element_text(color = "grey38", size = 18, face = "bold"), #, face = "normal"
            legend.text = element_text(color = "grey38", size = 18),
            legend.margin = margin(-7, 0, 0, 0),
            legend.spacing.y = unit(+0.03, "cm"))+
      guides(alpha = guide_legend(override.aes = list(lwd = 2.5,color =levels(df_mean_nonNar_$Color)),
                                  keyheight = 2.4,
    ...
    svg_device <- svglite(paste0(folder_output_DD_,
                                 "22_GrapheChroniqueProbabilite_OneModelHorizon20702100_Projections/",
                                 nomSim_,
                                 ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                                 "/Chroniques/",year(date_intervalle_[1]),year(date_intervalle_[2]),"_Legend/Eng/ProbaMeanValue_",pattern_rcp_,"_",year(date_intervalle_[1]),year(date_intervalle_[2]),"_HER",HER_h_,"_Eng_1_20240227.svg"),
                          width = 15)
    print(p_mean)
    dev.off()
    saveRDS(p_mean, file = paste0(folder_output_DD_,
                                  "22_GrapheChroniqueProbabilite_OneModelHorizon20702100_Projections/",
                                  nomSim_,
                                  ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                                  "/Chroniques/",year(date_intervalle_[1]),year(date_intervalle_[2]),"_Legend/Eng/ProbaMeanValue_",pattern_rcp_,"_",year(date_intervalle_[1]),year(date_intervalle_[2]),"_HER",HER_h_,"_Eng_1_20240227.rds"))
    
    png(paste0(folder_output_DD_,
               "22_GrapheChroniqueProbabilite_OneModelHorizon20702100_Projections/",
               nomSim_,
               ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
               "/Chroniques/",year(date_intervalle_[1]),year(date_intervalle_[2]),"_Legend/Eng/ProbaMeanValue_",pattern_rcp_,"_",year(date_intervalle_[1]),year(date_intervalle_[2]),"_HER",HER_h_,"_Eng_1_20240227.png"), width = 9*100)
    print(p_mean)
    dev.off()
    ...
    #                      labels = levels(df_mean_nonNar_$Legend))+
    #   
    #   geom_line(data = df_mean_nar_, aes(x = Jour_annee, y = Median_ValueLissee, group = Variable, color = Legend, lineend = "round"), alpha = 1, lwd = 1) + # linetype = Type,
    #   scale_color_manual(name = "dont narratifs :",
    #                      values = levels(df_mean_nar_$Color),
    #                      guide = "legend")+
    #   
    ...
    #   scale_color_manual(name = "including storylines:",
    #                      values = levels(df_mean_nar_$Color),
    #                      guide = "legend")+
    #   
    #   # Axes #
