...
files <- list.files(paste0(folder_output_,
                           "22_GrapheChroniqueProbabilite_OneModelHorizon20702100_Projections/",
                           nomSim_,
                           ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                           "/Tables/"), pattern = pattern_, recursive = T, full.names = T)

# Création d'une liste pour stocker les résultats
resultats <- list()

vartitle_ = NULL
output_name_ = paste0("/home/tjaouen/Documents/Output/ChangementClimatique2019/EtudeFrance/22_GrapheChroniqueProbabilite_OneModelHorizon20702100_Projections/32_ObservesReanalyseSafran_ApprentissageTousScenariosProjections_20240208/",nom_categorieSimu_,"/Map/Map_",pattern_,"_1_20240227_")
title_ = paste0("Proba moyenne assec juillet-octobre ",substr(pattern_,1,4),"-",substr(pattern_,5,8))
title_ = c("Probabilité moyenne\nd'assec")
