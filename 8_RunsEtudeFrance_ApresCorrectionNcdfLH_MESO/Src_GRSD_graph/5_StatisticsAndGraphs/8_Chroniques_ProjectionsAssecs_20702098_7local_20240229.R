...
for (HER_h_ in HER_){
  ...
  if (HER_h_ %in% colnames(tab_1_)){
    ...
    # tab_allModels_2070_2100_ = tab_allModels_[which(tab_allModels_$Date >= as.Date("1976-08-01") & tab_allModels_$Date <= as.Date("2005-07-31")),]
    # tab_allModels_2070_2100_ = tab_allModels_[which(tab_allModels_$Date >= as.Date("2020-08-01") & tab_allModels_$Date <= as.Date("2050-07-31") & grepl("rcp",tab_allModels_$Type)),]
    tab_allModels_2070_2100_ = tab_allModels_[which(tab_allModels_$Date >= as.Date("2071-01-01") & tab_allModels_$Date <= as.Date("2099-12-31") & grepl("rcp",tab_allModels_$Type)),]
    
    if (!(nrow(tab_allModels_2070_2100_[which(tab_allModels_2070_2100_$Type == "Historical"),]) %in% c(as.Date("2099-12-31")-as.Date("2071-01-01")+1,0))){
      stop(paste0("Error with Historical content. File :", list_files_chroniquesProba_[l], " - HER : ", HER_h_))
    }
    if (!(nrow(tab_allModels_2070_2100_[which(tab_allModels_2070_2100_$Type == "Safran"),]) %in% c(as.Date("2099-12-31")-as.Date("2071-01-01")+1,0))){
      stop(paste0("Error with Safran content. File :", list_files_chroniquesProba_[l], " - HER : ", HER_h_))
    }
    if (!(nrow(tab_allModels_2070_2100_[grepl("rcp",tab_allModels_2070_2100_$Type),]) %in% c(as.Date("2099-12-31")-as.Date("2071-01-01")+1,0))){
    ...
    #     (grepl("Safran", Type) & Alpha == 1) ~ "dashed"))
    
    pattern_rcp_ = unique(df_mean$Type)
    df_mean <- df_mean %>%
      mutate(LineType = case_when(
        (grepl(pattern_rcp_, Type)) ~ "solid"))#,
    # (grepl("Safran", Type)) ~ "dashed"))
    ...
    df_mean$LineType <- factor(df_mean$LineType, levels = intersect(c("solid","dashed"), levels(factor(df_mean$LineType))))
    ...
    if (!(dir.exists(paste0(folder_output_,
                            "22_GrapheChroniqueProbabilite_OneModelHorizon20702099_Projections/",
                            nomSim_,
    ...
    df_mean_overwriteColors_rcp_ = df_mean_overwriteColors[which(df_mean_overwriteColors$Type == pattern_rcp_),]
    
    
    p_mean <- ggplot() +
      geom_line(data = df_mean_rcp_, aes(x = Jour_annee, y = Mean_ValueLissee, group = Variable, color = Legend), linetype = "solid", alpha = 0.15, lwd = 2) + # linetype = Type, 
      geom_line(data = df_mean_overwriteColors_rcp_, aes(x = Jour_annee, y = Mean_ValueLissee, group = Variable, color = Legend), linetype = "solid", alpha = 1, lwd = 2) +
      
      # geom_line(data = df_mean_saf_, aes(x = Jour_annee, y = Mean_ValueLissee, group = Variable, color = Legend, linetype = "dashed"), alpha = 0.15, lwd = 2) + # linetype = Type, 
      # geom_line(data = df_mean_overwriteColors_saf_, aes(x = Jour_annee, y = Mean_ValueLissee, group = Variable, color = Legend, linetype = "dashed"), alpha = 1, lwd = 2) +
      
      scale_color_manual(name = "Modèles GCM-RCM",
                         values = levels(df_mean_rcp_$Color))+
      # scale_linetype_manual(values = as.factor(c("solid","dashed")),
      ...
      labs(title = paste0("Moyenne des probabilités d'assecs lissées sur 5 jours (2070-2099)\nModèle hydrologique ",str_before_first(nom_categorieSimu_,"_")," - HER ",HER_h_),
           x = "Date",
    ...
    svg_device <- svglite(paste0(folder_output_,
                                 ...
                                 nomSim_,
                                 ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                                 "/Chroniques/ProbaMeanValue_rcp85_20702099_HER",HER_h_,"_1_20240227.svg"),
                          width = 15)
    print(p_mean)
    dev.off()
    saveRDS(p_mean, file = paste0(folder_output_,
                                  ...
                                  nomSim_,
                                  ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                                  "/Chroniques/ProbaMeanValue_rcp85_20702099_HER",HER_h_,"_1_20240227.rds"))
    
    
    ### Mean - English ###
    p_mean <- ggplot() +
      geom_line(data = df_mean_rcp_, aes(x = Jour_annee, y = Mean_ValueLissee, group = Variable, color = Legend_English), linetype = "solid", alpha = 0.15, lwd = 2) + # linetype = Type, 
      geom_line(data = df_mean_overwriteColors_rcp_, aes(x = Jour_annee, y = Mean_ValueLissee, group = Variable, color = Legend_English), linetype = "solid", alpha = 1, lwd = 2) +
      
      # geom_line(data = df_mean_saf_, aes(x = Jour_annee, y = Mean_ValueLissee, group = Variable, color = Legend_English, linetype = "dashed"), alpha = 0.15, lwd = 2) + # linetype = Type, 
      # geom_line(data = df_mean_overwriteColors_saf_, aes(x = Jour_annee, y = Mean_ValueLissee, group = Variable, color = Legend_English, linetype = "dashed"), alpha = 1, lwd = 2) +
      ...
      labs(title = paste0("Mean smoothed drought probabilities over 5 days (2070-2099)\nHydrological Model ",str_before_first(nom_categorieSimu_,"_")," - HER ",HER_h_),
           x = "Date",
    ...
    svg_device <- svglite(paste0(folder_output_,
                                 ...
                                 nomSim_,
                                 ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                                 "/Chroniques/Eng/ProbaMeanValue_rcp85_20702099_HER",HER_h_,"_Eng_1_20240227.svg"),
                          width = 15)
    print(p_mean)
    dev.off()
    saveRDS(p_mean, file = paste0(folder_output_,
                                  ...
                                  nomSim_,
                                  ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                                  "/Chroniques/Eng/ProbaMeanValue_rcp85_20702099_HER",HER_h_,"_Eng_1_20240227.rds"))
    ...
    ### Median - French ###
    p_median <- ggplot() +
      geom_line(data = df_mean_rcp_, aes(x = Jour_annee, y = Median_ValueLissee, group = Variable, color = Legend), linetype = "solid", alpha = 0.15, lwd = 2) + # linetype = Type, 
      geom_line(data = df_mean_overwriteColors_rcp_, aes(x = Jour_annee, y = Median_ValueLissee, group = Variable, color = Legend), linetype = "solid", alpha = 1, lwd = 2) +
      
      # geom_line(data = df_mean_saf_, aes(x = Jour_annee, y = Median_ValueLissee, group = Variable, color = Legend, linetype = "dashed"), alpha = 0.15, lwd = 2) + # linetype = Type, 
      # geom_line(data = df_mean_overwriteColors_saf_, aes(x = Jour_annee, y = Median_ValueLissee, group = Variable, color = Legend, linetype = "dashed"), alpha = 1, lwd = 2) +
      ...
      labs(title = paste0("Médiane des probabilités d'assecs lissées sur 5 jours (2070-2099)\nModèle hydrologique ",str_before_first(nom_categorieSimu_,"_")," - HER ",HER_h_),
           x = "Date",
    ...
    svg_device <- svglite(paste0(folder_output_,
                                 ...
                                 nomSim_,
                                 ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                                 "/Chroniques/ProbaMedianValue_rcp85_20702099_HER",HER_h_,"_1_20240227.svg"),
                          width = 15)
    print(p_median)
    dev.off()
    saveRDS(p_mean, file = paste0(folder_output_,
                                  ...
                                  nomSim_,
                                  ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                                  "/Chroniques/ProbaMedianValue_rcp85_20702099_HER",HER_h_,"_1_20240227.rds"))
    
    
    ### Median - English ###
    p_median <- ggplot() +
      geom_line(data = df_mean_rcp_, aes(x = Jour_annee, y = Median_ValueLissee, group = Variable, color = Legend_English), linetype = "solid", alpha = 0.15, lwd = 2) + # linetype = Type, 
      geom_line(data = df_mean_overwriteColors_rcp_, aes(x = Jour_annee, y = Median_ValueLissee, group = Variable, color = Legend_English), linetype = "solid", alpha = 1, lwd = 2) +
      
      # geom_line(data = df_mean_saf_, aes(x = Jour_annee, y = Median_ValueLissee, group = Variable, color = Legend_English, linetype = "dashed"), alpha = 0.15, lwd = 2) + # linetype = Type, 
      # geom_line(data = df_mean_overwriteColors_saf_, aes(x = Jour_annee, y = Median_ValueLissee, group = Variable, color = Legend_English, linetype = "dashed"), alpha = 1, lwd = 2) +
      
      scale_color_manual(name = "GCM-RCM Models",
                         values = levels(df_mean_rcp_$Color))+
      # scale_linetype_manual(values = as.factor(c("solid")),
      #                       name = "Climate Input Data",
      #                       labels = c("Historical GCM-RCM","Safran"))+
      ...
      labs(title = paste0("Median smoothed drought probabilities over 5 days (2070-2099)\nHydrological Model ",str_before_first(nom_categorieSimu_,"_")," - HER ",HER_h_),
           x = "Date",
    ...
    svg_device <- svglite(paste0(folder_output_,
                                 ...
                                 nomSim_,
                                 ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                                 "/Chroniques/Eng/ProbaMedianValue_rcp85_20702099_HER",HER_h_,"_Eng_1_20240227.svg"),
                          width = 15)
    print(p_median)
    dev.off()
    saveRDS(p_mean, file = paste0(folder_output_,
                                  ...
                                  nomSim_,
                                  ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                                  "/Chroniques/Eng/ProbaMedianValue_rcp85_20702099_HER",HER_h_,"_Eng_1_20240227.rds"))
    ...
    write.table(df_mean,paste0(folder_output_,
                               ...
                               nomSim_,
                               ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                               "/Tables/Table_ProbaMeanValue_rcp85_20702099_HER",HER_h_,"_1_20240214.csv"),
                sep = ";", dec = ".", row.names = F)
