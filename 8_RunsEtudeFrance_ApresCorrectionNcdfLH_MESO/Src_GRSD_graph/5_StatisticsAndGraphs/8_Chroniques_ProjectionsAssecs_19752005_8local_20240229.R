...
# source("/lustre/jaouent/Src/ChangementClimatique_Bottet2019/CodesTristan/8_RunsEtudeFrance_ApresCorrectionNcdfLH_MESO/Src_GRSD_graph/PathsProgram/PathProgram_1_20230206.R")
# source("/lustre/jaouent/Src/ChangementClimatique_Bottet2019/CodesTristan/8_RunsEtudeFrance_ApresCorrectionNcdfLH_MESO/Src_GRSD_graph/1_Parameters_MESO/0_SimulationParameters_AvecCC_2_20230227_MESO.R")
...
for (HER_h_ in HER_){
  ...
  if (HER_h_ %in% colnames(tab_1_)){
    ...
    # tab_allModels_2070_2100_ = tab_allModels_[which(tab_allModels_$Date >= as.Date("1977-08-01") & tab_allModels_$Date <= as.Date("2004-07-31")),]
    tab_allModels_2070_2100_ = tab_allModels_[which(tab_allModels_$Date >= as.Date("1977-01-01") & tab_allModels_$Date <= as.Date("2004-12-31")),]
    if (!(nrow(tab_allModels_2070_2100_[which(tab_allModels_2070_2100_$Type == "Historical"),]) %in% c(as.Date("2004-12-31")-as.Date("1977-01-01")+1,0))){
      stop(paste0("Error with Historical content. File :", list_files_chroniquesProba_[l], " - HER : ", HER_h_))
    }
    if (!(nrow(tab_allModels_2070_2100_[which(tab_allModels_2070_2100_$Type == "Safran"),]) %in% c(as.Date("2004-12-31")-as.Date("1977-01-01")+1,0))){
      stop(paste0("Error with Safran content. File :", list_files_chroniquesProba_[l], " - HER : ", HER_h_))
    }
    if (!(nrow(tab_allModels_2070_2100_[grepl("rcp",tab_allModels_2070_2100_$Type),]) %in% c(as.Date("2004-12-31")-as.Date("1977-01-01")+1,0))){
    ...
    # df_mean = df_mean[which(df_mean$Type == "Historical"),]
    ...
    #     (grepl("Historical", Type) | Alpha == 0.15) ~ "solid",
    #     (grepl("Safran", Type) & Alpha == 1) ~ "longdash"))
    
    df_mean <- df_mean %>%
      mutate(LineType = case_when(
        (grepl("Historical", Type)) ~ "solid",
        (grepl("Safran", Type)) ~ "longdash"))
    ...
    df_mean$Color <- factor(df_mean$Color, levels = intersect(c("#E5E840", "#E2A138", "#70194E", "#447C57","black"), levels(factor(df_mean$Color))))
    df_mean$LineType <- factor(df_mean$LineType, levels = intersect(c("solid","longdash"), levels(factor(df_mean$LineType))))
    ...
    # df_mean$Alpha <- factor(df_mean$Alpha, levels = intersect(levels(factor(df_mean$Alpha)), c(1,1,1,1,0.15)))
    ...
    ### Historical ###
    df_mean_hist_ = df_mean[which(df_mean$Type == "Historical"),]
    
    ### Safran ###
    df_mean_saf_ = df_mean[which(df_mean$Type == "Safran"),]
    df_mean_saf_ <- df_mean_saf_ %>%
      group_by(Type, Jour_annee) %>%
      mutate(Mean_Value = mean(na.omit(Mean_Value)),
             Median_Value = median(na.omit(Median_Value)))
    df_mean_saf_ = df_mean_saf_[which(df_mean_saf_$Type == unique(df_mean_saf_$Variable)[1]),]
    df_mean_saf_$Variable = "Safran"
    df_mean_saf_$Color = "#253494"
    df_mean_saf_$Legend = "Safran"
    df_mean_hist_ = rbind(df_mean_hist_,
                          df_mean_saf_)
    ...
    # df_mean_overwriteColors_saf_ <- df_mean_saf_[which(df_mean_saf_$Type == "Safran" & df_mean_saf_$Legend %in% c("Modéré en réchauffement et\nen changement de précipitations",
    #                                                                "Sec toute l'année,\nrecharge moindre en hiver",
    #                                                                "Fort réchauffement et\nfort assèchement en été",
    #                                                                "Chaud et humide\nà toutes les saisons")),]
    
    df_mean_hist_ <- df_mean_hist_ %>%
      group_by(Type,Variable) %>%
      mutate(Mean_ValueLissee = (lag(Mean_Value, 2) + lag(Mean_Value, 1) + Mean_Value + lead(Mean_Value, 1) + lead(Mean_Value, 2)) / 5) %>%
      mutate(Median_ValueLissee = (lag(Median_Value, 2) + lag(Median_Value, 1) + Median_Value + lead(Median_Value, 1) + lead(Median_Value, 2)) / 5)
    
    df_mean_overwriteColors_hist_ <- df_mean_hist_[which(df_mean_hist_$Legend %in% c("Modéré en réchauffement et\nen changement de précipitations",
                                                                                     "Sec toute l'année,\nrecharge moindre en hiver",
                                                                                     "Fort réchauffement et\nfort assèchement en été",
                                                                                     "Chaud et humide\nà toutes les saisons",
                                                                                     "Safran")),]
    ...
    ### Axe x ###
    first_days <- df_mean_hist_[which(substr(df_mean_hist_$Jour_annee,3,5) == "-01"),]
    first_days$Date <- as.Date(paste0("2022-",first_days$Jour_annee), format = "%Y-%m-%d")
    
    
    
    # df_mean_overwriteColors_hist_ = df_mean_overwriteColors[which(df_mean_overwriteColors$Type == "Historical"),]
    # df_mean_overwriteColors_saf_ = df_mean_overwriteColors[which(df_mean_overwriteColors$Type == "Safran"),]
    
    p_mean <- ggplot() +
      geom_line(data = df_mean_hist_, aes(x = Jour_annee, y = Mean_ValueLissee, group = Variable, color = Legend, linetype = "solid"), alpha = 0.15, lwd = 2) + # linetype = Type, 
      geom_line(data = df_mean_overwriteColors_hist_, aes(x = Jour_annee, y = Mean_ValueLissee, group = Variable, linetype = "solid"), color = "white", alpha = 1, lwd = 4) +
      geom_line(data = df_mean_overwriteColors_hist_, aes(x = Jour_annee, y = Mean_ValueLissee, group = Variable, color = Legend, linetype = "solid"), alpha = 1, lwd = 2) +
      
      # geom_line(data = df_mean_saf_, aes(x = Jour_annee, y = Mean_ValueLissee, group = Variable, color = Legend, linetype = "longdash"), alpha = 0.15, lwd = 2) + # linetype = Type, 
      # geom_line(data = df_mean_overwriteColors_saf_, aes(x = Jour_annee, y = Mean_ValueLissee, group = Variable, color = Legend, linetype = "longdash"), alpha = 1, lwd = 2) +
      
      scale_color_manual(name = "Modèles GCM-RCM",
                         values = levels(df_mean_saf_$Color))+
      ...
      labs(title = paste0("Moyenne des probabilités d'assecs lissées sur 5 jours (1977-2004)\nModèle hydrologique ",str_before_first(nom_categorieSimu_,"_")," - HER ",HER_h_),
           x = "Date",
    ...
    svg_device <- svglite(paste0(folder_output_,
                                 ...
                                 nomSim_,
                                 ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                                 "/Chroniques/ProbaMeanValue_rcp85_19772004_HER",HER_h_,"_1_20240227.svg"),
                          width = 15)
    print(p_mean)
    dev.off()
    saveRDS(p_mean, file = paste0(folder_output_,
                                  ...
                                  nomSim_,
                                  ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                                  "/Chroniques/ProbaMeanValue_rcp85_19772004_HER",HER_h_,"_1_20240227.rds"))
    
    ### Mean - English ###
    p_mean <- ggplot() +
      geom_line(data = df_mean_hist_, aes(x = Jour_annee, y = Mean_ValueLissee, group = Variable, color = Legend_English, linetype = "solid"), alpha = 0.15, lwd = 2) + # linetype = Type, 
      geom_line(data = df_mean_overwriteColors_hist_, aes(x = Jour_annee, y = Mean_ValueLissee, group = Variable, color = Legend_English, linetype = "solid"), alpha = 1, lwd = 2) +
      
      geom_line(data = df_mean_saf_, aes(x = Jour_annee, y = Mean_ValueLissee, group = Variable, color = Legend_English, linetype = "longdash"), alpha = 0.15, lwd = 2) + # linetype = Type, 
      geom_line(data = df_mean_overwriteColors_saf_, aes(x = Jour_annee, y = Mean_ValueLissee, group = Variable, color = Legend_English, linetype = "longdash"), alpha = 1, lwd = 2) +
      
      scale_color_manual(name = "GCM-RCM Models",
                         values = levels(df_mean_saf_$Color))+
      scale_linetype_manual(values = as.factor(c("solid","longdash")),
                            name = "Climate Input Data",
                            labels = c("Historical GCM-RCM","Safran"))+
      
      geom_point(data = first_days, aes(x = Date, y = Mean_ValueLissee), size = 0, alpha = 0) +
      scale_x_date(date_breaks = "1 month", date_labels = "%b") +
      
      labs(title = paste0("Mean smoothed drought probabilities over 5 days (1977-2004)\nHydrological Model ",str_before_first(nom_categorieSimu_,"_")," - HER ",HER_h_),
           x = "Date",
           y = "Mean drought probabilities") +
      theme_minimal() +
      ylim(0,85) +
      theme(strip.text.x = element_blank(),
            strip.background = element_blank(),
    ...
    svg_device <- svglite(paste0(folder_output_,
                                 ...
                                 nomSim_,
                                 ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                                 "/Chroniques/Eng/ProbaMeanValue_rcp85_19772004_HER",HER_h_,"_Eng_1_20240227.svg"),
                          width = 15)
    print(p_mean)
    dev.off()
    saveRDS(p_mean, file = paste0(folder_output_,
                                  ...
                                  nomSim_,
                                  ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                                  "/Chroniques/Eng/ProbaMeanValue_rcp85_19772004_HER",HER_h_,"_Eng_1_20240227.rds"))
    ...
    ### Median - French ###
    p_median <- ggplot() +
      geom_line(data = df_mean_hist_, aes(x = Jour_annee, y = Median_ValueLissee, group = Variable, color = Legend, linetype = "solid"), alpha = 0.15, lwd = 2) + # linetype = Type, 
      geom_line(data = df_mean_overwriteColors_hist_, aes(x = Jour_annee, y = Median_ValueLissee, group = Variable, color = Legend, linetype = "solid"), alpha = 1, lwd = 2) +
      
      geom_line(data = df_mean_saf_, aes(x = Jour_annee, y = Median_ValueLissee, group = Variable, color = Legend, linetype = "longdash"), alpha = 0.15, lwd = 2) + # linetype = Type, 
      geom_line(data = df_mean_overwriteColors_saf_, aes(x = Jour_annee, y = Median_ValueLissee, group = Variable, color = Legend, linetype = "longdash"), alpha = 1, lwd = 2) +
      ...
      labs(title = paste0("Médiane des probabilités d'assecs lissées sur 5 jours (1977-2004)\nModèle hydrologique ",str_before_first(nom_categorieSimu_,"_")," - HER ",HER_h_),
           x = "Date",
           y = "Médiane des probabilités d'assecs") +
      theme_minimal() +
      ylim(0,85) +
      theme(strip.text.x = element_blank(),
            strip.background = element_blank(),
    ...
    svg_device <- svglite(paste0(folder_output_,
                                 ...
                                 nomSim_,
                                 ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                                 "/Chroniques/ProbaMedianValue_rcp85_19772004_HER",HER_h_,"_1_20240227.svg"),
                          width = 15)
    print(p_median)
    dev.off()
    saveRDS(p_mean, file = paste0(folder_output_,
                                  ...
                                  nomSim_,
                                  ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                                  "/Chroniques/ProbaMedianValue_rcp85_19772004_HER",HER_h_,"_1_20240227.rds"))
    
    
    ### Median - English ###
    p_median <- ggplot() +
      geom_line(data = df_mean_hist_, aes(x = Jour_annee, y = Median_ValueLissee, group = Variable, color = Legend_English, linetype = "solid"), alpha = 0.15, lwd = 2) + # linetype = Type, 
      geom_line(data = df_mean_overwriteColors_hist_, aes(x = Jour_annee, y = Median_ValueLissee, group = Variable, color = Legend_English, linetype = "solid"), alpha = 1, lwd = 2) +
      
      geom_line(data = df_mean_saf_, aes(x = Jour_annee, y = Median_ValueLissee, group = Variable, color = Legend_English, linetype = "longdash"), alpha = 0.15, lwd = 2) + # linetype = Type, 
      geom_line(data = df_mean_overwriteColors_saf_, aes(x = Jour_annee, y = Median_ValueLissee, group = Variable, color = Legend_English, linetype = "longdash"), alpha = 1, lwd = 2) +
      ...
      labs(title = paste0("Median smoothed drought probabilities over 5 days (1977-2004)\nHydrological Model ",str_before_first(nom_categorieSimu_,"_")," - HER ",HER_h_),
           x = "Date",
           y = "Median drought probabilities") +
      theme_minimal() +
      ylim(0,85) +
      theme(strip.text.x = element_blank(),
            strip.background = element_blank(),
    ...
    svg_device <- svglite(paste0(folder_output_,
                                 ...
                                 nomSim_,
                                 ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                                 "/Chroniques/Eng/ProbaMedianValue_rcp85_19772004_HER",HER_h_,"_Eng_1_20240227.svg"),
                          width = 15)
    print(p_median)
    dev.off()
    saveRDS(p_mean, file = paste0(folder_output_,
                                  ...
                                  nomSim_,
                                  ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                                  "/Chroniques/Eng/ProbaMedianValue_rcp85_19772004_HER",HER_h_,"_Eng_1_20240227.rds"))
    ...
    write.table(df_mean,paste0(folder_output_,
                               ...
                               nomSim_,
                               ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),
                               "/Tables/Table_ProbaMeanValue_rcp85_19772004_HER",HER_h_,"_1_20240214.csv"),
                sep = ";", dec = ".", row.names = F)
