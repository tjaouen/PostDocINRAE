...
for (HER_h_ in HER_){
  ...
  if (HER_h_ %in% colnames(tab_1_)){
    ...
    df_mean_saf_ = df_mean_saf_[which(df_mean_saf_$Variable == unique(df_mean_saf_$Variable)[1]),]
    df_mean_saf_$Variable = "Safran"
    df_mean = rbind(df_mean_hist_,
                    df_mean_saf_)
    ...
    ### Axe x ###
    # first_days <- df_mean[which(substr(df_mean$Jour_annee,3,5) == "-15"),]
    ...
    # first_days$Date <- as.Date(first_days$Jour_annee, format = "%Y-%m-%d")
    ...
    p_mean <- ggplot() +
      ...
      geom_line(data = df_mean_overwriteColors_, aes(x = Jour_annee, y = Mean_ValueLissee, group = Variable), color = "white", alpha = 1, lwd = 4.2, lineend = "round") +
      ...
      scale_x_date("Date",
                   ...
                   # breaks = seq(from=as.Date("2022-01-15"),to=as.Date("2022-12-15"),by="month"),
                   labels = year_labels,
                   expand = c(0,0),
                   minor_breaks=c(seq(from=as.Date("2022-01-01"),to=as.Date("2022-12-01"),by="month"),"2022-12-31")) +
      
      # scale_x_date(labels = labels, breaks = breaks, limits=range(breaks)) + 
      ...
      theme_minimal() +
      theme(axis.line.x = element_line(color = "black", size = 1), # Trait des abscisses plus épais
            #axis.ticks.x = element_line(color = "black"),
            axis.ticks.x=element_line(colour=c("black",rep(c(NA, "black"), t=12))),
            axis.ticks.length  = unit(0.35, "cm"),
            axis.line.y = element_blank(), # Supprimer l'axe y
            axis.text.x = element_text(vjust = 0, hjust=0.5), # angle = 90
            axis.title.x = element_blank(),
            axis.title.y = element_blank(),
            
            panel.grid.major.y = element_line(color = "lightgray", size = 0.5), # Traits horizontaux plus clairs
            panel.grid.minor.y = element_line(color = "lightgray", size = 0.5), # Traits horizontaux plus clairs
    ...
    p_mean <- ggplot() +
      ...
      # Axes #
      scale_x_date("Date",
                   breaks = c(seq(from=as.Date("2022-01-01"),to=as.Date("2022-12-31"),by="month")),
                   labels = year_labels_english,
                   expand = c(0,0)) +
      scale_y_continuous(expand = expansion(mult = c(0, 0.05)),
                         limits = c(0,70),
      ...
      theme_minimal() +
      theme(axis.line.x = element_line(color = "black", size = 1), # Thicker x-axis line
            axis.line.y = element_blank(), # Remove y-axis line
            axis.text.x = element_text(vjust = 0, hjust=0.5), # angle = 90
            axis.title.x = element_blank(),
            axis.title.y = element_blank(),
            
            panel.grid.major.y = element_line(color = "lightgray", size = 0.5), # Lighter horizontal lines
            panel.grid.minor.y = element_line(color = "lightgray", size = 0.5), # Lighter horizontal lines
            panel.grid.major.x = element_blank(), # Lighter horizontal lines
            panel.grid.minor.x = element_blank(), # Lighter horizontal lines
    ...
    p_median <- ggplot() +
      ...
      geom_line(data = df_mean_overwriteColors_, aes(x = Jour_annee, y = Median_ValueLissee, group = Variable), color = "white", alpha = 1, lwd = 4.2, lineend = "round") +
    ...
    p_median <- ggplot() +
      ...
      theme(axis.line.x = element_line(color = "black", size = 1), # Thicker x-axis line
            ...
            panel.grid.major.y = element_line(color = "lightgray", size = 0.5), # Lighter horizontal grid lines
            panel.grid.minor.y = element_line(color = "lightgray", size = 0.5), # Lighter horizontal grid lines
