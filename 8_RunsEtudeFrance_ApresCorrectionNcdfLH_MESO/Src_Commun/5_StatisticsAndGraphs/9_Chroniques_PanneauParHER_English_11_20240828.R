...
for (HER_h_ in HER_){
  ...
  map_HER_h_$layers <- map_HER_h_$layers[-c(3,4)]
  map_HER_h_ <- map_HER_h_ +
    annotation_scale(location = "bl",width_hint = 0.25,style = "ticks",text = element_blank(),line_col = "gray38",
                     text_col = "gray38",bar_cols = c("gray38", "gray38")) +
    annotation_north_arrow(height = unit(1, "cm"),width = unit(0.7, "cm"),location = "tr",which_north = "true",
  ...
  map_HER_h_$layers[[3]]$geomaes_params$pad_x = unit(-2,"cm")
  map_HER_h_$layers[[4]]$aes_params$pad_x = unit(-2,"cm")
  
  print(paste0("HER : ",HER_h_))
  ### Import chronicles ###
  print("Map OK")
  
  graph_legend_ <- readRDS("/media/tjaouen/Ultra Touch/Backup/Main/Output/ChangementClimatique2019/EtudeFrance/22_GrapheChroniqueProbabilite_OneModelHorizon20702100_Projections/32_ObservesReanalyseSafran_ApprentissageTousScenariosProjections_20240208/GRSD_20231128/ChroniquesCombinees_saf_hist_rcp85/Chroniques/20712098_Legend/Eng/ProbaMeanValue_rcp85_20712098_HER2_Eng_1_20240227.rds")
  ...
  rcp85_H0_l_$guides$colour$ncol = 1
  
  rcp85_H0_l_ <- rcp85_H0_l_ + theme(legend.spacing.y = unit(0.2,"cm"))
  
  
  grid_legend <- grid.arrange(
  ...
  grid_1_ <- grid.arrange(
    ...
    arrangeGrob(
      ...
      ncol = 5,
      widths = c(5,w_col2_*2,5,w_col5_*2/5,w_col5_*3/5),
      padding = unit(-0.5, "cm")))
  ...
  # )
  grid_bottom_right <- grid.arrange(
    # Ligne 6
    arrangeGrob(
      ...
      ncol = 4,
      widths = c(w_col1_,w_col3_,w_col4_,w_col5_),
      padding = unit(0.1, "cm")),
    ...
    nrow = 2,
    heights = c(h_graph_,h_graph_),
    padding = unit(0.1, "cm")  # ajuster la marge
  )
  
  
  grid_bottom <- grid.arrange(
    arrangeGrob(
      grid_legend,
      grid_bottom_right,
      ncol = 2,
      widths = c(w_col2_,sum(w_col1_,w_col3_,w_col4_,w_col5_))),
    nrow = 1,
    heights = c(h_graph_*2)
  )
  ...
  grid_graph_ <- grid.arrange(
    ...
    arrangeGrob(
      ...
      ncol = 5,
      widths = c(w_col2_,w_col1_,w_col3_,w_col4_,w_col5_),
      padding = unit(0.1, "cm")),
  ...
  grid_totale_ <- grid.arrange(
    grid_1_,
    grid_graph_,
    heights = c(15,h_line4_+h_graph_*3))
  ...
  ggsave(paste0(folder_output_DD_,
         ...
         plot = grid_totale_, width=14, height = 10)
