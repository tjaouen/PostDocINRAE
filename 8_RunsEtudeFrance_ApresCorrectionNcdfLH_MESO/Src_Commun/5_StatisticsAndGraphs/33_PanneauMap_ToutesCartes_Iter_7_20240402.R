...
for (nom_categorieSimu_ in nom_categorieSimu_list_){
  ...
  data_frame_graphes_ <- data.frame(nomCarte_ = c("nbMoyenJoursParAnSup10pct_ProjMediane","propAssecMoyenneJuilletOct_ProjMediane","dateMedianeDebutSup10pct_ProjMediane","dateMedianeFinSup10pct_ProjMediane"),
                                    title_ = c(paste0("Nombre moyen de jours par an avec une proportion\nd'assec supérieure à 10% (projection médiane)"),
                                               paste0("Probabilités moyennes d'assecs\nen juillet-octobre (projection médiane)"),
                                               paste0("Date médiane de la première proportion d'assec\nsupérieure à 10% (à partir d'avril, projection médiane)"),
                                               paste0("Date médiane de la dernière proportion d'assec\nsupérieure à 10% (à partir d'avril, projection médiane)")),
                                    subtitle_ = c(paste0("Modèle hydrologique : ",str_before_first(nom_categorieSimu_,"_"), " - Corr. Biais : ADAMONT"),
                                                  paste0("Modèle hydrologique : ",str_before_first(nom_categorieSimu_,"_"), " - Corr. Biais : ADAMONT"),
                                                  paste0("Modèle hydrologique : ",str_before_first(nom_categorieSimu_,"_"), " - Corr. Biais : ADAMONT"),
                                                  paste0("Modèle hydrologique : ",str_before_first(nom_categorieSimu_,"_"), " - Corr. Biais : ADAMONT")))
  ...
  for (df_line_ in 1:nrow(data_frame_graphes_)){
    ...
    rcp26_H0_ <- rcp26_H0_ + theme(legend.position =  "bottom",
                                   legend.text = element_text(colour = "grey38", size = 10),
    ...
    parameters_annotationscale_ <- rcp26_H0_$layers[[2]]$geom_params
    
    labels_name_ <- rcp26_H0_$scales$scales[[1]]$breaks
    ...
    legend_north_ <- rcp26_H0_
    legend_north_$layers <- rcp26_H0_$layers[-c(1,2)]
    ...
    rcp26_H0_$theme$legend.text$vjust = 0.5
    
    
    grid_legendAndNorth_ <- grid.arrange(
      arrangeGrob(
        cowplot::get_legend(rcp26_H0_),
    ...
    grid_1_ <- grid.arrange(
      ...
      arrangeGrob(
        nullGrob(),
        textGrob("Scénarios d’émissions Référence (1976-2005)", vjust = 0, gp = gpar(col = "grey38", fontsize = 16)),
        # textGrob("Horizon proche (2021-2050)", vjust = 2, gp = gpar(col = "grey38", fontsize = 13)),
        textGrob("Horizon moyen (2041-2070)", vjust = 0, gp = gpar(col = "grey38", fontsize = 16)),
        textGrob("Horizon lointain (2071-2100)", vjust = 0, gp = gpar(col = "grey38", fontsize = 16)),
      ...
      # Ligne 5
      arrangeGrob(
        textGrob("RCP 2.6",vjust = 1, gp = gpar(col = "grey38", fontsize = 16), rot = 90),
      ...
      # Ligne 6
      arrangeGrob(
        textGrob("RCP 4.5",vjust = 1, gp = gpar(col = "grey38", fontsize = 16), rot = 90),
      ...
      # Ligne 7
      arrangeGrob(
        textGrob("RCP 8.5",vjust = 1, gp = gpar(col = "grey38", fontsize = 16), rot = 90),
    ...
    ggsave(paste0(folder_output_DD_,
                  ...
                  nomSim_,
                  ifelse(nom_categorieSimu_=="","",paste0("/",str_before_first(nom_categorieSimu_,"/"))),
                  "/PanneauMap/Mean_French/ProbaBoard_",
                  ifelse(nom_categorieSimu_=="","",str_before_first(nom_categorieSimu_,"/")),"_",
                  nomCarte_,"_French_1_20240330.svg"),
    ...
    ggsave(paste0(folder_output_DD_,
                  ...
                  "/PanneauMap/Mean_French/ProbaBoard_",
                  ifelse(nom_categorieSimu_=="","",str_before_first(nom_categorieSimu_,"/")),"_",
                  nomCarte_,"_French_1_20240330.png"),
           plot = grid_1_)
    ...
    ggsave(paste0(folder_output_DD_,
                  ...
                  "/PanneauMap/Mean_French/ProbaBoard_",
                  ifelse(nom_categorieSimu_=="","",str_before_first(nom_categorieSimu_,"/")),"_",
                  nomCarte_,"_French_1_20240330.pdf"),
           plot = grid_1_) # width=15
