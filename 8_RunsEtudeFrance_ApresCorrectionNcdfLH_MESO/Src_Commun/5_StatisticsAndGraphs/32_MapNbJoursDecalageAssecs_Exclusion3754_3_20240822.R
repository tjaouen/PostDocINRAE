...
for (nom_categorieSimu_ in vect_){
  ...
  for (pattern_ in c(H0_,H2_,H3_)){
    ...
    #   table_globale <- table_globale[which(!(table_globale$HER %in% HER_eliminees_J2000)),]
    #   # table_globale[which(table_globale$HER %in% HER_eliminees_J2000),
    #   #               which(colnames(table_globale) != "HER")] <- NA
    # }
