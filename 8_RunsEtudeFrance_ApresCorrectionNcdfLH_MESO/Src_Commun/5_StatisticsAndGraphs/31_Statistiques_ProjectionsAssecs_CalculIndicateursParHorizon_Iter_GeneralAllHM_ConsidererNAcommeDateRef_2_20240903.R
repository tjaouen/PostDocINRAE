...
for (date_intervalle_ in list(c("1976-01-01","2005-12-31"),
  ...
  #                             propAssecMoyenneJuilletOct_ModeleMedian_CTRIP = NA,
  #                             propAssecMoyenneJuilletOct_ModeleMedian_GRSD = NA,
  #                             propAssecMoyenneJuilletOct_ModeleMedian_J2000 = NA,
  #                             propAssecMoyenneJuilletOct_ModeleMedian_ORCHIDEE = NA,
  #                             propAssecMoyenneJuilletOct_ModeleMedian_SMASH = NA)
  tab_allModels <- data.frame(HER = HER_,
    ...
    for (HER_h_ in HER_){
      ...
      for (nom_categorieSimu_ in nom_categorieSimu_list_){
        ...
        # print(tab_ProbaMeanJuilOct_HER_$Year)
      ...
      initDate_list_median_ <- lapply(tab_ProbaMeanJuilOct_HER_generale_[which(tab_ProbaMeanJuilOct_HER_generale_$Year >= min(year(date_intervalle_)) &
                                                                         ...
                                                                         2:ncol(tab_ProbaMeanJuilOct_HER_generale_)], function(x) median(x,na.rm=T))
      # initDate_list_median_ <- median(unlist(tab_ProbaMeanJuilOct_HER_generale_))
      ...
      indicateurs_$propAssecMoyenneJuilletOct_ModeleMedian_[which(indicateurs_$HER == HER_h_)] <- initDate_list_median_
    }
    tab_allModels[which(tab_allModels$HER == HER_h_),paste0("propAssecMoyenneJuilletOct_ModeleMedian_",str_before_first(nom_categorieSimu_,"_"))] = round(max(means_, na.rm = T))
    
}
  
  tab_allModels <- merge(tab_allModels,indicateurs_)
