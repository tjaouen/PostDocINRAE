...
if (grepl("J2000",nom_categorieSimu_)){
  HER_eliminees_J2000 <- c("10", "21", "22", "24", "27", "34", "35", "36", "57", "59", "61", "65", "67", "68",
                           ...
                           "55", "12", "53")
  HER_ <- HER_[!HER_ %in% HER_eliminees_J2000]
}
...
# for (HER_h_ in HER_[2:length(HER_)]){
...
output <- foreach(HER_h_ = HER_) %dopar% { #, errorhandling='pass'
  ...
  if (HER_h_ %in% colnames(tab_1_)){
    ...
    svg_device <- svglite(paste0(folder_output_,"22_GrapheChroniqueProbabilite_OneModelHorizon20702100_Projections/",
                                 nomSim_,"/",
                                 nom_categorieSimu_,
                                 "Chroniques/ProbaMeanValue_rcp85_HER",HER_h_,"_1_20240214.svg"),
                          width = 15)
    print(p_mean)
    dev.off()
    ...
    p_median <- ggplot() +
      ...
      geom_line(data = df_mean_overwriteColors, aes(x = Jour_annee, y = Median_Value, group = Variable, color = Legend), alpha = 1, lwd = 2) +
    ...
    svg_device <- svglite(paste0(folder_output_,"22_GrapheChroniqueProbabilite_OneModelHorizon20702100_Projections/",
                                 nomSim_,"/",
                                 nom_categorieSimu_,
                                 "Chroniques/ProbaMedianValue20702100_rcp85_HER",HER_h_,"_1_20240214.svg"),
                          width = 15)
    print(p_median)
    dev.off()
    
    write.table(df_mean, paste0(folder_output_,"22_GrapheChroniqueProbabilite_OneModelHorizon20702100_Projections/",
                                nomSim_,"/",
                                nom_categorieSimu_,
                                "Tables/Table_ProbaMeanValue20702100_rcp85_HER",HER_h_,"_1_20240214.csv"),
                sep = ";", dec = ".", row.names = F)
