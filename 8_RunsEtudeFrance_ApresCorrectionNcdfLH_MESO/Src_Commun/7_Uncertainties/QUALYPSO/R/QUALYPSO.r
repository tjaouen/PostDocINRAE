...
saveGraph_ <- function(output_name_, graph_){
  pdf(paste0(output_name_,".pdf"),
      width = 18)
  plot(graph_)
  dev.off()
  
  saveRDS(graph_, file = paste0(output_name_,".rds"))
...
#'
#' @author Guillaume Evin
create_plot_plotQUALYPSOclimateResponse <- function(){
...
plotQUALYPSOclimateResponse = function(QUALYPSOOUT,lim=NULL,xlab="X",ylab="Y",outputName_,...){
  ...
  for(iS in 1:nS){
    ...
    p <- ggplot() +
      geom_line(data = long_data[long_data$Type == "Ys", ], aes(x = X, y = Value), lwd = 1, color = "blue") +
      geom_line(data = long_data[long_data$Type == "phis", ], aes(x = X, y = Value), lwd = 3, color = "red") +
      labs(
        title = paste0(scenAvail[iS,], collapse = " / "),
        x = xlab,
        y = ylab
      ) +
      scale_x_continuous(limits = range(c(Xs, Xfut))) +
      scale_y_continuous(limits = range(c(Ys, phis))) +
      theme_minimal() +
      theme(plot.title = element_text(color = "#060403", size = 30, face = "bold"),
            ...
            legend.title = element_blank(), #, face = "normal"
    ...
    saveGraph_(output_name_ = outputName_,
               graph_ = p)
      ...
      #   name = NULL,
      #   values = c("blue", "red"),
      #   labels = c("Raw projection", "Climate response")
    ...
    #        lty=1,lwd=c(1,3),bty="n")
    # 
    # saveGraph_(output_name_, create_plot_plotQUALYPSOclimateResponse)

    # png(paste0(".png"), width = 800, height = 600)
    # create_plot_plotQUALYPSOclimateResponse()
    # dev.off()
    # 
    # # Sauvegarder en SVG
...
plotQUALYPSOgrandmean = function(QUALYPSOOUT,lim=NULL,col='black',xlab="",
                                 ylab="Grand mean",addLegend=T,outputName_,...){
  # vector of predictors
  Xfut = QUALYPSOOUT$Xfut
  ...
  p <- ggplot(data, aes(x = Xfut)) +
    geom_ribbon(aes(ymin = binf, ymax = bsup, fill = "95% CI"), alpha = 0.2) +
    geom_line(aes(y = meanPred, color = "Median"), size = 1) +
    labs(x = xlab, y = ylab) +
    theme_minimal() +
    theme(
      ...
      axis.ticks.x = element_line(colour = c("#2f2f32", rep(c("#2f2f32", "#2f2f32"), times = 12)), size = 1 * ratio_epaisseurs_),
      axis.ticks.length  = unit(0.4, "cm"),
    ...
    scale_fill_manual(name = "", values = colPoly) +
    scale_color_manual(name = "", values = col) +
    guides(
      fill = guide_legend(order = 1, override.aes = list(linetype = 0, shape = NA)),
...
#'
#' @author Guillaume Evin
# Fonction pour formater les labels de l'axe y
...
plotQUALYPSOeffect = function(QUALYPSOOUT,nameEff,includeMean=FALSE,lim=NULL,
                              col=1:20,xlab="",ylab="Effect",addLegend=TRUE,outputName_,ylim=NULL,
                              ...){
  # vector of predictors
  Xfut = QUALYPSOOUT$Xfut
  ...
  data <- data.frame(
    Xfut = rep(Xfut, nEff),
    meanPred = as.vector(meanPred),
    # lower = as.vector(EffHat$CI[,1,]),
    # upper = as.vector(EffHat$CI[,2,]),
    Effect = factor(rep(1:nEff, each = length(Xfut)))
  )
  
  if(is.null(ylim)) ylim = c(min(binf),max(bsup))
  
  # # Create ggplot
  # p <- ggplot(data, aes(x = Xfut, group = Effect, color = Effect)) +
  #   # geom_ribbon(aes(ymin = lower, ymax = upper, fill = Effect), alpha = 0.2) +
  #   geom_line(aes(y = meanPred), size = 1) +
  #   scale_y_continuous(limits = ylim) +  # Set y limits
  #   labs(x = xlab, y = ylab) +
  #   theme_minimal() +
  #   theme(
  #     plot.title = element_text(color = "#060403", size = 30, face = "bold"),
  ...
  #     axis.line.y = element_blank(),
  #     axis.ticks.x = element_line(colour = c("#2f2f32", rep(c("#2f2f32", "#2f2f32"), times = 12)), size = 1),
  #     axis.ticks.length = unit(0.4, "cm"),
  #     panel.grid.major.x = element_blank(),
  #     panel.grid.minor.x = element_blank(),
  ...
  #   scale_fill_manual(values = adjustcolor(col, alpha.f = 0.2)) +
  #   guides(
  #     # fill = guide_legend(override.aes = list(linetype = 0, shape = NA)),
  #     color = guide_legend(override.aes = list(linetype = 1, size = 1))
  #   )
  
  # Ajouter une colonne avec le numéro de la ligne
  if (nameEff == "GCM_RCM"){
    data$row_num <- NA
    data$row_num[which(data$Xfut == 2099)] = data$Effect[which(data$Xfut == 2099)]
    
    data$Variable <- QUALYPSOOUT$listScenarioInput$listEff[[1]][data$Effect]
    
    # Sans les narratifs #
    data <- data %>%
      mutate(Color = case_when(
        grepl("CNRM.*CERFACS.*CNRM.*ALADIN", Variable) ~ "#E5E840",
        grepl("CNRM.*CERFACS.*MOHC.*HadREM3", Variable) ~ "black",
        grepl("ICHEC.*EC.*EARTH.*KNMI.*RACMO22E", Variable) ~ "black",
        grepl("ICHEC.*EC.*EARTH.*MOHC.*HadREM3", Variable) ~ "#E2A138",
        grepl("ICHEC.*EC.*EARTH.*SMHI.*RCA4", Variable) ~ "black",
        grepl("IPSL.*IPSL.*CM5A.*MR.*DMI.*HIRHAM5", Variable) ~ "black",
        grepl("IPSL.*IPSL.*CM5A.*MR.*SMHI.*RCA4", Variable) ~ "black",
        grepl("MOHC.*HadGEM2.*ES.*CLMcom.*CCLM4", Variable) ~ "#70194E",
        grepl("MOHC.*HadGEM2.*ES.*CNRM.*ALADIN", Variable) ~ "#447C57",
        grepl("MOHC.*HadGEM2.*ES.*ICTP.*RegCM4", Variable) ~ "black",
        grepl("MOHC.*HadGEM2.*ES.*MOHC.*HadREM3.*GA7", Variable) ~ "black",
        grepl("MPI.*M.*MPI.*ESM.*LR.*CLMcom.*CCLM4", Variable) ~ "black",
        grepl("MPI.*M.*MPI.*ESM.LR.*ICTP.*RegCM4", Variable) ~ "black",
        grepl("MPI.*M.*MPI.*ESM.*LR.*MPI.*CSC.*REMO2009", Variable) ~ "black",
        grepl("NCC.*NorESM1.*M.*DMI.*HIRHAM5", Variable) ~ "black",
        grepl("NCC.*NorESM1.*M.*GERICS.*REMO2015", Variable) ~ "black",
        grepl("NCC.*NorESM1.*M.*IPSL.*WRF381P", Variable) ~ "black",
        grepl("Safran", Variable) ~ "#253494"))
    
    data <- data %>%
      mutate(Alpha = case_when(
        grepl("CNRM.*CERFACS.*CNRM.*ALADIN", Variable) ~ 1,
        grepl("CNRM.*CERFACS.*MOHC.*HadREM3", Variable) ~ 0.15,
        grepl("ICHEC.*EC.*EARTH.*KNMI.*RACMO22E", Variable) ~ 0.15,
        grepl("ICHEC.*EC.*EARTH.*MOHC.*HadREM3", Variable) ~ 1,
        grepl("ICHEC.*EC.*EARTH.*SMHI.*RCA4", Variable) ~ 0.15,
        grepl("IPSL.*IPSL.*CM5A.*MR.*DMI.*HIRHAM5", Variable) ~ 0.15,
        grepl("IPSL.*IPSL.*CM5A.*MR.*SMHI.*RCA4", Variable) ~ 0.15,
        grepl("MOHC.*HadGEM2.*ES.*CLMcom.*CCLM4", Variable) ~ 1,
        grepl("MOHC.*HadGEM2.*ES.*CNRM.*ALADIN", Variable) ~ 1,
        grepl("MOHC.*HadGEM2.*ES.*ICTP.*RegCM4", Variable) ~ 0.15,
        grepl("MOHC.*HadGEM2.*ES.*MOHC.*HadREM3.*GA7", Variable) ~ 0.15,
        grepl("MPI.*M.*MPI.*ESM.*LR.*CLMcom.*CCLM4", Variable) ~ 0.15,
        grepl("MPI.*M.*MPI.*ESM.LR.*ICTP.*RegCM4", Variable) ~ 0.15,
        grepl("MPI.*M.*MPI.*ESM.*LR.*MPI.*CSC.*REMO2009", Variable) ~ 0.15,
        grepl("NCC.*NorESM1.*M.*DMI.*HIRHAM5", Variable) ~ 0.15,
        grepl("NCC.*NorESM1.*M.*GERICS.*REMO2015", Variable) ~ 0.15,
        grepl("NCC.*NorESM1.*M.*IPSL.*WRF381P", Variable) ~ 0.15,
        grepl("Safran", Variable) ~ 1))

    data <- data %>%
      mutate(Legend = case_when(
        grepl("CNRM.*CERFACS.*CNRM.*ALADIN", Variable) ~ "Modéré en réchauffement et\nen changement de précipitations",
        grepl("CNRM.*CERFACS.*MOHC.*HadREM3", Variable) ~ "Autres modèles",
        grepl("ICHEC.*EC.*EARTH.*KNMI.*RACMO22E", Variable) ~ "Autres modèles",
        grepl("ICHEC.*EC.*EARTH.*MOHC.*HadREM3", Variable) ~ "Sec toute l'année,\nrecharge moindre en hiver",
        grepl("ICHEC.*EC.*EARTH.*SMHI.*RCA4", Variable) ~ "Autres modèles",
        grepl("IPSL.*IPSL.*CM5A.*MR.*DMI.*HIRHAM5", Variable) ~ "Autres modèles",
        grepl("IPSL.*IPSL.*CM5A.*MR.*SMHI.*RCA4", Variable) ~ "Autres modèles",
        grepl("MOHC.*HadGEM2.*ES.*CLMcom.*CCLM4", Variable) ~ "Fort réchauffement et\nfort assèchement en été",
        grepl("MOHC.*HadGEM2.*ES.*CNRM.*ALADIN", Variable) ~ "Chaud et humide\nà toutes les saisons",
        grepl("MOHC.*HadGEM2.*ES.*ICTP.*RegCM4", Variable) ~ "Autres modèles",
        grepl("MOHC.*HadGEM2.*ES.*MOHC.*HadREM3.*GA7", Variable) ~ "Autres modèles",
        grepl("MPI.*M.*MPI.*ESM.*LR.*CLMcom.*CCLM4", Variable) ~ "Autres modèles",
        grepl("MPI.*M.*MPI.*ESM.LR.*ICTP.*RegCM4", Variable) ~ "Autres modèles",
        grepl("MPI.*M.*MPI.*ESM.*LR.*MPI.*CSC.*REMO2009", Variable) ~ "Autres modèles",
        grepl("NCC.*NorESM1.*M.*DMI.*HIRHAM5", Variable) ~ "Autres modèles",
        grepl("NCC.*NorESM1.*M.*GERICS.*REMO2015", Variable) ~ "Autres modèles",
        grepl("NCC.*NorESM1.*M.*IPSL.*WRF381P", Variable) ~ "Autres modèles",
        grepl("Safran", Variable) ~ "Safran"))
    
    data <- data %>%
      mutate(Legend_num = case_when(
        grepl("CNRM.*CERFACS.*CNRM.*ALADIN", Variable) ~ "2",
        grepl("CNRM.*CERFACS.*MOHC.*HadREM3", Variable) ~ "1",
        grepl("ICHEC.*EC.*EARTH.*KNMI.*RACMO22E", Variable) ~ "1",
        grepl("ICHEC.*EC.*EARTH.*MOHC.*HadREM3", Variable) ~ "3",
        grepl("ICHEC.*EC.*EARTH.*SMHI.*RCA4", Variable) ~ "1",
        grepl("IPSL.*IPSL.*CM5A.*MR.*DMI.*HIRHAM5", Variable) ~ "1",
        grepl("IPSL.*IPSL.*CM5A.*MR.*SMHI.*RCA4", Variable) ~ "1",
        grepl("MOHC.*HadGEM2.*ES.*CLMcom.*CCLM4", Variable) ~ "4",
        grepl("MOHC.*HadGEM2.*ES.*CNRM.*ALADIN", Variable) ~ "5",
        grepl("MOHC.*HadGEM2.*ES.*ICTP.*RegCM4", Variable) ~ "1",
        grepl("MOHC.*HadGEM2.*ES.*MOHC.*HadREM3.*GA7", Variable) ~ "1",
        grepl("MPI.*M.*MPI.*ESM.*LR.*CLMcom.*CCLM4", Variable) ~ "1",
        grepl("MPI.*M.*MPI.*ESM.LR.*ICTP.*RegCM4", Variable) ~ "1",
        grepl("MPI.*M.*MPI.*ESM.*LR.*MPI.*CSC.*REMO2009", Variable) ~ "1",
        grepl("NCC.*NorESM1.*M.*DMI.*HIRHAM5", Variable) ~ "1",
        grepl("NCC.*NorESM1.*M.*GERICS.*REMO2015", Variable) ~ "1",
        grepl("NCC.*NorESM1.*M.*IPSL.*WRF381P", Variable) ~ "1",
        grepl("Safran", Variable) ~ "1"))
    ...
    ### Legende ###
    variables_a_afficher <- unique(data$Legend[which(data$Alpha == 1)])
    data$AfficherDansLegende <- ifelse(data$Legend %in% variables_a_afficher, as.character(data$Legend), "")
    
    desired_order <- c(unique(data$Legend[which(data$Color == "#E5E840")]),
                       unique(data$Legend[which(data$Color == "#E2A138")]),
                       unique(data$Legend[which(data$Color == "#70194E")]),
                       unique(data$Legend[which(data$Color == "#447C57")]),
                       unique(data$Legend[which(data$Color == "#253494")]),
                       "")
    
    data$AfficherDansLegende <- factor(data$AfficherDansLegende, levels = c(desired_order))
    ...
    data_nonNar_ <- data[which(!(data$Variable %in% c("CNRM-CERFACS-CNRM-CM5 + CNRM-ALADIN6","CNRM_CERFACS_CNRM_CM5 + CNRM_ALADIN6",
                                                     "ICHEC-EC-EARTH + MOHC-HadREM3-GA7-05","ICHEC_EC_EARTH + MOHC_HadREM3_GA7_05",
                                                     "MOHC-HadGEM2-ES + CLMcom-CCLM4-8-17","MOHC_HadGEM2_ES + CLMcom_CCLM4_8_17",
                                                     "MOHC-HadGEM2-ES + CNRM-ALADIN6","MOHC_HadGEM2_ES + CNRM_ALADIN6"))),]
    data_nonNar_$Color <- factor(data_nonNar_$Color, levels = intersect(c("#E5E840", "#E2A138", "#70194E", "#447C57","#253494","black"), levels(factor(data_nonNar_$Color))))
    data_nonNar_$Legend <- factor(data_nonNar_$Legend, levels = intersect(desired_order, levels(factor(data_nonNar_$Legend))))
    data_nonNar_$Legend_English <- factor(data_nonNar_$Legend_English, levels = intersect(desired_order_english, levels(factor(data_nonNar_$Legend_English))))
    data_nonNar_$Legend_num <- factor(data_nonNar_$Legend_num, levels = intersect(c("1","2","3","4","5"), levels(factor(data_nonNar_$Legend_num))))
    data_nonNar_$Variable <- factor(data_nonNar_$Variable, levels = unique(data_nonNar_$Variable))
    data_nonNar_$Alpha <- factor(data_nonNar_$Alpha, levels = c(0.15))
    
    data_nar_ <- data[which(data$Variable %in% c("CNRM-CERFACS-CNRM-CM5 + CNRM-ALADIN6","CNRM_CERFACS_CNRM_CM5 + CNRM_ALADIN6",
                                                "ICHEC-EC-EARTH + MOHC-HadREM3-GA7-05","ICHEC_EC_EARTH + MOHC_HadREM3_GA7_05",
                                                "MOHC-HadGEM2-ES + CLMcom-CCLM4-8-17","MOHC_HadGEM2_ES + CLMcom_CCLM4_8_17",
                                                "MOHC-HadGEM2-ES + CNRM-ALADIN6","MOHC_HadGEM2_ES + CNRM_ALADIN6")),]
    data_nar_$Color <- factor(data_nar_$Color, levels = intersect(c("#E5E840", "#E2A138", "#70194E", "#447C57","#253494","black"), levels(factor(data_nar_$Color))))
    data_nar_$Legend <- factor(data_nar_$Legend, levels = intersect(desired_order, levels(factor(data_nar_$Legend))))
    data_nar_$Legend_English <- factor(data_nar_$Legend_English, levels = intersect(desired_order_english, levels(factor(data_nar_$Legend_English))))
    data_nar_$Legend_num <- factor(data_nar_$Legend_num, levels = intersect(c("1","2","3","4","5"), levels(factor(data_nar_$Legend_num))))
    data_nar_$Variable <- factor(data_nar_$Variable, levels = unique(data_nar_$Variable))
    
    
    # Créer le plot ggplot avec les numéros de ligne
    p <- ggplot() +
      geom_line(data = data_nonNar_, aes(x = Xfut, y = meanPred, group = Effect, lineend = "round", alpha = Alpha), lwd = 1) + # linetype = Type, color = Legend, 
      scale_alpha_manual(name = "Climate\nforcing",
                         values = as.numeric(levels(data_nonNar_$Alpha)),
                         labels = levels(data_nonNar_$Legend_num))+
      
      geom_line(data = data_nar_, aes(x = Xfut, y = meanPred, group = Effect, color = Color, lineend = "round"), alpha = 1, lwd = 1) + # linetype = Type,
      scale_color_manual(name = "including\nstory-lines:",
                         values = levels(data_nar_$Color),
                         labels = levels(data_nar_$Legend_num),
                         guide = "legend")+
      scale_y_continuous(limits = ylim,
                         labels = format_y_labels) +  # Set y limits and use custom label function
      # scale_y_continuous(limits = ylim) +  # Set y limits
      labs(x = xlab, y = ylab) +
      theme_minimal() +
      theme(
        ...
        axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        axis.text.x = element_text(color = "#2f2f32", vjust = 0, hjust = c(0,0.5,1), size = 28),
        axis.text.y = element_text(color = "#2f2f32", size = 28),
        axis.line.x = element_line(color = "#2f2f32", size = 1),
        axis.line.y = element_blank(),
        axis.ticks.x = element_line(colour = c("#2f2f32", rep(c("#2f2f32", "#2f2f32"), times = 12)), size = 1),
      ...
      scale_x_continuous(expand = c(0, 0),
                         breaks = c(1990,2050,2099)) +
      # scale_color_manual(values = col) +
      # scale_fill_manual(values = adjustcolor(col, alpha.f = 0.2)) +
      guides(
        alpha = guide_legend(override.aes = list(lwd = 2.5,color =levels(data_nonNar_$Color)),
                             keyheight = 3.2,
                             keywidth = unit(2,"cm"),
                             order = 1),
        color = guide_legend(
    ...
    if (nameEff == "HM"){
      data$Effect_Categorie_ <- QUALYPSOOUT$listScenarioInput$listEff[[3]][data$Effect]
    }
    if (nameEff == "RCP"){
      data$Effect_Categorie_ <- QUALYPSOOUT$listScenarioInput$listEff[[4]][data$Effect]
    }
    if (nameEff == "GCM"){
      data$Effect_Categorie_ <- QUALYPSOOUT$listScenarioInput$listEff[[1]][data$Effect]
    }
    if (nameEff == "RCM"){
      data$Effect_Categorie_ <- QUALYPSOOUT$listScenarioInput$listEff[[2]][data$Effect]
    }
    
    # Créer le plot ggplot avec les numéros de ligne
    p <- ggplot(data, aes(x = Xfut, group = Effect_Categorie_, color = Effect_Categorie_)) +
      # geom_ribbon(aes(ymin = lower, ymax = upper, fill = Effect), alpha = 0.2) +
      geom_line(aes(y = meanPred), size = 1) +
      scale_y_continuous(limits = ylim, labels = format_y_labels) +  # Set y limits and use custom label function
      # scale_y_continuous(limits = ylim) +  # Set y limits
      labs(x = xlab, y = ylab) +
      theme_minimal() +
      theme(
  ...
  # plot(-100,-100,xlim=range(Xfut),xlab=xlab,ylab=ylab,...)
  # # plot(-100,-100,xlim=range(Xfut),ylim=c(lim[1],lim[2]),xlab=xlab,ylab=ylab,...)
  # 
  # for(i in 1:nEff){
  #   # colors polygon
  ...
  #   }
  # 
  #   legend('topleft',bty='n',lwd=2,lty=1,col=col,
...
plotQUALYPSOTotalVarianceDecomposition = function(QUALYPSOOUT,vecEff=NULL,
                                                  col=c("#a6cee3","#1f78b4","#b2df8a","#33a02c","#fdb863","#e66101","#d01c8b"),
                                                  # col=c("orange","yellow","cadetblue1","blue1","darkgreen","darkgoldenrod4","darkorchid1"),
                                                  xlab="",ylab="% Total Variance",addLegend=TRUE,outputName_,...){
  ...
  for (i in 1:(nEff+2)) {
    ...
    temp_data <- data.frame(
      x = c(Xfut, rev(Xfut)),
      y = c(cumPrevious, rev(cum)),
      group = i,
      nom = colnames(VARDECOMP)[i],
      fill = col[i]
      # fill = rev(col)[i]
    )
    
    polygon_data <- rbind(polygon_data, temp_data)
  }
  
  # Convertir en facteur pour ggplot
  # polygon_data$fill <- as.factor(polygon_data$fill)
  polygon_data$nom <- gsub("InternalVar", "Internal\nvariability", polygon_data$nom)
  polygon_data$nom <- gsub("ResidualVar", "Residual\nvariability", polygon_data$nom)
  
  # initial_levels <- c("GCM_RCM", "HM", "RCP", "ResidualVar", "InternalVar")
  if ("GCM_RCM" %in% polygon_data$nom){
    initial_levels <- c("Internal\nvariability", "Residual\nvariability", "RCP", "HM", "GCM_RCM")
    # initial_levels <- c("GCM_RCM", "HM", "RCP", "ResidualVar", "InternalVar")
  }else{
    # initial_levels <- c("RCP", "GCM", "RCM", "HM", "Residual\nvariability", "Internal\nvariability")
    initial_levels <- c("Internal\nvariability", "Residual\nvariability", "RCP", "HM", "RCM", "GCM")
    # initial_levels <- c("GCM", "RCM", "HM", "RCP", "Residual\nvariance", "Internal\nvariance")
  }
  # Filtrer les niveaux en fonction de ce qui est présent dans data$nom
  actual_levels <- initial_levels[initial_levels %in% polygon_data$nom]
  # Appliquer factor avec les niveaux filtrés
  polygon_data$nom <- factor(polygon_data$nom, levels = actual_levels)
  # Trier les données selon les niveaux
  # polygon_data <- data[order(polygon_data$nom), ]
  
  # Créer une palette de couleurs associée à chaque nom
  color_palette <- polygon_data %>% distinct(nom, fill)
  custom_colors <- setNames(as.character(color_palette$fill), as.character(color_palette$nom))

  # Créer le plot ggplot
  p <- ggplot(polygon_data, aes(x = x, y = y, group = nom, fill = nom)) +
    geom_polygon() +
  ...
  for (y in scales::pretty_breaks(n = 5)(c(0, 1))) {
    p <- p + geom_abline(intercept = y, slope = 0, color = "#dcdad9", linetype = "solid", size = 0.3)
  }
...
exportQUALYPSOTotalVarianceDecomposition = function(QUALYPSOOUT,vecEff=NULL,
                                                    col=c("#a6cee3","#1f78b4","#b2df8a","#33a02c","#fdb863","#e66101","#d01c8b"),
                                                    # col=c("orange","yellow","cadetblue1","blue1","darkgreen","darkgoldenrod4","darkorchid1"),
                                                    xlab="",ylab="% Total Variance",addLegend=TRUE,outputName_,...){
  # future predictor values
  ...
  return(VARDECOMP[which(Xfut %in% c(2070,2080,2090,2099)),])
...
plotQUALYPSOTotalVarianceByScenario = function(QUALYPSOOUT,nameEff,nameScenario,
  ...
  # figure
  if(is.null(col)){
    default.col = c("#a6cee3","#1f78b4","#b2df8a","#33a02c","#fdb863","#e66101","#d01c8b")
  ...
  #   geom_polygon(data = data, aes(x = Xfut, y = Y, group = Group, fill = Group), linetype = 1) +
  #   # Ajouter la ligne meanPred
  #   geom_line(data = data.frame(Xfut = Xfut, meanPred = meanPred), aes(x = Xfut, y = meanPred), color = "white", size = 1) +
  #   # Ajuster les limites des axes
  #   scale_x_continuous(limits = range(Xfut), expand = c(0, 0)) +
  #   scale_y_continuous(limits = ylim, expand = c(0, 0)) +
  #   # Ajouter des lignes horizontales
  #   geom_hline(yintercept = axTicks(side = 2), color = "black", size = 0.3, linetype = 1) +
  ...
  #         legend.position = "none") +
  #   # Ajuster les couleurs des polygones
  #   scale_fill_manual(values = col)
  # 
  # # Ajouter la légende si nécessaire
  # if (addLegend) {
  #   namesEff <- QUALYPSOOUT$namesEff[-iEff]
  #   legend_labels <- c(namesEff, 'Res. Var.', 'Int. Variab.')
  #   p <- p + theme(legend.position = "topleft") +
  #     guides(fill = guide_legend(title = NULL, override.aes = list(fill = rev(col)))) +
  #     scale_fill_manual(values = rev(col), labels = legend_labels)
  # }
...
guide_squarekey <- function(...) {
  ...
  x <- guide_legend(reverse = T, legend.spacing.y = unit(1, "cm"), ...)
...
plotQUALYPSOMeanChangeAndUncertainties = function(QUALYPSOOUT,col=NULL,ylim=NULL,
                                                  xlab="",ylab="Change variable",addLegend=TRUE,outputName_,...){
  ...
  data <- data.frame(
    Xfut = rep(Xfut, times = nEff + 2),
    meanPred = rep(meanPred, times = nEff + 2),
    binf = rep(binf, times = nEff + 2),
    bsup = rep(bsup, times = nEff + 2),
    limIntInf = as.vector(t(limIntInf)),
    limIntSup = as.vector(t(limIntSup)),
    group = rep(1:(nEff + 2), each = nFut),
    fill = rep(col, each = nFut),
    nom = rep(rownames(vNormRev), each = nFut)
  )
  data <- data %>%
    mutate(nom = recode(nom,
                        "InternalVar" = "Internal\nvariability",
                        "ResidualVar" = "Residual\nvariability",
                        "RCP" = "RCP",
                        "HM" = "HM",
                        "GCM_RCM" = "GCM+RCM",
                        "GCM" = "GCM",
  ...
  color_palette <- data %>% distinct(nom, fill)
  custom_colors <- setNames(as.character(color_palette$fill), rev(as.character(color_palette$nom)))
  
  # figure
  if(is.null(ylim)) ylim = c(min(binf),max(bsup))
  
  
  # data$nom <- factor(data$nom, levels = c("InternalVar","ResidualVar","RCP","HM","GCM_RCM"))
  # data <- data[order(data$nom), ]
  
  # initial_levels <- c("InternalVar", "ResidualVar", "RCP", "HM", "GCM_RCM")
  if ("GCM_RCM" %in% data$nom){
    initial_levels <- c("InternalVar", "ResidualVar", "RCP", "HM", "GCM_RCM")
    # initial_levels <- c("GCM_RCM", "HM", "RCP", "ResidualVar", "InternalVar")
  }else{
    # initial_levels <- c("RCP", "GCM", "RCM", "HM", "Residual\nvariability", "Internal\nvariability")
  ...
  # Filtrer les niveaux en fonction de ce qui est présent dans data$nom
  actual_levels <- initial_levels[initial_levels %in% data$nom]
  # Appliquer factor avec les niveaux filtrés
  data$nom <- factor(data$nom, levels = actual_levels)
  # Trier les données selon les niveaux
  data <- data[order(data$nom), ]
  ...
  p <- ggplot(data, aes(x = Xfut)) +
    # geom_ribbon(aes(ymin = limIntInf, ymax = limIntSup, fill = nom), alpha = 1) +
    geom_ribbon(aes(ymin = limIntInf, ymax = limIntSup, fill = factor(nom)), alpha = 1) +
    scale_fill_manual(values = custom_colors,
                      guide = "squarekey") +
    geom_line(aes(y = meanPred), color = "#dcdad9", size = 3) +
    labs(x = "", y = "Y Axis Label", fill = "Legend") +
    
    geom_hline(yintercept = scales::pretty_breaks(n = 5)(ylim), color = "#dcdad9", size = 0.5) +
    ...
    theme(
      ...
      axis.title.x = element_blank(),
      axis.title.y = element_blank(),
      axis.text.x = element_text(color = "#2f2f32", vjust = 0, hjust = c(0,0.5,1), size = 34),
      axis.text.y = element_text(color = "#2f2f32", size = 34),
      # axis.text.x = element_text(color = "#2f2f32", vjust = 0, hjust = c(0,0.5,1), size = 28),
      # axis.text.y = element_text(color = "#2f2f32", size = 28),
      axis.line.x = element_line(color = "#2f2f32", size = 1),
      axis.line.y = element_blank(),
      ...
      strip.background = element_blank(),
      legend.title = element_blank(),
      legend.text = element_text(color = "#2f2f32", size = 34),
      legend.margin = margin(-7, 0, 0, 0)#,
      # legend.spacing.y = unit(0.03, "cm")
    ...
    scale_y_continuous(limits = ylim,
                       ...
                       labels = format_y_labels) #+  # Set y limits
    # guides(fill = guide_legend(reverse = TRUE))
    # guides(
    #   fill = guide_legend(
    #     # keywidth = unit(1, "cm"), # Set key width
    #     # keyheight = unit(1, "cm"), # Set key height
    #     label.theme = element_text(size = 18, color = "#2f2f32") # Keep label text size consistent
    #   ))
  # p
      # scale_y_continuous(expand = c(0, 0), breaks = scales::pretty_breaks(n = 5))
