...
plotQUALYPSOeffect = function(QUALYPSOOUT,nameEff,includeMean=FALSE,lim=NULL,
  ...
  if (nameEff == "GCM_RCM"){
    ...
    data_nonNar_ <- data[which(!(data$Variable %in% c("CNRM-CERFACS-CNRM-CM5 + CNRM-ALADIN6",
                                                      "ICHEC-EC-EARTH + MOHC-HadREM3-GA7-05",
                                                      "MOHC-HadGEM2-ES + CLMcom-CCLM4-8-17",
                                                      "MOHC-HadGEM2-ES + CNRM-ALADIN6"))),]
    ...
    data_nar_ <- data[which(data$Variable %in% c("CNRM-CERFACS-CNRM-CM5 + CNRM-ALADIN6",
                                                 "ICHEC-EC-EARTH + MOHC-HadREM3-GA7-05",
                                                 "MOHC-HadGEM2-ES + CLMcom-CCLM4-8-17",
                                                 "MOHC-HadGEM2-ES + CNRM-ALADIN6")),]
    ...
    #   # geom_ribbon(aes(ymin = lower, ymax = upper, fill = Effect), alpha = 0.2) +
    #   geom_line(aes(y = meanPred), size = 1) +
    #   geom_text_repel(aes(y = meanPred, label = row_num), 
    #                   size = 5, 
    #                   force = 5,
    #                   # vjust = -0.5, 
    #                   hjust = -10,
    #                   check_overlap = TRUE) #+  # Ajouter les numéros de ligne
    ...
    p <- ggplot() +
      ...
      # geom_text_repel(aes(y = meanPred, label = Legend_num), 
      #                 size = 5, 
      #                 force = 5,
      #                 # vjust = -0.5, 
      #                 hjust = -10,
...
plotQUALYPSOMeanChangeAndUncertainties = function(QUALYPSOOUT,col=NULL,ylim=NULL,
  ...
  p <- ggplot(data, aes(x = Xfut)) +
    ...
    scale_y_continuous(limits = ylim, expand = c(0, 0), breaks = scales::pretty_breaks(n = 5))  # Set y limits
  p
  # scale_y_continuous(expand = c(0, 0), breaks = scales::pretty_breaks(n = 5))
