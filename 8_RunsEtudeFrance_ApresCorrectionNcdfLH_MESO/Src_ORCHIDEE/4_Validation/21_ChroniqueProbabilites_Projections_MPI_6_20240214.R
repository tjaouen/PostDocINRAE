...
if (Rrank+1 > nFiles_to_use) {
  ...
  # liste_scenarios_ = liste_scenarios_[rank+1]
...
if (! is.null(liste_scenarios_)){
  for (sc_ in liste_scenarios_){
    ...
    for (i in colnames(chroniqueFDCbyHERweighted_hist_)[3:length(chroniqueFDCbyHERweighted_hist_)]){
      ...
      if (length((exp(tab_modeles_HER_$Inter_logit_Learn+chroniqueFDCbyHERweighted_hist_[[as.character(i)]]*tab_modeles_HER_$Slope_logit_Learn)/
                  (1+exp(tab_modeles_HER_$Inter_logit_Learn+chroniqueFDCbyHERweighted_hist_[[as.character(i)]]*tab_modeles_HER_$Slope_logit_Learn)))*100) == 0){
        print(paste0("i=",i," Inter=",round(tab_modeles_HER_$Inter_logit_Learn)," Slope=",round(tab_modeles_HER_$Slope_logit_Learn), " Chro=",chroniqueFDCbyHERweighted_hist_[[as.character(i)]]))
      }
