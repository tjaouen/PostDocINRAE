...
for (code_ONDE in liste_HYDRO[,1]){
  ...
  if (file.exists(paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Predictions/",methode,"/France_Stations_HYDRO/Output_BM_station_HYDRO_",code_ONDE,".csv",sep=""))){
    donnees_st <- read.table(paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Predictions/",methode,"/France_Stations_HYDRO/Output_BM_station_HYDRO_",code_ONDE,".csv",sep=""), sep=";", header = T)

    compt2=0
    for (annee in 2012:2016){
      
      select_an=which(as.numeric(format(as.Date(donnees_st$Date,"%d/%m/%Y"),"%Y"))==annee)
      assec_y=which((donnees_st$PRED[select_an])==1) # Nombre de jour en assec par an
      assec_y_obs=which((donnees_st$OBS[select_an])<=1) # Nombre de jour en assec par an
      ...
      for (j in 1:length(select_an)){
        ...
        if (donnees_st$PRED[select_an[j]]==1){
          if (j==1){
            compt_ev=compt_ev+1
            ...
            if(donnees_st$PRED[select_an[j-1]]!=1){
              compt_ev=compt_ev+1
              compt_dur=compt_dur+1
            } else if (donnees_st$PRED[select_an[j-1]]==1){
              compt_dur=compt_dur+1
            }
          }
        } else if (donnees_st$PRED[select_an[j]]==0){
          if (j!=1){
            if(donnees_st$PRED[select_an[j-1]]==1){
              if (compt_dur>compt_dur_max){
                compt_dur_max=compt_dur
        ...
        if (!is.na(donnees_st$OBS[select_an[j]])){
          if (donnees_st$OBS[select_an[j]]<=1){
            if (j==1){
              ...
              if(!is.na(donnees_st$OBS[select_an[j-1]])){
                if(donnees_st$OBS[select_an[j-1]]>1){
                  compt_ev_obs=compt_ev_obs+1
                  compt_dur_obs=compt_dur_obs+1
                } else if (donnees_st$OBS[select_an[j-1]]<=1){
                  compt_dur_obs=compt_dur_obs+1
                }
              } else {
                compt_ev_obs=compt_ev_obs+1
          ...
          } else if (donnees_st$OBS[select_an[j]]>1){
            if (j!=1){
              if(!is.na(donnees_st$OBS[select_an[j-1]])){
                if(donnees_st$OBS[select_an[j-1]]<=1){
                  if (compt_dur_obs>compt_dur_max_obs){
...
write.table(output,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Predictions/Metrics_PRED_Stations_HYDRO_",methode,"_J30_moy_2012_2016.csv", sep=""), sep=";", row.names = F, col.names = T)
...
write.table(output2,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Predictions/Metrics_OBS_Stations_HYDRO_",methode,"_J30_moy_2012_2016.csv", sep=""), sep=";", row.names = F, col.names = T)
