...
for (j in 30){
  for (year_test in list_year){
    ...
    for (nbnode in list_node){
      ...
      # nn <- neuralnet(f,data=train.cv,hidden=c(3),linear.output=T,stepmax = 1e+06)
      n <- nnet(f,data=train.cv,size=nbnode,linout=T)
      
      yhat10 <- predict(n, train.cv[,2:ncol(train.cv)], type="raw")
      
      # yhat10 <- yhat2$net.result
      ...
      while (seuil <= 1){
        ...
        for (id in 1:length(yhat10)){
          ...
          if(yhat10_seuil[id]==1 & train.cv[id,1]==1){
            TP=TP+1
          } else if (yhat10_seuil[id]==1 & train.cv[id,1]==0){
            FP=FP+1
          } else if (yhat10_seuil[id]==0 & train.cv[id,1]==1){
            FN=FN+1
          } else if (yhat10_seuil[id]==0 & train.cv[id,1]==0){
            TN=TN+1
          }
        }
        seuil=seuil+0.05
        ...
        frame_test[ligne,3]<-nbnode
      }
    }
    
    seuil_opti <- frame_test[which(frame_test[,2]==max(frame_test[,2], na.rm=T)),1]
    node_opti <- frame_test[which(frame_test[,2]==max(frame_test[,2], na.rm=T)),3]
    n <- nnet(f,data=train.cv,size=node_opti,linout=T)
    
    for (code_ONDE in liste_st){
      if (file.exists(paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/Data/Var_exp_station_NEW_30J/Input_",code_ONDE,".txt",sep=""))){
        
        # loading des input de la station à prédire:
        donnees_st<-read.table(paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/Data/Var_exp_station_NEW_30J/Input_",code_ONDE,".txt",sep=""), sep=";", header = T, quote="", stringsAsFactors=F)
        ...
        x.data_st[1:nrow(x.data_st),109] <- donnees[zerocal[1],116]
        x.data_st[1:nrow(x.data_st),110] <- donnees[zerocal[1],118]
        ...
        if (nrow(data_st)>0){
          ...
          yhat <- predict(n, scaled.prd, type="raw")

          seuil = seuil_opti # 0.45
          ...
          # write.table(donnees_st,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Predictions/ANN/J5_",year_test,"/Output_ANN_",code_ONDE,"_",year_test,".csv", sep=""), sep=";", row.names = F, col.names = T)
          write.table(data_st,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Predictions/ANN/J30_moy/Output_ANN_",code_ONDE,".csv", sep=""), sep=";", row.names = F, col.names = T)
        }
      } # boucle file exist
    } # station
  }
