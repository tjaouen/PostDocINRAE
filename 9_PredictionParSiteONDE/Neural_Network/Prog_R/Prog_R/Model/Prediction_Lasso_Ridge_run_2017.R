...
library(glmnet)
library(pROC)
# 33(2) 35(2) 39(1) 43(2) 79(3) 89(2) 73(2) 37(2)
...
for (HERc in HER2[,3]){
  if (HERc != 33 & HERc != 35 & HERc != 39 & HERc != 43 & HERc != 79 & HERc != 89 & HERc != 73 & HERc != 37){
    if (file.exists(paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/Data/Input_Matrice/Input_test_classif_HER_",HERc,"_30_jours_fin_caract_new_meteo_FINAL.txt",sep=""))==T){
      ...
      if(length(unique(donnees[select,127]))==1 & unique(donnees[select,127])==0){
        if (file.exists(paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/Data/Input_Matrice_2017/Input_test_classif_HER_",HERc,"_30_jours_fin_caract_new_meteo_FINAL.txt",sep=""))){
          ...
          if (nrow(data_st)>0){
            ...
            write.table(data_st,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Predictions/LASSO/J30_FRANCE_2017/Output_lasso_",HERc,"_2017.csv", sep=""), sep=";", row.names = F, col.names = T)
        ...
        if (ncol(data)>0){
          ...
          train.cv <- as.matrix(scaled[index,2:ncol(scaled)])
          # test.cv <- scaled[-index,2:ncol(scaled)]
          y.train=scaled[index,1]
          # y.test=scaled[]
          
          assec <- length(which(y.train==1))
          flow <- length(which(y.train==0))
          
          fit10<-cv.glmnet(train.cv, y.train, type.measure="mse", alpha=1, family="binomial", nfolds = 3)
          yhat10 <- predict(fit10, s=fit10$lambda.min, newx=train.cv, type="response")
          ...
          if (file.exists(paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/Data/Input_Matrice_2017/Input_test_classif_HER_",HERc,"_30_jours_fin_caract_new_meteo_FINAL.txt",sep=""))){
            ...
            if (nrow(data_st)>0){
              ...
              POD <- (TP/(TP+FN))*100
              FAR <- (FP/(FP+TP))*100
              accuracy <- ((TP+TN)/nrow(yhat10))*100
              precision <- TP/(TP+FP)
              Recall <- TP/(TP+FN)
              F1.score <- (2*precision*Recall)/(precision+Recall)
      ...
      } # Test assecs observés dans matrice
    } # Test matrice input existe
  }
} # Boucle HER

colnames(critere_full)=c("HERc","POD", "FAR","Precision","Recall","F1_score","seuil_opti")
write.table(critere_full,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Predictions/LASSO/J30_FRANCE_2017/Criteres_lasso_PRED_2017.csv", sep=""), sep=";", row.names = F, col.names = T)
