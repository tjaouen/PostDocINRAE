  ...
  for (j in 30){
    ...
    # for (year_test in 1:30){
      for (param in list_param){
        ...
        # plot(fit5, main="Elastic Net", label = TRUE)
        ...
        while (seuil <= 1){
          ...
          for (id in 1:length(yhat0)){
            ...
            if (yhat0[id] > seuil){
              yhat0_seuil[id]=1
              compt=compt+1
            } else if (yhat0[id] < seuil){
              yhat0_seuil[id]=0
              compt=compt+1
            } else {
              yhat0_seuil[id]=yhat0[id]
            }
            
            if(yhat0_seuil[id]==1 & y.test[id]==1){
              TP1=TP1+1
            } else if (yhat0_seuil[id]==1 & y.test[id]==0){
              FP1=FP1+1
            } else if (yhat0_seuil[id]==0 & y.test[id]==1){
              FN1=FN1+1
            } else if (yhat0_seuil[id]==0 & y.test[id]==0){
              TN1=TN1+1
            }
            ...
            if (yhat5[id] > seuil){
              yhat5_seuil[id]=1
              compt=compt+1
            } else if (yhat5[id] < seuil){
              yhat5_seuil[id]=0
              compt=compt+1
            } else {
              yhat5_seuil[id]=yhat5[id]
            }
            
            if(yhat5_seuil[id]==1 & y.test[id]==1){
              TP2=TP2+1
            } else if (yhat5_seuil[id]==1 & y.test[id]==0){
              FP2=FP2+1
            } else if (yhat5_seuil[id]==0 & y.test[id]==1){
              FN2=FN2+1
            } else if (yhat5_seuil[id]==0 & y.test[id]==0){
              TN2=TN2+1
            }
            ...
            if(yhat10_seuil[id]==1 & y.test[id]==1){
              TP3=TP3+1
            } else if (yhat10_seuil[id]==1 & y.test[id]==0){
              FP3=FP3+1
            } else if (yhat10_seuil[id]==0 & y.test[id]==1){
              FN3=FN3+1
            } else if (yhat10_seuil[id]==0 & y.test[id]==0){
              TN3=TN3+1
            }
          }
          seuil=seuil+0.05
    ...
    write.table(critere_full,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Graph_critères/Ridge_Lasso_Elastic_net/Résultats_locaux_HER_97/critere_seuils_opti_test_2012_2016_FULL_moy_j",j,"_new_meteo_all_caract_lambda_",nblambda,".csv", sep=""), sep=";", row.names = F, col.names = T)
    colnames(coefficient) <- c("2012","2012","2012","2013","2013","2013","2014","2014","2014","2015","2015","2015","2016","2016","2016")
    write.table(coefficient,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Graph_critères/Ridge_Lasso_Elastic_net/Résultats_locaux_HER_97/Coefficient_2012_2016_FULL_moy_j",j,"_new_meteo_all_caract_lambda_",nblambda,".csv", sep=""), sep=";", row.names = F, col.names = T)
