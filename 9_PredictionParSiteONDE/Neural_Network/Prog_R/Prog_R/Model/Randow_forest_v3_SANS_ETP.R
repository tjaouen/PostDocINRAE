  ...
  if (file.exists(paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/Data/Input_Matrice/Input_test_classif_HER_",HERc,"_30_jours_fin_caract_new_meteo_FINAL.txt",sep=""))==T){
    ...
    if (ncol(data)>0){
      ...
      for (year_test in 1:20){
        ...
        fit <- randomForest(Assec ~ ., data = train, ntree=500, na.action = na.omit, importance=T) #
      ...
      # write.table(critere_full,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Graph_critères/Random_forest/Résultats_locaux_FRANCE/critere_seuils_opti_test_2012_2016_FULL_moy_j",j,"_new_meteo_all_caract_HER_",HERc,"_SANS_ETP.csv", sep=""), sep=";", row.names = F, col.names = T)
      # colnames(coefficient) <- c("2012","2013","2014","2015","2016")
      # write.table(coefficient,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Graph_critères/Random_forest/Résultats_locaux_FRANCE/Coefficient_2012_2016_FULL_moy_j",j,"_new_meteo_all_caract_HER_",HERc,"_SANS_ETP.csv", sep=""), sep=";", row.names = F, col.names = T)
      # 
      T2 <- Sys.time()
