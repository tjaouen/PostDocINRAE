...
for (j in 30){
  ...
  for (year_test in list_year){
    for (param in list_param){
      ...
      while (seuil <= 1){
        ...
        for (id in 1:length(yhat)){
          if (yhat[id] > seuil){
            ...
            yhat_seuil[id]=yhat[id]
          }
          
          if(yhat_seuil[id]==1 & test[id,1]==1){
            TP=TP+1
          } else if (yhat_seuil[id]==1 & test[id,1]==0){
            FP=FP+1
          } else if (yhat_seuil[id]==0 & test[id,1]==1){
            FN=FN+1
          } else if (yhat_seuil[id]==0 & test[id,1]==0){
            TN=TN+1
          }
        }
        seuil=seuil+0.05
      ...
      critere = data.frame(cbind(POD,FAR,AUC,precision,Recall,F1.score,brier1,seuil_opti,year_test,mod$bestTune))
  ...
  colnames(critere_full) <- c("POD", "FAR","AUC","Precision","Recall","F1_score","Brier","seuil_opti","year_test","mtry_opti")
  write.table(critere_full,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Graph_critères/Random_forest/Résultats_locaux_HER_97/critere_seuils_opti_test_2012_2016_FULL_moy_j",j,"_new_meteo_all_caract_test4.csv", sep=""), sep=";", row.names = F, col.names = T)
  
  colnames(coefficient) <- c("2012","2013","2014","2015","2016")
  write.table(coefficient,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Graph_critères/Random_forest/Résultats_locaux_HER_97/Coefficient_2012_2016_FULL_moy_j",j,"_new_meteo_all_caract_test4.csv", sep=""), sep=";", row.names = F, col.names = T)
}
