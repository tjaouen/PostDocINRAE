...
# ind<-which(liste_HYDRO[,1]=="Y2580510")

for (ind in c(3,5,49,54)){ # 1:nrow(liste_HYDRO)
  compt=0
  output<-data.frame()
  if (file.exists(paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Predictions/ANN/J30_FRANCE_2012_2016_station_HYDRO/Output_ANN_station_HYDRO_",liste_HYDRO[ind,1],"_HER2_",liste_HYDRO[ind,2],".csv",sep=""))==T){
    ...
    for (l in 1:nrow(donnees)){
      if (file.exists(paste("C:/Users/aurelien.beaufort/Documents/QJHYDRO/FDC_2018/FDC_",liste_HYDRO[ind,1],"_sans_zero.txt",sep=""))==T){
        flow <- read.table(paste("C:/Users/aurelien.beaufort/Documents/QJHYDRO/FDC_2018/FDC_",liste_HYDRO[ind,1],"_sans_zero.txt",sep=""), sep=";", header = T, quote="", stringsAsFactors=F)
  ...
  if (nrow(output)>0){
    ...
    write.table(output,paste("C:/Users/aurelien.beaufort/Documents/Article/Article 2/Figures/Results Pred Station HYDRO/Freq_FAR_ANN/Freq_NonDep_FULL_station_HYDRO_",liste_HYDRO[ind,1],"_HER2_",liste_HYDRO[ind,2],"_sans_zero.csv", sep=""), sep=";", row.names = F, col.names = T)
  }
}