...
nom_data <- paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/Data/Liste_station_HER_FRANCE_2.csv",sep="")
liste_HYDRO <- read.table(file = nom_data, header = TRUE, sep = ";",  row.names = NULL, quote="")
...
for (HERc in HER[14:43]){
  if (file.exists(paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/Data/Input_Matrice/Input_test_classif_HER_",HERc,"_30_jours_fin_caract_new_meteo_FINAL.txt",sep=""))==T){
    ...
    if (ncol(data)>0){
      ...
      x.train <- as.matrix(scaled[index,2:ncol(scaled)])
      
      y.train <- scaled[index,1]
      
      fit10<-cv.glmnet(x.train, y.train, type.measure="mse", alpha=1, family="binomial")
      yhat10 <- predict(fit10, s=fit10$lambda.min, newx=x.train, type="response")
      ...
      for (ind in 1:nrow(liste_HYDRO)){
        if(liste_HYDRO[ind,2]==HERc){
          ...
          if (file.exists(paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/Data/Var_exp_station_NEW_30J_STATION_HYDRO_NEW/2012_2016/Input_Station_HYDRO_",code_HYDRO,"_HER2_",HERc,".txt",sep=""))){
            if (file.exists(paste("C:/Users/aurelien.beaufort/Documents/QJHYDRO/Export_2018/",code_HYDRO,".txt",sep=""))){
              ...
              for (ligne in 1:nrow(x.data_st)){
                ...
                no_flow=which(((as.numeric(flow[DebSelect,4]) <= 1) & (substr(flow[DebSelect,3],5,8)==paste(mois,"25",sep=""))))
                              # | ((as.numeric(flow[DebSelect,4]) <= 1) & substr(flow[DebSelect,3],5,8)==paste(mois,"26",sep="")) 
                              # | ((as.numeric(flow[DebSelect,4]) <= 1) & substr(flow[DebSelect,3],5,8)==paste(mois,"27",sep=""))
                              # | ((as.numeric(flow[DebSelect,4]) <= 1) & substr(flow[DebSelect,3],5,8)==paste(mois,"28",sep=""))
                              # | ((as.numeric(flow[DebSelect,4]) <= 1) & substr(flow[DebSelect,3],5,8)==paste(mois,"24",sep=""))
                              # | ((as.numeric(flow[DebSelect,4]) <= 1) & substr(flow[DebSelect,3],5,8)==paste(mois,"23",sep=""))
                              # | ((as.numeric(flow[DebSelect,4]) <= 1) & substr(flow[DebSelect,3],5,8)==paste(mois,"22",sep="")))
                
                all_flow=which(((as.numeric(flow[DebSelect,4])>=0) & (substr(flow[DebSelect,3],5,8)==paste(mois,"25",sep="")))) 
                               # | ((as.numeric(flow[DebSelect,4])>=0) & substr(flow[DebSelect,3],5,8)==paste(mois,"26",sep="")) 
                               # | ((as.numeric(flow[DebSelect,4])>=0) & substr(flow[DebSelect,3],5,8)==paste(mois,"27",sep=""))
                               # | ((as.numeric(flow[DebSelect,4])>=0) & substr(flow[DebSelect,3],5,8)==paste(mois,"28",sep=""))
                               # | ((as.numeric(flow[DebSelect,4])>=0) & substr(flow[DebSelect,3],5,8)==paste(mois,"24",sep=""))
                               # | ((as.numeric(flow[DebSelect,4])>=0) & substr(flow[DebSelect,3],5,8)==paste(mois,"23",sep=""))
                               # | ((as.numeric(flow[DebSelect,4])>=0) & substr(flow[DebSelect,3],5,8)==paste(mois,"22",sep="")))
              ...
              if (nrow(data_st)>0){
                ...
                yhat10 <- predict(fit10, s=fit10$lambda.1se, newx=scaled.prd, type="response")
                ...
                write.table(data_st,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Predictions/LASSO/J30_FRANCE_2012_2016_station_HYDRO/Output_lasso_station_HYDRO_",code_HYDRO,"_HER2_",HERc,".csv", sep=""), sep=";", row.names = F, col.names = T)
            ...
            } # Test si débit existe
          } # Test si matrice station existe
        } # Test bonne HER
      }  # Boucle station
      colnames(score)=c("Code_Hydro","POD","FAR","Precision","Recall","F_scrore")
      write.table(score,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Predictions/LASSO/Scores_stations_HYDRO/Score_LASSO_station_HYDRO_HER2_",HERc,".csv", sep=""), sep=";", row.names = F, col.names = T)
      T2 <- Sys.time()
      Tdiff = difftime(T2, T1)
      print(Tdiff)
    } # Test données dispo dans matrice
  } 
} # Boucle HER