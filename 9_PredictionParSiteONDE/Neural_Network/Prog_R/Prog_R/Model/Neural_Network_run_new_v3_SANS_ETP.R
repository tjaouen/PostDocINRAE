...
for (HERc in 58) {
  ...
  if (file.exists(paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/Data/Input_Matrice/Input_test_classif_HER_",HERc,"_30_jours_fin_caract_new_meteo_FINAL.txt",sep=""))==T){
    ...
    if (ncol(data)>0){
      if(test_piezo==0){
        data=data[,-c(7:11,23:116)] # on supprime les variables non utilisées
        colnames(data)=c("Assec","PRCP_J","PRCP_J1","PRCP_J10","PRCP_J20","PRCP_J30","TA_J","TA_J1","TA_J10","TA_J20","TA_J30","FreqQ_j","FreqQ_j5","FreqQ_j10","FreqGW_j","FreqGW_j5","FreqGW_j10","P_DRYING_M","Altitude","AI","REC_HIV","Aire","Pente")
      } else if (test_piezo==1){
        data=data[,-c(7:11,20:105)] # on supprime les variables non utilisées
        colnames(data)=c("Assec","PRCP_J","PRCP_J1","PRCP_J10","PRCP_J20","PRCP_J30","TA_J","TA_J1","TA_J10","TA_J20","TA_J30","FreqQ_j","FreqQ_j5","FreqQ_j10","P_DRYING_M","Altitude","AI","REC_HIV","Aire","Pente")
      }
      ...
      for (annee in 1:10){
        ...
        if (test_piezo==0){
          f <- (cbind(Assec,Assec.C2) ~ PRCP_J + PRCP_J1 + PRCP_J10 + PRCP_J20 + PRCP_J30  + TA_J + TA_J1 + TA_J10 + 
      ...
      write.table(critere_full,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Graph_critères/Neural_network/critere_seuils_opti_FULL_moy_j",j,"_new_meteo_all_caract_NEW_decay_0_2_HER_",HERc,"_RANDOM_FINAL_SANS_ETP.csv", sep=""), sep=";", row.names = F, col.names = T)
      write.table(nn_opti$wts,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Graph_critères/Neural_network/Poids_seuils_opti_test_2012_2016_FULL_moy_j",j,"_new_meteo_all_caract_NEW_decay_0_2_HER_",HERc,"_RANDOM_FINAL_SANS_ETP.csv", sep=""), sep=";", row.names = F)
