...
for (HERc in 105) {
  ...
  if (file.exists(paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/Data/Input_Matrice/Input_test_classif_HER_",HERc,"_30_jours_fin_caract_new_meteo_FINAL.txt",sep=""))==T){
    ...
    if (ncol(data)>0){
      ...
      for (annee in 1:10){
        ...
        for (k in 1:nfold){
          ...
          for (nr in 1:nuc){
            ...
            nn <- nn.start(nstart,train.cv[-ridk,],huc[nr],f,lambda=0.0001) # decay LASSO -> 0.0001304285
      ...
      write.table(critere_full,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Graph_critères/Neural_network/critere_seuils_opti_FULL_moy_j",j,"_new_meteo_all_caract_NEW_decay_0_0001_HER_",HERc,"_RANDOM_FINAL_UC4.csv", sep=""), sep=";", row.names = F, col.names = T)
      write.table(nn_opti$wts,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Graph_critères/Neural_network/Poids_seuils_opti_test_2012_2016_FULL_moy_j",j,"_new_meteo_all_caract_NEW_decay_0_0001_HER_",HERc,"_RANDOM_FINAL_UC4.csv", sep=""), sep=";", row.names = F)
