    ...
    if (ncol(data)>0){
      ...
      for (annee in 1:10){
        ...
        nn.start <- function(nstart,d.train,nnode,f.nn,maxit=1000,lambda=0){
          nnet(f,data=train.cv,size=huc[nr], softmax=T, maxit=1000, skip=T,trace=F,decay=0)
          min.val=Inf
          nn.min=NULL
          for (a in 1:nstart){
        ...
        for (k in 1:nfold){
          ...
          for (nr in 1:nuc){
            ...
            nn <- nn.start(nstart,train.cv,huc[nr],f,lambda=0.0001) # decay LASSO -> 0.0001304285
            
            # on teste le modèle sur le pli sélectionné
            pr.nn2 <- predict(nn, train.cv[,c(-1,-ncol(train.cv))], type="raw")
            
            # Evaluation of cost fcts for each model neg-log-like
            # Calcul du % de déviance
            nlls.nr <- -sum(train.cv$Assec*log(pr.nn2[,1]+10^(-9))+train.cv$Assec.C2*log(pr.nn2[,2]+10^(-9)))
            # Matrice resultats
        ...
        nstart2=10
        nn_opti <- nn.start(nstart2,train.cv,huc[rep],f,lambda=0.0001)
      ...
      write.table(critere_full,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Graph_critères/Neural_network/critere_seuils_opti_FULL_moy_j",j,"_new_meteo_all_caract_NEW_decay_0_0001_HER_",HERc,"_RANDOM_FINAL.csv", sep=""), sep=";", row.names = F, col.names = T)
      write.table(nn_opti$wts,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/OUTPUT/Graph_critères/Neural_network/Poids_seuils_opti_test_2012_2016_FULL_moy_j",j,"_new_meteo_all_caract_NEW_decay_0_0001_HER_",HERc,"_RANDOM_FINAL.csv", sep=""), sep=";", row.names = F)
