...
for (HERc in HER_[1]){
  ...
  for (l in list_files_){
    ...
    chroProba_dates_ <- chroProba_[which(chroProba_$Type == "Safran"),which(!(colnames(chroProba_) %in% c("Jour_annee","Type")))]
    input_ <- merge(input_, chroProba_dates_, by = "Date", all.x = TRUE)
  }
  
  # chroProba_rcp26_ <- read.table(paste0("/media/tjaouen/Ultra Touch/Backup/Main/Input/HYDRO/EtudeFrance/Tab_ChroniquesProbaParHER2_LearnBrut_ByHer/FDC_Projections/CTRIP_20231128/ChroniquesCombinees_saf_hist_rcp26/Tab_ChroniquesProba_LearnBrut_HER",HERc,".txt"),
  #                          sep=";",dec=".",header=T)
  # chroProba_rcp26_dates_ <- chroProba_rcp26_[which(chroProba_rcp26_$Date %in% input_$Date & chroProba_rcp26_$Type == "Safran"),]
  # 
  # chroProba_rcp45_ <- read.table(paste0("/media/tjaouen/Ultra Touch/Backup/Main/Input/HYDRO/EtudeFrance/Tab_ChroniquesProbaParHER2_LearnBrut_ByHer/FDC_Projections/CTRIP_20231128/ChroniquesCombinees_saf_hist_rcp45/Tab_ChroniquesProba_LearnBrut_HER",HERc,".txt"),
  #                                sep=";",dec=".",header=T)
  # chroProba_rcp45_dates_ <- chroProba_rcp26_[which(chroProba_rcp45_$Date %in% input_$Date & chroProba_rcp45_$Type == "Safran"),]
  # 
  # chroProba_rcp26_ <- read.table(paste0("/media/tjaouen/Ultra Touch/Backup/Main/Input/HYDRO/EtudeFrance/Tab_ChroniquesProbaParHER2_LearnBrut_ByHer/FDC_Projections/CTRIP_20231128/ChroniquesCombinees_saf_hist_rcp26/Tab_ChroniquesProba_LearnBrut_HER",HERc,".txt"),
  #                                sep=";",dec=".",header=T)
  # chroProba_rcp26_dates_ <- chroProba_rcp26_[which(chroProba_rcp26_$Date %in% input_$Date & chroProba_rcp26_$Type == "Safran"),]
  
  # input_merge_ <- merge(input_, chroProba_dates_, by = "Date", all.x = TRUE)
