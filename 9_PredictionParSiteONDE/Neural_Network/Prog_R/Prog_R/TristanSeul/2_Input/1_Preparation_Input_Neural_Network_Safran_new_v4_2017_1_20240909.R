...
PRCP_5 <- read.table(paste0("/home/tjaouen/Documents/Input/Climat/SAFRAN_2010_2022/PRCP_2015_2016.txt",sep=""),sep=";", dec = ".", header=T, quote = "")
PRCP_6 <- read.table(paste0("/home/tjaouen/Documents/Input/Climat/SAFRAN_2010_2022/PRCP_2016_2017.txt",sep=""),sep=";", dec = ".", header=T, quote = "")
colnames(PRCP_5) <- as.Date(gsub("X","",colnames(PRCP_5)), format = "%Y.%m.%d")
colnames(PRCP_6) <- as.Date(gsub("X","",colnames(PRCP_6)), format = "%Y.%m.%d")
...
for (HERc in HER2[1]){ #
  ...
  if (ncol(output)>0){
    ...
    list=unique(output$V1)
    
    for (i in 1:length(list)){
      select=which(as.character(output$V1)==as.character(list[i]))
      
      for (id in 1:length(select)){
        # Calcul du Zeroqual par Mois
        no_flow=which((as.character(output$V4[select])=="Ecoulement non visible" | as.character(output$V4[select])=="Assec") & (format(as.Date(output[select,3],"%d/%m/%Y"),"%m")==format(as.Date(output[select[id],3],"%d/%m/%Y"),"%m")))
        flow=which((as.character(output$V4[select])=="Ecoulement visible") & (format(as.Date(output[select,3],"%d/%m/%Y"),"%m")==format(as.Date(output[select[id],3],"%d/%m/%Y"),"%m")))
    ...
    for (a in 1:nrow(output)){
      ...
      if (length(repere) > 0){
        ...
        if (ref_nowak[repere,23] > 0 & !is.null(ref_nowak[repere,23]) & !is.na(ref_nowak[repere,23]) & ref_nowak[repere,23]!= 999){
          output[a,129]<-ref_nowak[repere,23] # Distance ? la source
        } else {
          output[a,129]<-NA
        }
      }
