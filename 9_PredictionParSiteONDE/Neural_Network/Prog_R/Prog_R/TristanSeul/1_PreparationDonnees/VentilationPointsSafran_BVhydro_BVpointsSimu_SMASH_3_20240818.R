...
liste_ptsSimExp2_SMASH_$id <- 1:nrow(liste_ptsSimExp2_SMASH_)
...
merge_SMASH_ <- merge_SMASH_[order(merge_SMASH_$id), ]
merge_SMASH_$Code10_BV <- merge_SMASH_$Code10_ChoixDefinitifPointSimu
length(which(is.na(merge_SMASH_$ControlMerge))) # 35

# Passer a 0 si le BV ne contient pas de point Safran Climat
length(which(sapply(merge_SMASH_$IndPointsSafranIntersect_, length) == 0))
merge_SMASH_$ControlMerge[which(sapply(merge_SMASH_$IndPointsSafranIntersect_, length) == 0)] = NA
length(which(is.na(merge_SMASH_$ControlMerge))) # 72
