...
for (HERc in HER_[1]){
  ...
  # list_filesInput_ <- list.files(path = paste0(folder_output_,"/23_PredictionParSiteONDE_PremierTest/Input/Data/InputTestClassif/AjoutProbaAssecParHER_1_20240927/"),
  ...
  for (simu in grep("debit_",colnames(data_ini))[1]){
    ...
    for (i in 1:nrow(data_ini)){
      ...
      if(test_piezo==TRUE){
        ...
        data[compteur,24] = data_ini[i,simu]
        ...
        data[compteur,26] = data_ini$AI_JanvJuil[i]
        data[compteur,27] = data_ini$REC_HIV[i]
        data[compteur,28] = data_ini$Aire_BV[i]
        data[compteur,29] = data_ini$Pente[i]
        data[compteur,30] = data_ini$Code_Onde[i]
        data[compteur,31] = data_ini$Date[i]
      }else{
    ...
    if (ncol(data)>0){
      ...
      if (!dir.exists(paste0(folder_output_,"/23_PredictionParSiteONDE_PremierTest/Random_Forest/4_ResultatsLocaux_France/",colnames(data_ini)[simu],"/"))){
        dir.create(paste0(folder_output_,"/23_PredictionParSiteONDE_PremierTest/Random_Forest/4_ResultatsLocaux_France/",colnames(data_ini)[simu],"/"))
      }
      if (!dir.exists(paste0(folder_output_,"/23_PredictionParSiteONDE_PremierTest/Random_Forest/4_ResultatsLocaux_France/",colnames(data_ini)[simu],"/CrossValidationSafran/"))){
        dir.create(paste0(folder_output_,"/23_PredictionParSiteONDE_PremierTest/Random_Forest/4_ResultatsLocaux_France/",colnames(data_ini)[simu],"/CrossValidationSafran/"))
      }
      ...
      write.table(frame_test,paste0(folder_output_,"/23_PredictionParSiteONDE_PremierTest/Random_Forest/4_ResultatsLocaux_France/",colnames(data_ini)[simu],"/CrossValidationSafran/CritereSeuilsOptiTest_2012_2022_Calibration_moyJ",j,"_HER",HERc,".csv"), sep=";", row.names = F, col.names = T)
      write.table(coefficient,paste0(folder_output_,"/23_PredictionParSiteONDE_PremierTest/Random_Forest/4_ResultatsLocaux_France/",colnames(data_ini)[simu],"/CrossValidationSafran/Coefficient_2012_2022_Calibration_moyJ",j,"_HER_",HERc,".csv"), sep=";", row.names = F, col.names = T)
