  ...
  for (simu in grep("debit_",colnames(data_ini))){
    ...
    while (seuil <= 1){
      ...
      if ((TP_train+FP_train) > 0 & (TP_train+FN_train) > 0 & (TN_train+FP_train) > 0){
        ...
        frame_train[ligne,10]<-round((TP_train+TN_train)/(TP_train+FN_train+TN_train+FP_train),4) # Accuracy
        frame_train[ligne,11]<-round(my_roc$auc,4)
        frame_train[ligne,12]<-round(TP_train/(TP_train+FN_train),4) # Sensi
        frame_train[ligne,13]<-round(TN_train/(TN_train+FP_train),4) # Speci
        frame_train[ligne,14]<-round(FP_train/(FP_train+TP_train),4) # FAR
        ...
        # moy_obs = mean(y*100)
        moy_obs = mean(y)
        frame_train[ligne,18]<-round(1 - sum((y_pred-(y))**2) / sum((moy_obs-(y))**2),4) # NASH = 1 - SCE_residuelle / SCE_Aexpliquer = SCE_expliquee par modele
        ...
        frame_train[ligne,20]<-round(sd(y_pred)/mean(y_pred),4) #CV => Indicateur litigieux car mean(y) n'est pas pondere au nombre de donnees ONDE utilisees pour obtenir y
        
        frame_train[ligne,21] <- round(sqrt(1/length(y) * sum((y-y_pred)^2)),4) # RMSE
        frame_train[ligne,22] <- round(sum(y_pred-y)/length(y),4) # Biais
        frame_train[ligne,23] <- round(sum(abs(y_pred-y))/length(y),4) # EMA
