...
for (HERc in HER_[1]){
  ...
  list_filesInput_ <- list.files(path = "/home/tjaouen/Documents/Output/ChangementClimatique2019/EtudeFrance/23_PredictionParSiteONDE_PremierTest/Input/Data/InputTestClassif/",
                                 pattern = paste0("_30_jours_fin_caract_new_meteo_FINAL"), full.names = T)
  ...
  for (i in 1:nrow(data_ini)){
    ...
    if(test_piezo==TRUE){
      ...
      data[compteur,24] = data_ini$Altitude[i]
      data[compteur,25] = data_ini$AI_JanvJuil[i]
      data[compteur,26] = data_ini$REC_HIV[i]
      data[compteur,27] = data_ini$Aire_BV[i]
      data[compteur,28] = data_ini$Pente[i]
      data[compteur,29] = data_ini$Code_Onde[i]
      data[compteur,30] = data_ini$Date[i]
    }else{
  ...
  if (ncol(data)>0){
    if(test_piezo==TRUE){
      ...
      colnames(data)=c("Assec",
                       ...
                       "P_DRYING_M","Altitude","AI","REC_HIV","Aire","Pente","Code_Onde","Date")
    ...
    write.table(critere_full,paste("/home/tjaouen/Documents/Output/ChangementClimatique2019/EtudeFrance/23_PredictionParSiteONDE_PremierTest/Random_Forest/ResultatsLocaux_France/CritereSeuilsOptiTest_2012_2016_FULL_moy_j",j,"_new_meteo_all_caract_HER_",HERc,"_RandomFinal.csv", sep=""), sep=";", row.names = F, col.names = T)
