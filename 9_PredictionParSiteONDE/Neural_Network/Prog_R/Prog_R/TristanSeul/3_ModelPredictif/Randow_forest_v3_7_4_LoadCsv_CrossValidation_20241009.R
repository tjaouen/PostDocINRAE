...
# "2", "3", 
HER_ <- c("58","38","85","105")
# HER_ <- c("5", "12", "13", "14", "17", "21", "22", "24", "25", "27", "28", "34", "35", "36",
...
output <- foreach(HERc = HER_) %dopar% { #, errorhandling='pass'
  ...
  for (simu in grep("debit_",colnames(data_ini))){
    ...
    if (length(years_)>0){
      for (year_test in 1:length(years_)){
        ...
        print(paste0('year_test: ',year_test))
        ...
        print(paste0('year_test2: ',year_test))
        ...
        print(paste0('year_test3: ',year_test))
  ...
  # }
  # ,error=function(e){
  #   erreurs_HERc <<- c(erreurs_HERc, HERc)  # Ajouter HERc à la liste des erreurs
  #   cat("Erreur avec HERc = ",HERc,"\n")
  #   return(NULL)
  # })
} # boucle HER
