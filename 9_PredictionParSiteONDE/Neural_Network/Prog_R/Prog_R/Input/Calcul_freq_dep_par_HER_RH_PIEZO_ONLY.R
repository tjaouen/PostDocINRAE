...
liste
test=0
NB_Station_PIEZO<-data.frame()
compteur=0
list_HER=sort(HER2[,3])

for (HER in list_HER[12:85]) {
...
# for (HER in 13) {
  ...
  if (length(select_PIEZO) > 0){
    for (st_piezo in 1:length(select_PIEZO)){
      if ((as.character(Piezo[select_PIEZO[st_piezo],12]) != "Cycle pluriannuel unique (inertie forte)") & (as.character(Piezo[select_PIEZO[st_piezo],12]) != "Double cycle (inertie forte)")){
        ...
        if (file.exists(paste("C:/Users/aurelien.beaufort/Documents/Bd ades/FDC_1970_2017/FDC_",code_ades,".txt",sep=""))==TRUE){
          col_deb=col_deb+1
          debExtract<-read.table(paste("C:/Users/aurelien.beaufort/Documents/Bd ades/FDC_1970_2017/FDC_",code_ades,".txt",sep=""), header = T, sep = ";", row.names = NULL, quote="")
  ...
  for (RH in liste_RH){
    ...
    if (nrow(debExtract_full)>0){
      ...
      write.table(output,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/Data/Freq_2017_HER_HR/Freq_2017_",HER,"_",RH,"_PIEZO_ONLY.txt",sep=""),sep=";", row.name=F,quote=F)
      T2 <- Sys.time()
    ...
    compteur=compteur+1
    NB_Station_PIEZO[compteur,1]=HER
    NB_Station_PIEZO[compteur,2]=RH
    NB_Station_PIEZO[compteur,3]=(ncol(debExtract_full)/2)
  } # Boucle RH
} # HER2


colnames(NB_Station_PIEZO)<-c("HER","RH","NB_ST_PIEZO")
write.table(NB_Station_PIEZO,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/Data/Freq_2017_HER_HR/NB_Station_PIEZO_HER.csv",sep=""),sep=";", row.name=F,quote=F)
