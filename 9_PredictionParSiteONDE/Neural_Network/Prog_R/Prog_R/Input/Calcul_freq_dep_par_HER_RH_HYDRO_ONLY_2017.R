...
source("/home/tjaouen/Documents/Src/ChangementClimatique_Bottet2019/CodesTristan/9_PredictionParSiteONDE/Neural_Network/Prog_R/BFI.r")
source("/home/tjaouen/Documents/Src/ChangementClimatique_Bottet2019/CodesTristan/9_PredictionParSiteONDE/Neural_Network/Prog_R/HydrologicalYear.r")
...
# for (HER in 13) {
  ...
  for (RH in liste_RH){
    ...
    n_obs=0
    # obs_tot=153*5
    obs_tot=as.Date(paste("31/12/2017",sep = ""), "%d/%m/%Y")-as.Date(paste("01/01/2017",sep = ""), "%d/%m/%Y")
    # col_deb=(ncol_piezo/2)
    col_deb=0
    
    while(n_obs < obs_tot){
      date_onde=format(as.Date(paste("01/01/2017",sep = ""), "%d/%m/%Y")+n_obs,"%d/%m/%Y")
      ...
      if (compt==1){
        ...
        if (RH < 9){
          ...
          # on s?lectionne les stations ONDE et HYDRO ayant un m?me r?gime hydrologique dans l'HER
          select_HYDRO2<-which(hydro[select_HYDRO1,26]==RH)
        }

        if (length(select_HYDRO2) > 0){
          # On rassemble les freq au non d?passement dans une matrice
      ...
      #-----------------------------------------------------------------------------------------------------
      # Calcul des fr?quences au non d?passement des d?bits et des cotes pi?zom?triques pour le suivi usuel
      #-----------------------------------------------------------------------------------------------------
      if (nrow(debExtract_full)>0){
        # On va chercher les d?bits aux dates d'observations usuelles ONDE
        output_HYDRO=data.frame()
        compt2=0
        
        # ------------------------------------------------------
        # On calcul les fr?quences au non d?passement des d?bits
        # ------------------------------------------------------
        for (station in 1:(ncol(debExtract_full)/2)){
          ...
          if (length(flow_cible) > 0){
            if (as.numeric(flow_cible) > 10){
              compt2=compt2+1
              output_HYDRO[compt2,1]=debExtract_full[flow_cible[1],(station*2)] # Freq non d?passement au jour j
              output_HYDRO[compt2,2]=debExtract_full[flow_cible[1]-1,(station*2)] # Freq non d?passement au jour j-1
              output_HYDRO[compt2,3]=debExtract_full[flow_cible[1]-2,(station*2)] # Freq non d?passement au jour j-2
              output_HYDRO[compt2,4]=debExtract_full[flow_cible[1]-3,(station*2)] # Freq non d?passement au jour j-3
              output_HYDRO[compt2,5]=debExtract_full[flow_cible[1]-4,(station*2)] # Freq non d?passement au jour j-4
              output_HYDRO[compt2,6]=debExtract_full[flow_cible[1]-5,(station*2)] # Freq non d?passement au jour j-5
              output_HYDRO[compt2,7]=debExtract_full[flow_cible[1]-6,(station*2)] # Freq non d?passement au jour j-1
              output_HYDRO[compt2,8]=debExtract_full[flow_cible[1]-7,(station*2)] # Freq non d?passement au jour j-2
              output_HYDRO[compt2,9]=debExtract_full[flow_cible[1]-8,(station*2)] # Freq non d?passement au jour j-3
              output_HYDRO[compt2,10]=debExtract_full[flow_cible[1]-9,(station*2)] # Freq non d?passement au jour j-4
              output_HYDRO[compt2,11]=debExtract_full[flow_cible[1]-10,(station*2)] # Freq non d?passement au jour j-5
            }
          }
        }
        ...
        if (nrow(output_HYDRO)>0){
          for (h in 1:11){
            output[compt,(1+h)] = mean(output_HYDRO[,h], na.rm = T) # Moyenne des d?bits sp?cifiques au jour j de toutes les stations HYDRO d'une r?gime hydro donn?e
          }
        } else {
          for (h in 1:11){
            output[compt,(1+h)] = NA
    ...
    if (nrow(debExtract_full)>0){
      ...
      # write.table(output,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/Data/Freq_2017_HER_HR/Freq_2012_2016_",HER,"_",RH,"_HYDRO_ONLY_test.txt",sep=""),sep=";", row.name=F,quote=F)
      T2 <- Sys.time()
...
write.table(NB_Station_HYDRO,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/Data/Freq_2017_HER_HR/NB_Station_HYDRO_HER.csv",sep=""),sep=";", row.name=F,quote=F)
