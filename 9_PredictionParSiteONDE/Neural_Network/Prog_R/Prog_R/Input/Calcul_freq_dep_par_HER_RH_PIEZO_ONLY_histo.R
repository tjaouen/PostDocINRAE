...
# for (HER in 13) {
  ...
  for (RH in liste_RH){
    ...
    n_obs=0
    # obs_tot=153*5
    obs_tot=as.Date(paste("31/12/2016",sep = ""), "%d/%m/%Y")-as.Date(paste("01/01/1958",sep = ""), "%d/%m/%Y")
    col_deb=(ncol_piezo/2)
    
    
    while(n_obs < obs_tot){
      date_onde=format(as.Date(paste("01/01/1958",sep = ""), "%d/%m/%Y")+n_obs,"%d/%m/%Y")
    ...
    # if (nrow(debExtract_full)>0){
      ...
      write.table(output,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/Data/Freq_1958_2016_HER_HR/Freq_1958_2016_",HER,"_",RH,"_PIEZO_ONLY.txt",sep=""),sep=";", row.name=F,quote=F)
      T2 <- Sys.time()
...
# write.table(NB_Station_PIEZO,paste("C:/Users/aurelien.beaufort/Documents/Neural_Network/Data/Freq_2012_2016_HER_HR/NB_Station_PIEZO_HER.csv",sep=""),sep=";", row.name=F,quote=F)
