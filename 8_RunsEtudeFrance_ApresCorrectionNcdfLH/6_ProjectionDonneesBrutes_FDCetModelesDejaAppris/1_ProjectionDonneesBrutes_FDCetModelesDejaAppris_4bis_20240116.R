...
source("/home/tjaouen/Documents/Src/ChangementClimatique_Bottet2019/CodesTristan/8_RunsEtudeFrance_ApresCorrectionNcdfLH/1_Parameters/0_SimulationParameters_AvecCC_2_20230227_ProjectionSurFDCApprise_2.R")
...
folder_a_projeter_ = folder_a_projeter_[c(17,19,20,21,22,26,31,32)]
# folder_a_projeter_ = folder_a_projeter_[c(1:4,6,8:9,11:12,15:17)]
# folder_a_projeter_ = folder_a_projeter_[1:16]
# folder_a_projeter_ = folder_a_projeter_[17:34]

# for (ind_folder_ in 1:3){
...
output <- foreach(folder_ = folder_a_projeter_) %dopar% { #, errorhandling='pass'
  ...
  # for (ind_file_ in 1:10){
  # for (ind_file_ in 1:799){
  for (ind_file_ in 639:length(files_a_projeter_)){
  # for (ind_file_ in 1:380){
    tab_debit_aProjeter_ = read.table(files_a_projeter_[ind_file_], sep = ";", dec = ".", header = T)
