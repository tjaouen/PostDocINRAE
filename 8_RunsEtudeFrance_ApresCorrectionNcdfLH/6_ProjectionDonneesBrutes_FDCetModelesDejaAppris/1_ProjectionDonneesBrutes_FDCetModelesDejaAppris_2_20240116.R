...
# for (ind_folder_ in 2:2){
for (ind_folder_ in 1:length(folder_a_projeter_)){
  files_a_projeter_ = list.files(paste0(folder_a_projeter_[ind_folder_], "/"), full.names = T)
  
  for (ind_file_ in 320:length(files_a_projeter_)){
    tab_debit_aProjeter_ = read.table(files_a_projeter_[ind_file_], sep = ";", dec = ".", header = T)
    ...
    if (!dir.exists(paste0("/media/tjaouen/Lexar/INRAE_Bckp/20231123/Input/HYDRO/EtudeFrance/FlowDurationCurves/FDC_Projections/",
                           ...
                           "Modele_",
                           substr(str_after_first(nom_categorieSimu_,"/"),1,nchar(str_after_first(nom_categorieSimu_,"/"))-1),
                           "_",
                           str_before_first(str_after_first(folder_a_projeter_[ind_folder_],"debit_France_"),"_historical"),
                           "_",
                           substr(str_before_first(str_after_first(folder_a_projeter_[ind_folder_],"p1_"),"SAFRAN"), 1, nchar(str_before_first(str_after_first(folder_a_projeter_[ind_folder_],"p1_"),"SAFRAN")) - 1),
                           "/"))){
    ...
    if (!dir.exists(paste0("/media/tjaouen/Lexar/INRAE_Bckp/20231123/Input/HYDRO/EtudeFrance/FlowDurationCurves/FDC_Projections/",
                           ...
                           substr(str_before_first(str_after_first(folder_a_projeter_[ind_folder_],"p1_"),"SAFRAN"), 1, nchar(str_before_first(str_after_first(folder_a_projeter_[ind_folder_],"p1_"),"SAFRAN")) - 1),
                           "/",
                           str_after_last(folder_a_projeter_[ind_folder_],"/")))){
      dir.create(paste0("/media/tjaouen/Lexar/INRAE_Bckp/20231123/Input/HYDRO/EtudeFrance/FlowDurationCurves/FDC_Projections/",
                        str_before_first(nom_categorieSimu_,"/"),
                        "/ChroniquesBrutes_historical/",
                        "Modele_",
    ...
    write.table(tab_debit_aProjeter_,
                ...
                "/ChroniquesBrutes_historical/",
                "Modele_",
                substr(str_after_first(nom_categorieSimu_,"/"),1,nchar(str_after_first(nom_categorieSimu_,"/"))-1),"_",
                str_before_first(str_after_first(folder_a_projeter_[ind_folder_],"debit_France_"),"_historical"),"_",
                substr(str_before_first(str_after_first(folder_a_projeter_[ind_folder_],"p1_"),"SAFRAN"), 1, nchar(str_before_first(str_after_first(folder_a_projeter_[ind_folder_],"p1_"),"SAFRAN")) - 1),"/",
                str_after_last(folder_a_projeter_[ind_folder_],"/"),"/",
