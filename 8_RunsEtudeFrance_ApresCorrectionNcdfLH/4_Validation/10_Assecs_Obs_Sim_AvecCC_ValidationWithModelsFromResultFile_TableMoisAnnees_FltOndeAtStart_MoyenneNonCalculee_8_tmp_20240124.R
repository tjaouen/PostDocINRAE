...
for (an in annees_learnModels_p_){
  ...
  onde_an_ <- onde[which(format(as.Date(onde$Date),"%Y") %in% an),]
  ...
  Assecs_val = FUNC_CAL_VAL_ValidationSansLearn_SansIC_ParDate(1,onde_an_,liste_Her,regr,tab_Reg_,jourMin_ = jourMin_, jourMax_ = jourMax_)
