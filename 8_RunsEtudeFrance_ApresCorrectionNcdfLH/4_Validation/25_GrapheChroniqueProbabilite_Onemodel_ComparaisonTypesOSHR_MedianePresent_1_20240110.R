...
# date_max_past = "2019-12-31"
date_max_past = "2022-12-31"
date_min_fut = "2012-01-01"
# date_max_fut = "2019-12-31"
...
tab_rcp_20702100 = tab_[which(tab_$Type == "rcp85" & tab_$Date >= min(tab_saf_19752005$Date) & tab_$Date <= max(tab_saf_19752005$Date)),]
...
for (HER_h_ in HER_){
  ...
  if (HER_h_ %in% colnames(tab_)){
    ...
    chro_obs_$Date <- as.Date(chro_obs_$Date)
    chro_obs_$Jour_annee <- format(chro_obs_$Date, format = "%m-%d")
    proba_obs_ <- chro_obs_ %>%
      group_by(Jour_annee) %>%
      summarize(Proba_moyen = mean(Proba, na.rm = TRUE),
                Proba_median = median(Proba, na.rm = TRUE),
                Proba_Q5 = quantile(Proba, probs = 0.05, na.rm = TRUE),
    ...
    proba_obs_$Periode <- paste0("Obs_20122019")
    proba_saf_19752005$Periode <- paste0("Safran_20122019")
    # proba_hist_19752005$Periode <- paste0("Hist_19702005")
    proba_rcp_20702100$Periode <- paste0("Rcp_20122019")
    ...
    combined_data <- rbind(proba_obs_,
                           proba_saf_19752005,
                           # proba_hist_19752005,
                           proba_rcp_20702100)
    ...
    custom_colors <- c("Obs_20122019" = "#2ca25f",
                       "Safran_20122019" = "#2b8cbe",
                       # "Hist_19702005" = "#fc9272",
                       "Rcp_20122019" = "#cb181d")
    desired_order <- c("Obs_20122019",
                       "Safran_20122019",
                       # "Hist_19702005",
                       "Rcp_20122019")
    labels_ <- c(paste0("Observed ",year(min(chro_obs_$Date)),"/",year(max(chro_obs_$Date))),
                 paste0("Safran ",year(min(chro_saf_19752005$Date)),"/",year(max(chro_saf_19752005$Date))),
                 # paste0("Safran ",year(min(chro_saf_Safran20122019$Date)),"/",year(max(chro_saf_Safran20122019$Date))),
                 paste0("RCP 8.5 ",year(min(chro_rcp_20702100$Date)),"/",year(max(chro_rcp_20702100$Date))))
    # desired_order <- c(paste0(dat_modelsRL_$model[ind_],"__Data_Safran_19702005"),
    #                    paste0(dat_modelsRL_$model[ind_],"__Data_Hist_19702005"),
    #                    paste0(dat_modelsRL_$model[ind_],"__Data_Rcp_20702100"))
    ...
    p <- ggplot() +
      ...
      ggtitle(dat_modelsRL_$model[ind_]) +
      ...
      # ylim(0,50) +
      ylim(0,max(max(na.omit(combined_data$Proba_median)),max(tab_input_h_min_max_$Max_X_Assec))) +
      theme_minimal() +
      ...
      labs(x = "Date",
           ...
           color = "Data",
           title = paste0("Médiane de la probabilité d'assec prédite lissée",
                          ...
                          "\nModel: ",nom_GCM_,
                          "\nHER: ",HER_h_,
                          "\nAttention, les probabilités dites observées n'ont pas été obtenues par cette RL mais par l'ancienne RL apprise sur l'observé."))
    p

    
    if (!dir.exists(paste0(folder_output_,
                           "18_GrapheChroniqueProbabilite_Onemodel_ComparaisonTypesOSHR_MedianePresent/",
                           nomSim_,
