...
FUNC_CAL_VAL_ValidationSansLearn_Globale <- function(annee,Matrice_comp,liste_Her,nbrRep,proportionTest){
  ...
  for (id in 1:length(liste_Her)) {
    ...
    for (i in 1:nbrRep){
      ...
      RMSE_tmp[i] <- sqrt(1/length(y) * sum((y*100-y_pred*100)^2))
      # Biais_tmp[i] <- sum(y_pred-y)/length(y)
      Biais_tmp[i] <- sum(y_pred*100-y*100)/length(y)
      ErreurMoyenneAbsolue_tmp[i] <- sum(abs(y_pred*100-y*100))/length(y)
      ...
      RMSE_3maxApred_tmp[i] <- sqrt(1/length(y) * sum((y*100-y_pred*100)^2))
      # Biais_3maxApred_tmp[i] <- sum(y_pred-y)/length(y)
      Biais_3maxApred_tmp[i] <- sum(y_pred*100-y*100)/length(y)
      ErreurMoyenneAbsolue_3maxApred_tmp[i] <- sum(abs(y_pred*100-y*100))/length(y)
      ...
      RMSE_mai_tmp[i] <- sqrt(1/length(y) * sum((y*100-y_pred*100)^2))
      # Biais_mai_tmp[i] <- sum(y_pred-y)/length(y)
      Biais_mai_tmp[i] <- sum(y_pred*100-y*100)/length(y)
      ErreurMoyenneAbsolue_mai_tmp[i] <- sum(abs(y_pred*100-y*100))/length(y)
      ...
      RMSE_juin_tmp[i] <- sqrt(1/length(y) * sum((y*100-y_pred*100)^2))
      # Biais_juin_tmp[i] <- sum(y_pred-y)/length(y)
      Biais_juin_tmp[i] <- sum(y_pred*100-y*100)/length(y)
      ErreurMoyenneAbsolue_juin_tmp[i] <- sum(abs(y_pred*100-y*100))/length(y)
      ...
      RMSE_juillet_tmp[i] <- sqrt(1/length(y) * sum((y*100-y_pred*100)^2))
      # Biais_juillet_tmp[i] <- sum(y_pred-y)/length(y)
      Biais_juillet_tmp[i] <- sum(y_pred*100-y*100)/length(y)
      ErreurMoyenneAbsolue_juillet_tmp[i] <- sum(abs(y_pred*100-y*100))/length(y)
      ...
      RMSE_aout_tmp[i] <- sqrt(1/length(y) * sum((y*100-y_pred*100)^2))
      # Biais_aout_tmp[i] <- sum(y_pred-y)/length(y)
      Biais_aout_tmp[i] <- sum(y_pred*100-y*100)/length(y)
      ErreurMoyenneAbsolue_aout_tmp[i] <- sum(abs(y_pred*100-y*100))/length(y)
      ...
      RMSE_septembre_tmp[i] <- sqrt(1/length(y) * sum((y*100-y_pred*100)^2))
      # Biais_septembre_tmp[i] <- sum(y_pred-y)/length(y)
      Biais_septembre_tmp[i] <- sum(y_pred*100-y*100)/length(y)
      ErreurMoyenneAbsolue_septembre_tmp[i] <- sum(abs(y_pred*100-y*100))/length(y)
    ...
    RMSE_general[id] <- round(sqrt(1/length(y) * sum((y*100-y_pred*100)^2)),3)
    # Biais_general[id] <- round(sum(y_pred-y)/length(y),5) #,3)
    Biais_general[id] <- round(sum(y_pred*100-y*100)/length(y),3)
    ErreurMoyenneAbsolue_general[id] <- round(sum(abs(y_pred*100-y*100))/length(y),3)
    
    ### Sur les 5 valeurs les plus hautes de la Proba d'assec a predire ###
    tab_test_3val = tab_test[which(tab_test$ProbaAssec_HERMoisAnnee_Apredire_CValid %in% head(unique(sort(tab_test$ProbaAssec_HERMoisAnnee_Apredire_CValid, decreasing = T)),3)),]
    y <- tab_test_3val$ProbaAssec_HERMoisAnnee_Apredire_CValid
    y_pred <- tab_test_3val$ProbaAssec_HERMoisAnnee_Predite_CValid
    RMSE_3maxApred_general[id] <- round(sqrt(1/length(y) * sum((y*100-y_pred*100)^2)),3)
    # Biais_3maxApred_general[id] <- round(sum(y_pred-y)/length(y),5) #,3)
    Biais_3maxApred_general[id] <- round(sum(y_pred*100-y*100)/length(y),3)
    ErreurMoyenneAbsolue_3maxApred_general[id] <- round(sum(abs(y_pred*100-y*100))/length(y),3)
    
    ### Sur les valeurs de mai ###
    tab_test_mai = tab_test[which(months(as.Date(tab_test$Date)) == "mai"),]
    y <- tab_test_mai$ProbaAssec_HERMoisAnnee_Apredire_CValid
    y_pred <- tab_test_mai$ProbaAssec_HERMoisAnnee_Predite_CValid
    RMSE_mai_general[id] <- round(sqrt(1/length(y) * sum((y*100-y_pred*100)^2)),3)
    # Biais_mai_general[id] <- round(sum(y_pred-y)/length(y),5) #,3)
    Biais_mai_general[id] <- round(sum(y_pred*100-y*100)/length(y),3)
    ErreurMoyenneAbsolue_mai_general[id] <- round(sum(abs(y_pred*100-y*100))/length(y),3)
    
    ### Sur les valeurs de juin ###
    tab_test_juin = tab_test[which(months(as.Date(tab_test$Date)) == "juin"),]
    y <- tab_test_juin$ProbaAssec_HERMoisAnnee_Apredire_CValid
    y_pred <- tab_test_juin$ProbaAssec_HERMoisAnnee_Predite_CValid
    RMSE_juin_general[id] <- round(sqrt(1/length(y) * sum((y*100-y_pred*100)^2)),3)
    # Biais_juin_general[id] <- round(sum(y_pred-y)/length(y),5) #,3)
    Biais_juin_general[id] <- round(sum(y_pred*100-y*100)/length(y),3)
    ErreurMoyenneAbsolue_juin_general[id] <- round(sum(abs(y_pred*100-y*100))/length(y),3)
    
    ### Sur les valeurs de juillet ###
    tab_test_juillet = tab_test[which(months(as.Date(tab_test$Date)) == "juillet"),]
    y <- tab_test_juillet$ProbaAssec_HERMoisAnnee_Apredire_CValid
    y_pred <- tab_test_juillet$ProbaAssec_HERMoisAnnee_Predite_CValid
    RMSE_juillet_general[id] <- round(sqrt(1/length(y) * sum((y*100-y_pred*100)^2)),3)
    # Biais_juillet_general[id] <- round(sum(y_pred-y)/length(y),5) #,3)
    Biais_juillet_general[id] <- round(sum(y_pred*100-y*100)/length(y),3)
    ErreurMoyenneAbsolue_juillet_general[id] <- round(sum(abs(y_pred*100-y*100))/length(y),3)
    
    ### Sur les valeurs de aout ###
    tab_test_aout = tab_test[which(months(as.Date(tab_test$Date)) == "août"),]
    y <- tab_test_aout$ProbaAssec_HERMoisAnnee_Apredire_CValid
    y_pred <- tab_test_aout$ProbaAssec_HERMoisAnnee_Predite_CValid
    RMSE_aout_general[id] <- round(sqrt(1/length(y) * sum((y*100-y_pred*100)^2)),3)
    # Biais_aout_general[id] <- round(sum(y_pred-y)/length(y),5) #,3)
    Biais_aout_general[id] <- round(sum(y_pred*100-y*100)/length(y),3)
    ErreurMoyenneAbsolue_aout_general[id] <- round(sum(abs(y_pred*100-y*100))/length(y),3)
    
    ### Sur les valeurs de septembre ###
    tab_test_septembre = tab_test[which(months(as.Date(tab_test$Date)) == "septembre"),]
    y <- tab_test_septembre$ProbaAssec_HERMoisAnnee_Apredire_CValid
    y_pred <- tab_test_septembre$ProbaAssec_HERMoisAnnee_Predite_CValid
    RMSE_septembre_general[id] <- round(sqrt(1/length(y) * sum((y*100-y_pred*100)^2)),3)
    # Biais_septembre_general[id] <- round(sum(y_pred-y)/length(y),5) #,3)
    Biais_septembre_general[id] <- round(sum(y_pred*100-y*100)/length(y),3)
    ErreurMoyenneAbsolue_septembre_general[id] <- round(sum(abs(y_pred*100-y*100))/length(y),3)
    ...
    RMSE_maxAnnees_general[id] <- round(sqrt(1/length(y) * sum((y*100-y_pred*100)^2)),3)
    # Biais_maxAnnees_general[id] <- round(sum(y_pred-y)/length(y),5) #,3)
    Biais_maxAnnees_general[id] <- round(sum(y_pred*100-y*100)/length(y),3)
    ErreurMoyenneAbsolue_maxAnnees_general[id] <- round(sum(abs(y_pred*100-y*100))/length(y),3)
