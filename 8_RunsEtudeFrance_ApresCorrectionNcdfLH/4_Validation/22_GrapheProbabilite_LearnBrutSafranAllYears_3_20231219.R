...
chroniqueFDCbyHERweighted_ <- read.table(paste0(folder_input_,"FlowDurationCurves_HERweighted_meanJm6Jj/",
                                                ifelse(obsSim_=="",nom_GCM_,
                                                       paste0("FDC_",obsSim_,
                                                              ...
                                                              # "ChroniquesBrutes_historical/Modele_ChroniquesCombinees_saf_rcp85_CNRM-CM5_CNRM-ALADIN63/",
...
tab_modeles_ = read.table(list.files(paste0(folder_output_,
                          ...
                          dec = ".", sep = ";", header = T)
                          #        # "/TableGlobale",
                          #        "/ModelResults_ByHERDates_",str_after_nth(str_before_last(basename(il), "\\."),"\\_",2),"_ValidGlobale_",paste(substr(annees_validModels_,3,4),collapse = "-"),
                          #        "_J",ifelse(jourMin_ == 0,"j",paste0("m",jourMin_)),"J",ifelse(jourMax_ == 0,"j",paste0("m",jourMax_)),"_logit.csv"),
