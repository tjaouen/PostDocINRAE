...
for (source_ in source_files_){
  ...
  ### Projections ###
  list_HM_saf_hist_rcp85_ = list.files(paste0(folder_input_,"Tab_ChroniquesProba_LearnBrut_ByHer/",
  ...
  list_HM_saf_rcp85_ = list.files(paste0(folder_input_,"Tab_ChroniquesProba_LearnBrut_ByHer/",
                                         ifelse(obsSim_=="",nom_GCM_,
                                                paste0("FDC_",obsSim_,
                                                       ifelse(nom_FDCfolder_=="","",paste0("_",nom_FDCfolder_)),"/",
  ...
  for (HER_h_ in HER_){
    ...
    if (HER_h_ %in% colnames(tab_)){
      ...
      proba_rcp_20702100 <- chro_rcp26_20702100 %>%
        group_by(Jour_annee) %>%
        summarize(Proba_moyen = mean(Proba, na.rm = TRUE),
                  Proba_median = median(Proba, na.rm = TRUE),
                  Proba_Q5 = quantile(Proba, probs = 0.05, na.rm = TRUE),
      ...
      proba_rcp_20702100 <- chro_rcp45_20702100 %>%
        group_by(Jour_annee) %>%
        summarize(Proba_moyen = mean(Proba, na.rm = TRUE),
                  Proba_median = median(Proba, na.rm = TRUE),
                  Proba_Q5 = quantile(Proba, probs = 0.05, na.rm = TRUE),
      ...
      proba_rcp26_20702100$Periode <- paste0("Rcp45_20702100")
      proba_rcp45_20702100$Periode <- paste0("Rcp26_20702100")
      ...
      custom_colors <- c("Safran_19702005" = "#2b8cbe",
                         "Hist_19702005" = "#fc9272",
                         "Rcp85_20702100" = "#cb181d",
                         "Rcp45_20702100" = ,
                         "Rcp26_20702100" = )
