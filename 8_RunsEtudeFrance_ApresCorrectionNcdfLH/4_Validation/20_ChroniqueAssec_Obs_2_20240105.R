  ...
  if (paste0("eco",sprintf("%03d", i)) %in% colnames(hydro)){
    ...
    if(length(Select_hydro) > 0 ){
      ...
      for (st in 1:length(Select_hydro)){
        ...
        if (dir.exists(paste0(folder_input_,"FlowDurationCurves_meanJm6Jj/",
          ...
          if (file.exists(paste0(folder_input_,"FlowDurationCurves_meanJm6Jj/",
            ...
            # st_hydro <- st_hydro[which((as.Date(st_hydro$Date) >= "2012-01-01") & (as.Date(st_hydro$Date) <= "2022-11-29")),]
            ...
            if ((length(which(!(FDCweighted_$Date == st_hydro$Date))) == 0) & (length(which(!(FDCweighted_$Type == st_hydro$Type))) == 0)){
              ...
              FDCweighted_ <- FDCweighted_[which(FDCweighted_$Date >= min(st_hydro$Date) & FDCweighted_$Date <= max(st_hydro$Date)),]
              st_hydro <- st_hydro[which(st_hydro$Date >= min(FDCweighted_$Date) & st_hydro$Date <= max(FDCweighted_$Date)),]
              FDCweighted_ = cbind(FDCweighted_,
                                   st_hydro$Moyenne_6jours * Weight_hydro[st])
              colnames(FDCweighted_)[length(colnames(FDCweighted_))] = st
              # print(st)
      ...
      if ((length(which(!(tab_HER2_$Date == FDCweighted_$Date))) == 0) & (length(which(!(tab_HER2_$Type == FDCweighted_$Type))) == 0)){
        ...
        tab_HER2_ <- tab_HER2_[which(tab_HER2_$Date >= min(FDCweighted_$Date) & tab_HER2_$Date <= max(FDCweighted_$Date)),]
        FDCweighted_ <- FDCweighted_[which(FDCweighted_$Date >= min(tab_HER2_$Date) & FDCweighted_$Date <= max(tab_HER2_$Date)),]
        tab_HER2_ = cbind(tab_HER2_,
                          FDCweighted_$FDCmeanWeighted)
        colnames(tab_HER2_)[length(colnames(tab_HER2_))] = i
...
if (!(dir.exists(paste0(folder_input_,"FlowDurationCurves_HERweighted_meanJm6Jj/",
                        ifelse(obsSim_=="",nom_GCM_,
                               paste0("FDC_",obsSim_,
                                      ifelse(nom_FDCfolder_param_=="","",paste0("_",nom_FDCfolder_param_)),"/",
                                      ifelse(nom_categorieSimu_=="","",nom_categorieSimu_),"/",
                                      ifelse(nom_GCM_=="","",nom_GCM_))))))){
  dir.create(paste0(folder_input_,"FlowDurationCurves_HERweighted_meanJm6Jj/",
                    ifelse(obsSim_=="",nom_GCM_,
                           paste0("FDC_",obsSim_,
                                  ifelse(nom_FDCfolder_param_=="","",paste0("_",nom_FDCfolder_param_)),"/",
                                  ifelse(nom_categorieSimu_=="","",nom_categorieSimu_),"/",
...
write.table(tab_HER2_, paste0(folder_input_,"FlowDurationCurves_HERweighted_meanJm6Jj/",
                              ifelse(obsSim_=="",nom_GCM_,
                                     paste0("FDC_",obsSim_,
                                            ifelse(nom_FDCfolder_param_=="","",paste0("_",nom_FDCfolder_param_)),"/",
                                            ifelse(nom_categorieSimu_=="","",nom_categorieSimu_),"/",
