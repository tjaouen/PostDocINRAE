...
for (an in annees_learnModels_p_){
  ...
  ### Table par Annee ###
  # Assecs_val = FUNC_CAL_VAL_ValidationSansLearn_SansIC(1,onde,liste_Her,regr,tab_Reg_,jourMin_ = jourMin_, jourMax_ = jourMax_)
  Assecs_val = FUNC_CAL_VAL_ValidationSansLearn_SansIC(annee = 1, Matrice_comp = onde, liste_Her = liste_Her, regression = regr, tab_Reg_ = tab_Reg_, jourMin_ = jourMin_, jourMax_ = jourMax_)
  Assecs_val[3:ncol(Assecs_val)] = round(Assecs_val[3:ncol(Assecs_val)],3)
  ...
  if (dir.exists(paste0(folder_output_,
    ...
    write.table(Assecs_val, paste0(folder_output_,
                                   ...
                                   ifelse(nom_apprentissage_=="","",paste0("/",nom_apprentissage_)),
                                   ifelse(nom_validation_=="","",paste0("/",nom_validation_)),
                                   "/TableParCategories/Results_Jm",jourMin_,"Jj/ModelResults",str_after_nth(str_before_last(basename(il), "\\."),"\\_",2),"_Valid_",paste(substr(annees_validModels_,3,4),collapse = "-"),
