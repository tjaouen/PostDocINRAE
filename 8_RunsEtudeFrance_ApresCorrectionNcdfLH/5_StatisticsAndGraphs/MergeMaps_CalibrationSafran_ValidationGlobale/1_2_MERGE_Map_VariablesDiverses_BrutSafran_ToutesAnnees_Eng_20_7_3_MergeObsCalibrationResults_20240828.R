...
prepareSubgraph <- function(graph_, pos_, removeLegend = T, removeScale = T, scaleIndx, lastline_ = F){
  ...
  if (removeLegend){
    ...
    graph_nl_ <- graph_nl_ + theme(legend.key.height = unit(0.45*ratio_epaisseurs_, "cm"),
                                   ...
                                   legend.title = element_text(colour = "gray38"),
  ...
  graph_nl_$layers[[scaleIndx[1]]]$geom_params$pad_x = unit(+3,"cm")
  graph_nl_$layers[[scaleIndx[1]]]$geom_params$pad_y = unit(+0.5,"cm")
  graph_nl_$layers[[scaleIndx[2]]]$geom_params$pad_x = unit(2,"cm")
...
image3 <- image3 + ggtitle("Bias (unitless)")
image1 <- prepareSubgraph(graph_ = image1, pos_ = c(1,1), removeLegend = F, removeScale = T, scaleIndx = c(2,3), lastline_ = T)
image2 <- prepareSubgraph(graph_ = image2, pos_ = c(1,2), removeLegend = F, removeScale = T, scaleIndx = c(4,5), lastline_ = T)
image3 <- prepareSubgraph(graph_ = image3, pos_ = c(1,3), removeLegend = F, removeScale = F, scaleIndx = c(4,5), lastline_ = T)
...
ggsave(paste0(folder_output_,
              ...
              gsub(".rds",".tiff",str_after_first(basename(chemin_1),"_"))),
       grid_1_,
       width = 13.5, height = 5)
...
ggsave(paste0(folder_output_,
              ...
              ifelse(nom_GCM_=="","",paste0("/",nom_GCM_)),
              "/TableGlobaleMerge/Map_English/MergeObs_Article_",
              gsub(".rds",".png",str_after_first(basename(chemin_1),"_"))),
       grid_1_,
       width = 13.5, height = 5)
...
ggsave(paste0(folder_output_,
              ...
              ifelse(nom_GCM_=="","",paste0("/",nom_GCM_)),
              "/TableGlobaleMerge/Map_English/MergeObs_Article_",
              gsub(".rds",".svg",str_after_first(basename(chemin_1),"_"))),
       grid_1_,
       width = 13.5, height = 5)
...
ggsave(paste0(folder_output_,
              ...
              ifelse(nom_GCM_=="","",paste0("/",nom_GCM_)),
              "/TableGlobaleMerge/Map_English/MergeObs_Article_",
              gsub(".rds",".pdf",str_after_first(basename(chemin_1),"_"))),
       grid_1_,
       width = 13.5, height = 5)
