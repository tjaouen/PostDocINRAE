...
for (g in 1:nrow(graph_list_)){
  ...
  # chemin_6 <- paste0(folder_output_,
  #                    "/15_ResultatsModeles_ValidationParAnnees_ParHer/",
  #                    "30_PresentMesures_HERh_FltOndeAtStart_JctHER89et92_ValidAnSecInterHum_2012_2022_20231221/DebitsComplets_From20120101_To20221231/ApprentissageLeaveOneYearOut/Validation_2LeaveOneYearOut/TableGlobale/Map_English/",
  #                    "/4_Map_Globale_KGE_NonBoot/KGE_NonBoot_sansEt.rds")
  chemin_6 <- list.files(list.files(path = paste0(folder_output_,
                                                  "/15_ResultatsModeles_ValidationParAnnees_ParHer/",
                                                  "30_PresentMesures_HERh_FltOndeAtStart_JctHER89et92_ValidAnSecInterHum_2012_2022_20231221/DebitsComplets_From20120101_To20221231/ApprentissageLeaveOneYearOut/Validation_2LeaveOneYearOut/TableGlobale/Map_English/"),
                                    pattern = paste0("_Map_Globale_",graph_list_$variable[g],"_NonBoot"), full.names = T), pattern = "sansEt.rds", full.names = T)
  # pattern = paste0("_Map_Globale_",strsplit(graph_list_$variable[g],"_")[[1]][1],"_NonBoot"), full.names = T), pattern = "sansEt.rds", full.names = T)
  chemin_6 <- chemin_6[!grepl("save",chemin_6)]
  chemin_1 <- list.files(list.files(path = paste0(folder_output_,
                                                  "/15_ResultatsModeles_ValidationParAnnees_ParHer/",
                                                  nomSim_,
                                                  "/CTRIP_20231128/TableMediane/Map_English/"), pattern = paste0("_Map_Globale_",graph_list_$variable[g],"_NonBoot"), full.names = T), pattern = "sansEt.rds", full.names = T)
  chemin_2 <- list.files(list.files(path = paste0(folder_output_,
                                                  "/15_ResultatsModeles_ValidationParAnnees_ParHer/",
                                                  nomSim_,
                                                  "/GRSD_20231128/TableMediane/Map_English/"), pattern = paste0("_Map_Globale_",graph_list_$variable[g],"_NonBoot"), full.names = T), pattern = "sansEt.rds", full.names = T)
  chemin_5 <- list.files(list.files(path = paste0(folder_output_,
                                                  "/15_ResultatsModeles_ValidationParAnnees_ParHer/",
                                                  nomSim_,
                                                  "/J2000_20231128/TableMediane/Map_English/"), pattern = paste0("_Map_Globale_",graph_list_$variable[g],"_NonBoot"), full.names = T), pattern = "sansEt.rds", full.names = T)
  chemin_3 <- list.files(list.files(path = paste0(folder_output_,
                                                  "/15_ResultatsModeles_ValidationParAnnees_ParHer/",
                                                  nomSim_,
                                                  "/ORCHIDEE_20231128/TableMediane/Map_English/"), pattern = paste0("_Map_Globale_",graph_list_$variable[g],"_NonBoot"), full.names = T), pattern = "sansEt.rds", full.names = T)
  chemin_4 <- list.files(list.files(path = paste0(folder_output_,
                                                  "/15_ResultatsModeles_ValidationParAnnees_ParHer/",
                                                  nomSim_,
                                                  "/SMASH_20231128/TableMediane/Map_English/"), pattern = paste0("_Map_Globale_",graph_list_$variable[g],"_NonBoot"), full.names = T), pattern = "sansEt.rds", full.names = T)
  ...
  image4 <- readRDS(chemin_4)
  image5 <- readRDS(chemin_5)
  image6 <- readRDS(chemin_6)
  image1 <- image1 + ggtitle("CTRIP")
  image2 <- image2 + ggtitle("GRSD")
  image3 <- image3 + ggtitle("ORCHIDEE")
  image4 <- image4 + ggtitle("SMASH")
  image5 <- image5 + ggtitle("J2000")
  image6 <- image6 + ggtitle("Observed data\nLeave-One-Year-Out validation")
  ...
  image1 <- prepareSubgraph(image1, pos_ = c(1,2), removeLegend = T)
  image2 <- prepareSubgraph(image2, pos_ = c(1,3), removeLegend = T)
  image3 <- prepareSubgraph(image3, pos_ = c(2,2), removeLegend = T)
  image4 <- prepareSubgraph(image4, pos_ = c(2,3), removeLegend = T)
  image5 <- prepareSubgraph(image5, pos_ = c(2,1), removeLegend = T)
  image6 <- prepareSubgraph(image6, pos_ = c(1,1), removeLegend = T)
