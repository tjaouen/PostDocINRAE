...
noms_colonnes_nouveaux <- colnames(tableau_sans_geometry_grouped_km2)[-c(1,2)] %>%
  as.numeric() %>%
  sprintf("eco%03d", .)
colnames(tableau_sans_geometry_grouped_km2)[-c(1,2)] <- noms_colonnes_nouveaux
...
setdiff(bassins_versants$Code8, tableau_sans_geometry_grouped_km2$Code8) # Un bassin versant present dans le shp n'est pas present dans le tableau final : Y4537010 -> Il est situe dans la mediterranee et n'a d'intersection avec aucun HER2h.
setdiff(tableau_sans_geometry_grouped_km2$Code8,bassins_versants$Code8) # NA
...
write.table(tableau_pourcentages,
            "/home/tjaouen/Documents/Input/HYDRO/EtudeFrance/CorrespondanceHydroHer/HER2hybrides_PointsSimulationExplore2/Version_HER2h20190607_1_TJ20231130/PropSurfaceHER2hybrides_HorsRMCfromShp_TabDoubleEntree_Proportions_5_20231130.csv",
            sep = ";", dec = ".", row.names = FALSE)
# write.table(tableau_pourcentages,
