...
### Run ###
liste <- c(list.files(paste0(folder_output_,"1_MatricesInputModeles_ParHERDates/",nomSim_,ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_))), pattern="MatInputModel", full.names = T))
NSE <- c()
colNSE <- c()
...
write.table(onde_CampOndeExcl_, paste0(folder_output_,"1_MatricesInputModeles_ParHERDates/",nomSim_,ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),"/",str_before_first(basename(il),"_"),"_CampOndeExcl_",str_after_first(basename(il),"_")), sep=";", row.name=F, quote=F)
...
write.table(nouvelle_table,
            paste0(folder_output_,
                   ...
                   nom_GCM_,"_",as.character(min(annees_inputMatrice_)),"_",as.character(max(annees_inputMatrice_)),"_",obsSim_,"_Weight_merge.csv"), sep=";", row.name=F, quote=F)
