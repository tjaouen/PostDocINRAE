...
# source("/home/tjaouen/Documents/Src/ChangementClimatique_Bottet2019/CodesTristan/7_RunsEtudeFrance_DifferentsModelesHydro/1_Parameters/0_SimulationParameters_AvecCC_2_20230227_parallel.R")
...
### Run ###
liste <- list.files(paste0(folder_input_,
                           "StationsSelectionnees/SelectionCsv/",
                           nom_selectStations_,"/",
                           ifelse(nom_GCM_=="","","TablesParModele_20231003/")),pattern="Stations_HYDRO", full.names = T)
# liste <- list.files(paste0(folder_input_,"StationsSelectionnees/SelectionCsv/",nom_selectStations_),pattern="Stations_HYDRO_KGES", full.names = T)
# liste <- list.files(paste0(folder_input_,"StationsSelectionnees/SelectionCsv/",nom_selectStations_,"/",climatScenarioModeleHydro_),pattern="Stations_HYDRO_KGES", full.names = T)
...
for (il in liste) {
  ...
  # grid <- expand.grid(annee = annees_learn_, i = HER_)
  # 
  # # Etude par annee, par HER et par station ONDE
  # output <- foreach(idx = 1:nrow(grid), .combine = "rbind", .verbose = T) %dopar% {
  #   
  #   annee <- grid$annee[idx]
  #   i <- grid$i[idx]
    ...
    foreach(i = HER_, .combine = "rbind", .verbose = T) %dopar% { #, errorhandling='pass'
      ...
      tryCatch({
            ...
            for (date in 1:length(liste_mois)){
              ...
              if(length(Select_hydro) > 0 ){
                ...
                for (st in 1:length(Select_hydro)){
                  ...
                  if (dir.exists(paste0(folder_input_,"FlowDurationCurves/",presFut_param_,"/",
                    ...
                    if (file.exists(paste0(folder_input_,"FlowDurationCurves/",presFut_param_,"/",
                                           ifelse(obsSim_=="",nom_GCM_,
                                                  paste0("FDC_",obsSim_,
                                                         ifelse(nom_FDCfolder_param_=="","",paste0("_",nom_FDCfolder_param_)),"/",
                                                         ifelse(nom_categorieSimu_=="","",nom_categorieSimu_),"/",
                      ...
                      st_hydro = read.table(paste0(folder_input_,"FlowDurationCurves/",presFut_param_,"/",
                                                   ifelse(obsSim_=="",nom_GCM_,
                                                          paste0("FDC_",obsSim_,
                                                                 ifelse(nom_FDCfolder_param_=="","",paste0("_",nom_FDCfolder_param_)),"/",
                                                                 ifelse(nom_categorieSimu_=="","",nom_categorieSimu_),"/",
                      ...
                      if (length(which(st_hydro$Date == as.Date(date_deb))) >0){
                        ...
                        cmpt_hydro_ <- cmpt_hydro_ + 1
                        row_st_hydro_ = which(st_hydro$Date == as.Date(date_deb))
                        
                        for(y in -nbJoursIntervalle_:nbJoursIntervalle_){
                          if (length(which(st_hydro$Date == as.Date(date_deb)+y)) != 0){
                            
                            if(which(st_hydro$Date == as.Date(date_deb)+y) <nrow(st_hydro) & which(st_hydro$Date == as.Date(date_deb)+y)>0){
                              if (!is.na(st_hydro$FreqNonDep[which(st_hydro$Date == as.Date(date_deb)+y)])){
                                compt = compt + 1
                                weightcompt[(nbJoursIntervalle_+1+y)] = weightcompt[(nbJoursIntervalle_+1+y)]+Weight_hydro[st]
                                if (st_hydro$Date[row_st_hydro_+y] == (as.Date(date_deb)+y)){
                                  output_hydro[compt,y+nbJoursIntervalle_+1] = Weight_hydro[st]*st_hydro$FreqNonDep[row_st_hydro_+y]
                                }else{
                                  if (which(st_hydro$Date == as.Date(date_deb)+y) > 0){
                                    output_hydro[compt,y+nbJoursIntervalle_+1] = Weight_hydro[st]*st_hydro$FreqNonDep[which(st_hydro$Date == as.Date(date_deb)+y)] # Freq non depassement des jours j-6 a j+6
                                  }
                                }
                              }else{
                                compt = compt + 1
                                output_hydro[compt,y+nbJoursIntervalle_+1] = NA
                                cmpt_NA_ = cmpt_NA_ + 1
                              }
                            }
                            else{
                            ...
                            print("ATTENTION, DONNEES INCOMPLETES AUTOUR D'UNE DATE ONDE.")
                            compt = compt + 1
                            output_hydro[compt,y+nbJoursIntervalle_+1] = NA
                            cmpt_NA_ = cmpt_NA_ + 1
                          }
                ...
                if (nrow(output_hydro) > 0){
                  ...
                  output_parAnneeParHer_toutesDates[date,5+2*nbJoursIntervalle_]=cmpt_hydro_
                  output_parAnneeParHer_toutesDates[date,6+2*nbJoursIntervalle_]=length(Select_hydro)
                  output_parAnneeParHer_toutesDates[date,7+2*nbJoursIntervalle_]=cmpt_NA_
                  output_parAnneeParHer_toutesDates[date,8+2*nbJoursIntervalle_]=length(obs_us) #Nombre d'observations ONDE
                  output_parAnneeParHer_toutesDates[date,9+2*nbJoursIntervalle_]=length(assec_us) #Nombre d'observations ONDE
                  
                } else {
                  output_parAnneeParHer_toutesDates[date,4:(9+2*nbJoursIntervalle_)] = NA
                }
        ...
        colnames(output_parAnneeParHer_toutesDates)<-c(HER_variable_,"Date","%_Assec",
                                                       paste0("Freq_jmoins",nbJoursIntervalle_:1),
                                                       "Freq_j",
                                                       paste0("Freq_jplus",1:nbJoursIntervalle_),
        ...
        if (!(dir.exists(paste0(folder_output_,"1_MatricesInputModeles_ParHERDates/",nomSim_,ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),"/tmp/")))){ dir.create(paste0(folder_output_,"1_MatricesInputModeles_ParHERDates/",nomSim_,ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),"/tmp/"))}
        write.table(output_parAnneeParHer_toutesDates, paste0(folder_output_,"1_MatricesInputModeles_ParHERDates/",nomSim_,ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),"/tmp/MatInputModel_ByHERDates_",nom_GCM_,"_",as.character(min(annees_inputMatrice_)),"_",as.character(max(annees_inputMatrice_)),"_",str_replace_all(basename(il), 'Stations_HYDRO_|.csv', ''),"_",obsSim_,"_Weight_",annee,"_HER",i,".csv"), sep=";", row.name=F, quote=F)
  ...
  write.table(output, paste0(folder_output_,"1_MatricesInputModeles_ParHERDates/",nomSim_,ifelse(nom_categorieSimu_=="","",paste0("/",nom_categorieSimu_)),"/MatInputModel_ByHERDates_",nom_GCM_,"_",as.character(min(annees_inputMatrice_)),"_",as.character(max(annees_inputMatrice_)),"_",str_replace_all(basename(il), 'Stations_HYDRO_|.csv', ''),"_",obsSim_,"_Weight.csv"), sep=";", row.name=F, quote=F)
}
