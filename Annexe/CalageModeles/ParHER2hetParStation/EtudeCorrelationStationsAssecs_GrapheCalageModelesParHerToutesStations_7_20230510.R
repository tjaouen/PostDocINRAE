...
for (HER_ in HER_list_){
  ...
  if (dim(tab_input_)[1] > 0){
    ...
    if (sum(!(colnames(tab_merge_)[numColStat_] == colnames(tab_weight)))>0){
      print("Stop")
      print(Stop)
    }
    ...
    if (sum(!(colnames(tab_[4:ncol(tab_)]) == colnames(tab_weight)))>0){
      print("Stop")
      print(Stop)
    }
    ...
    png(paste0(folder_output_,"13_Graphes_CalageModele_FDCAssecs_ParHERetParStations/",nomSim_,"/Version1_20230512/GrapheModParHERetParStat_HER_",HER_,"_logit.png"),
