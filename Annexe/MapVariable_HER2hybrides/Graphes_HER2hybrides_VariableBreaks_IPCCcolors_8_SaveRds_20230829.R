...
plot_map_variable <- function(tab_, varname_, vartitle_, breaks_, output_name_, title_, reverseColors_, nomPalette_, labels_name_ = FALSE, reverseLegend_ = FALSE, echelleAttenuee_ = FALSE, addValueUnder = NULL){
  ...
  if (reverseColors_ == T){
    ...
    # palette_col_ <- rev(colorRampPalette(brewer.pal(min(length(breaks_) - 1, 9), "IPCC6"))(length(breaks_) - 1))
    ...
    # palette_col_ <- colorRampPalette(brewer.pal(min(length(breaks_) - 1, 9), "IPCC6"))(length(breaks_) - 1)
  ...
  p <- p + ggtitle(title_) +
    ...
    theme(axis.line = element_line(color = rgb(0.6,0.6,0.6)),
  ...
  p_main <- ggplot() +
    ...
    scale_fill_manual(values=setNames(palette_col_,labels_name_),
                      ...
                      drop = F)
  if (paste0(varname_,"_IC95inf") %in% colnames(tab_)){
    p_main <- p_main + geom_label_repel(data = data.frame(x = her2_centers[,1], y = her2_centers[,2],
                                                label = paste0(her2_attrib, ifelse(is.na(fr.prj[[varname_]]),"",paste0(" : ",round(fr.prj[[varname_]],2),"\n[",round(fr.prj[[paste0(varname_,"_IC95inf")]],2),";",round(fr.prj[[paste0(varname_,"_IC95sup")]],2),"]")))),
                              aes(x = x, y = y, label = label),
                              fill.alpha = 0.5,
                              box.padding = 0.5,
...
### Function ###
plot_map_variable_sansEtiquettes <- function(tab_, varname_, vartitle_, breaks_, output_name_, title_, reverseColors_, nomPalette_, labels_name_ = FALSE, sansTexteHer_ = FALSE, reverseLegend_ = FALSE, echelleAttenuee_ = FALSE, addValueUnder = NULL){
