...
plot_map_variable <- function(tab_, varname_, vartitle_, breaks_, output_name_, title_, reverseColors_, nomPalette_, labels_name_ = FALSE, reverseLegend_ = FALSE, echelleAttenuee_ = FALSE, addValueUnder = NULL, HER2_excluesDensity_ = NULL, limits_ = c(0,40)){
  ...
  p <- ggplot() +
    ...
    scale_fill_gradientn(colors = palette_col_,
                         ...
                         breaks = breaks_,
                         labels = breaks_,
                         # breaks = seq(limits_[1], limits_[2], by = 5),
                         # labels = seq(limits_[1], limits_[2], by = 5),
                         # labels = paste0("\n",seq(limits_[1], limits_[2], by = 5)),
                         # breaks = c(min(na.omit(fr.df$Moyenne_Mean_Value)), (max(na.omit(fr.df$Moyenne_Mean_Value))-min(na.omit(fr.df$Moyenne_Mean_Value)))/2 , max(na.omit(fr.df$Moyenne_Mean_Value))),
                         # labels = c(round(min(na.omit(fr.df$Moyenne_Mean_Value))), round((max(na.omit(fr.df$Moyenne_Mean_Value))-min(na.omit(fr.df$Moyenne_Mean_Value)))/2), round(max(na.omit(fr.df$Moyenne_Mean_Value)))),
  ...
  # theme(legend.position = c(468634.4,6326432),
  theme(legend.position = c(0.92,0.5),
    ...
    guides(#fill = guide_colorbar(barwidth = 1, barheight = 10),
      fill = guide_legend(reverse = reverseLegend_))+#,
                          # keywidth = 2,
                          # keyheight = 2)) +
    coord_fixed(ratio = 1)
