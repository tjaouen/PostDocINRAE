...
plot_map_variable <- function(tab_, varname_, vartitle_, breaks_, output_name_, title_, reverseColors_, nomPalette_, labels_name_ = FALSE, reverseLegend_ = FALSE, echelleAttenuee_ = FALSE, addValueUnder = NULL, HER2_excluesDensity_ = NULL, subtitle_ = NULL){
  ...
  # labels_name_annotation_ <- substr(str_before_first(labels_name_[1],","),2,nchar(str_before_first(labels_name_[1],",")))
  ...
  # pos_leg_x_ = max(fr.df$long) - (1-1.2)*(max(fr.df$long) - min(fr.df$long))
  # pos_leg_x_ = max(fr.df$long) - (1-0.98)*(max(fr.df$long) - min(fr.df$long))
  # pos_leg_y_ = max(fr.df$lat) - 0.5*(max(fr.df$lat) - min(fr.df$lat))
  ...
  p <- ggplot() +
    ...
    scale_fill_manual(values=df_color_,
                      name=vartitle_,
                      breaks=labels_,
                      # labels=labels_name_annotation_,
                      drop = F)
  if (paste0(varname_,"_IC95inf") %in% colnames(tab_)){
  ...
  p <- p + annotate("text",
                    ...
                    # label = TeX("\\textbf{Probabilité moyenne d'assec}"),
                    label = title_,
                    # label = TeX(title_),
                    # label = TeX(paste0("Probabilité moyenne d'assec\nModèle hydrologique ",str_before_first(nom_categorieSimu_,"_"))),
                    # label = TeX(paste0("\\textbf{Probabilité moyenne d'assec}")),
                    bold = T,
                    size = 10,
                    hjust = 0,
                    vjust = 0.5)#,
  ...
  p <- p + annotate("text",
                    ...
                    label = subtitle_,
                    # label = TeX(subtitle_),
                    # label = TeX(paste0("Modèle hydrologique ",str_before_first(nom_categorieSimu_,"_"),"\nPériode : Juillet-Octobre ",)),
                    size = 6,
                    hjust = "right",
                    vjust = 0.5)
  ...
  legend_data <- plot_build$plot$layers[[1]]$data
  ...
  g <- ggplot_build(p)
  legend_coords <- g$layout$panel_params[[1]]$layout$legend
  legend_x <- legend_coords$left + (legend_coords$width / 2)
  legend_y <- legend_coords$top + (legend_coords$height / 2)
  
  
  
  g <- ggplotGrob(p)
  legend <- which(g$layout$name == "guide-box")
  legend_coords <- g$layout[legend, c("t", "l", "b", "r")]
  legend_x <- mean(c(legend_coords$l, legend_coords$r))
  legend_y <- legend_coords$t
  
  legend_height <- legend_coords$b - legend_coords$t
  
  # Calculer les coordonnées y pour placer les annotations au-dessus et en dessous de la légende
  legend_y_top <- legend_coords$t + legend_height
  legend_y_bottom <- legend_coords$t
  
  
  p + annotate("segment", x = 468634.4, xend = 464564.4, y = 6326432, yend = 6326432,
               colour = "blue")

  # Ajouter une annotation au-dessus de la légende
  p <- p + annotate("text",
                    x = 1,
                    y = 1, 
                    label = "a")#,
  ...
  #                   x = legend_x,
  #                   y = legend_y_top, 
  #                   label = labels_name_annotation_)#,
                    # color = "black", size = 5, hjust = 0.5)
  
  # Ajouter une annotation en dessous de la légende
  p <- p + annotate("text", x = legend_x, y = legend_y_bottom, 
                    label = min_legende_annotation_, color = "black", size = 5, hjust = 0.5)
  
  # p <- p + annotate("text", x = legend_x, y = legend_y + unit(1, "npc"), 
  #                   label = labels_name_annotation_, color = "black", size = 5, hjust = 0.5)
  # 
  # # Ajouter une annotation en dessous de la légende
  # p <- p + annotate("text", x = legend_x, y = legend_y - unit(1, "npc"), 
  #                   label = min_legende_annotation_, color = "black", size = 5, hjust = 0.5)
  # 
  # p <- p + annotate("text", x = NULL, y = NULL, label = labels_name_annotation_, 
  #                   color = "black", size = 5, hjust = 0.5, vjust = -1) +
  #   annotate("text", x = NULL, y = NULL, label = min_legende_annotation_, 
  #            color = "black", size = 5, hjust = 0.5, vjust = 2.5)
