...
for(inum in 1:length(liste.GCM)){
  ...
  iONDEPST <- which(imPST>1 & imPST<8) # ajustement on veut mai (5) - octobre (10)
  iONDEFUT <- which(imFUT>1 & imFUT<8)
...
write.table(meanPST, paste0(dir.out,"statistique_revised/","PSTmean.csv"),
            sep=";", row.name=F, quote=F)
write.table(meanMI, paste0(dir.out,"statistique_revised/","MImean.csv"),
            sep=";", row.name=F, quote=F)
write.table(meanFIN, paste0(dir.out,"statistique_revised/","FINmean.csv"),
            sep=";", row.name=F, quote=F)

write.table(sdPST, paste0(dir.out,"statistique_revised/","PSTsd.csv"),
            sep=";", row.name=F, quote=F)
write.table(sdMI, paste0(dir.out,"statistique_revised/","MIsd.csv"),
            sep=";", row.name=F, quote=F)
write.table(sdFIN, paste0(dir.out,"statistique_revised/","FINsd.csv"),
            sep=";", row.name=F, quote=F)
...
paste0(dir.out,"statistique_revised/","statHER_mean.csv"),
sep=";", row.name=F, quote=F)
...
paste0(dir.out,"statistique_revised/","statHER_sd.csv"),
sep=";", row.name=F, quote=F)
...
write.table(rbind(apply(meanPST[,2:length(meanPST[1,])],2,function(x){sd(as.numeric(x))}),
            ...
            paste0(dir.out,"statistique_revised/","statHER_sdmean.csv"),
            sep=";", row.name=F, quote=F)
