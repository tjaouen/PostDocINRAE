...
x11()
ggplot() +
  geom_sf(data = region_HER, fill = "#ece7f2", alpha = 0, color = "#454547", lwd = 1) +
  
  geom_sf(data = intersection_BV, alpha = 0, color = "#034e7b", lwd = 0.5) +
  # geom_sf(data = intersection_BV, fill = "#a6bddb", alpha = 0.5, color = "#034e7b", lwd = 0.2) +
  
  geom_sf(data = intersection_RHT, color = "#3690c0", lwd = 0.2) +
  geom_sf(data = intersection_HYDRO, aes(label = Cod10), color = "#034e7b", size = 5) +
...
# Filtrer les données de la région avec CdHER2 = 8
region_HER <- HER[HER$CdHER2 == 3, ]

# Filtrer les points ONDE dans la région CdHER2 = 8
points_ONDE <- ONDE[st_within(ONDE, region_HER), ]




# Filtrer les points HYDRO dans la région CdHER2 = 8
points_HYDRO <- HYDRO[st_within(HYDRO, region_HER), ]

# Filtrer les cours d'eau RHT dans la région CdHER2 = 8
rivers_RHT <- RHT[st_intersects(RHT, region_HER), ]

# Afficher le ggplot
ggplot() +
  geom_sf(data = region_HER, fill = "lightblue") +
  geom_sf(data = points_ONDE, color = "red") +
  geom_sf(data = points_HYDRO, color = "green") +
  geom_sf(data = rivers_RHT, color = "blue") +
  theme_void()




# Vérifier les systèmes de coordonnées spatiales (CRS)
st_crs(HER)
st_crs(ONDE)
st_crs(HYDRO)
st_crs(RHT)

# Identifier le CRS le plus approprié et reprojeter les données
# Par exemple, si HER est votre référence, vous pouvez reprojeter ONDE, HYDRO et RHT dans son CRS
ONDE <- st_transform(ONDE, st_crs(HER))
HYDRO <- st_transform(HYDRO, st_crs(HER))
RHT <- st_transform(RHT, st_crs(HER))
