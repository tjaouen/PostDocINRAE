...
### Parameters ###
nom_categorieSimu_CTRIP <- "CTRIP_20231128/ChroniquesCombinees_saf_hist_rcp85/"
chro_CTRIP = read.table(paste0("/media/tjaouen/Lexar/INRAE_Bckp/20231123/Input/HYDRO/EtudeFrance/Tab_ChroniquesProba_LearnBrut_ByHer/FDC_Observes/Mod_DebitsComplets_From20120101_To20221231/Tab_ChroniquesProba_LearnBrut_ByHer.txt"),
                   sep = ";", dec = ".", header = T)
colnames(chro_CTRIP) <- gsub("^X", "", colnames(chro_CTRIP))
chro_CTRIP <- chro_CTRIP[,c("Date","Type",as.character(HER_h_))]
chro_CTRIP <- chro_CTRIP[which(chro_CTRIP$Type == "Observes" & year(chro_CTRIP$Date) >= 2012 & year(chro_CTRIP$Date) <= 2022),]
chro_CTRIP <- subset(chro_CTRIP, select = -c(Type))
chro_CTRIP$Mediane <- chro_CTRIP[,2]
chro_CTRIP$HM <- "CTRIP"

nom_categorieSimu_ <- "GRSD_20231128/ChroniquesCombinees_saf_hist_rcp85/"
chro_GRSD = read.table(paste0("/media/tjaouen/Lexar/INRAE_Bckp/20231123/Input/HYDRO/EtudeFrance/Tab_ChroniquesProba_LearnBrut_ByHer/FDC_Observes/Mod_DebitsComplets_From20120101_To20221231/Tab_ChroniquesProba_LearnBrut_ByHer.txt"),
                   sep = ";", dec = ".", header = T)
colnames(chro_GRSD) <- gsub("^X", "", colnames(chro_GRSD))
chro_GRSD <- chro_GRSD[,c("Date","Type",as.character(HER_h_))]
chro_GRSD <- chro_GRSD[which(chro_GRSD$Type == "Observes" & year(chro_GRSD$Date) >= 2012 & year(chro_GRSD$Date) <= 2022),]
chro_GRSD <- subset(chro_GRSD, select = -c(Type))
chro_GRSD$Mediane <- chro_GRSD[,2]
chro_GRSD$HM <- "GRSD"

nom_categorieSimu_ <- "ORCHIDEE_20231128/ChroniquesCombinees_saf_hist_rcp85/"
chro_ORCHIDEE = read.table(paste0("/media/tjaouen/Lexar/INRAE_Bckp/20231123/Input/HYDRO/EtudeFrance/Tab_ChroniquesProba_LearnBrut_ByHer/FDC_Observes/Mod_DebitsComplets_From20120101_To20221231/Tab_ChroniquesProba_LearnBrut_ByHer.txt"),
                   sep = ";", dec = ".", header = T)
colnames(chro_ORCHIDEE) <- gsub("^X", "", colnames(chro_ORCHIDEE))
chro_ORCHIDEE <- chro_ORCHIDEE[,c("Date","Type",as.character(HER_h_))]
chro_ORCHIDEE <- chro_ORCHIDEE[which(chro_ORCHIDEE$Type == "Observes" & year(chro_ORCHIDEE$Date) >= 2012 & year(chro_ORCHIDEE$Date) <= 2022),]
chro_ORCHIDEE <- subset(chro_ORCHIDEE, select = -c(Type))
chro_ORCHIDEE$Mediane <- chro_ORCHIDEE[,2]
chro_ORCHIDEE$HM <- "ORCHIDEE"

nom_categorieSimu_ <- "SMASH_20231128/ChroniquesCombinees_saf_hist_rcp85/"
chro_SMASH = read.table(paste0("/media/tjaouen/Lexar/INRAE_Bckp/20231123/Input/HYDRO/EtudeFrance/Tab_ChroniquesProba_LearnBrut_ByHer/FDC_Observes/Mod_DebitsComplets_From20120101_To20221231/Tab_ChroniquesProba_LearnBrut_ByHer.txt"),
                   sep = ";", dec = ".", header = T)
colnames(chro_SMASH) <- gsub("^X", "", colnames(chro_SMASH))
chro_SMASH <- chro_SMASH[,c("Date","Type",as.character(HER_h_))]
chro_SMASH <- chro_SMASH[which(chro_SMASH$Type == "Observes" & year(chro_SMASH$Date) >= 2012 & year(chro_SMASH$Date) <= 2022),]
chro_SMASH <- subset(chro_SMASH, select = -c(Type))
chro_SMASH$Mediane <- chro_SMASH[,2]
chro_SMASH$HM <- "SMASH"
...
p_mean <- ggplot() +
  ...
  geom_line(data = chro_,
            aes(x = as.Date(Date), y = Mediane, group = 1, lineend = "round", color = HM), lwd = 1*ratio_epaisseurs_) + # linetype = Type, color = Legend, color = "#cc4c02" 
...
png(paste0("/home/tjaouen/Documents/Administratif/Conferences/EGU2024/Presentation/Images_JaouenTristan/ChroniqueCalibration_HER",HER_h_,"_Observed_3_20240830.png"), width = 9*100)
plot(p_mean)
dev.off()
