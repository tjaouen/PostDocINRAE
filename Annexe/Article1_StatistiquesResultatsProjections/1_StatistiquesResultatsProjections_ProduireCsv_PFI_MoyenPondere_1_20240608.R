...
for (d in 1:nrow(dates_)){
  ...
  for (n in 1:length(nom_categorieSimu_list_)){
    ...
    for (f in 1:length(list_)){
      ...
      # tab_filtered <- tab_ %>%
      #   filter(Year >= dates_$date_debut[d] & Year <= dates_$date_fin[d] & 
      ...
      tab_filtered <- tab_ %>%
        filter(year(Date) >= dates_$date_debut[d] & year(Date) <= dates_$date_fin[d] &
                 ...
                 Type != "Safran") %>%
        select(contains("debit"))
      means_HER_[f] <- mean(unlist(tab_filtered), na.rm = TRUE)
    ...
    results_ <- merge(results_, df_means_HER_, by.x = "HER", by.y = "HER", all.x = T)
    results_[[gsub("/","_",nom_categorieSimu_list_[n])]] = results_$means_HER_
    results_ <- results_ %>% select(-means_HER_)
    # dim(df_merge_)
    
    # weighted_median_HER <- df_merge_ %>%
    #   summarize(weighted_median = sum(means_HER_ * Area_km2) / sum(Area_km2))
    # weighted_median_HER
    
    # results_
    # results_$MedianPFIjuilletOct[n] <- length(which(means_HER_>25))
    # print(results_)
    
    write.table(results_,
                paste0("/media/tjaouen/Ultra Touch/Backup/Main/Output/ChangementClimatique2019/EtudeFrance/22_GrapheChroniqueProbabilite_OneModelHorizon20702100_Projections/32_ObservesReanalyseSafran_ApprentissageTousScenariosProjections_20240208/StatistiquesPFI/StatistiquesPFI_PFIMoyennePondereeOctJuil_HistoricalRcp_JuilOct_",dates_$date_debut[d],dates_$date_fin[d],"_1_20240522.csv"),
                sep = ";", dec = ".", row.names = F)
