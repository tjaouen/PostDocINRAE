...
for (nom_categorieSimu_list_ in nom_categorieSimu_list_list_){
  ...
  for (date_intervalle_ in list(c("1976-01-01","2005-12-31"),
    ...
    for (HER_h_ in HER_){
      ...
      # median(as.Date(unlist(tab_finishDate_HER_generale_[which(tab_finishDate_HER_generale_$Year >= min(year(date_intervalle_)) &
      #                                                                                   tab_finishDate_HER_generale_$Year <= max(year(date_intervalle_))),
      #                                                                           2:ncol(tab_finishDate_HER_generale_)])) - 
      #                                 as.Date(unlist(tab_finishDate_HER_generale_[which(tab_finishDate_HER_generale_$Year >= min(year(c("1976-01-01","2005-12-31"))) &
      #                                                                                     tab_finishDate_HER_generale_$Year <= max(year(c("1976-01-01","2005-12-31")))),
      #                                                                             2:ncol(tab_finishDate_HER_generale_)])), na.rm = T)
    }
  }
}
