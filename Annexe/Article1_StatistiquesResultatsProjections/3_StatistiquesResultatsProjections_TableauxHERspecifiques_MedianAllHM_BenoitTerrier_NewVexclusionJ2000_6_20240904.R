...
for (nom_categorieSimu_list_ in nom_categorieSimu_list_list_){
  ...
  for (date_intervalle_ in list(c("1976-01-01","2005-12-31"),
    ...
    for (HER_h_ in HER_){
      ...
      # Proba mean #
      proba_median_ <- lapply(unlist(tab_ProbaMeanJuilOct_HER_generale_[which(tab_ProbaMeanJuilOct_HER_generale_$Year >= min(year(date_intervalle_)) &
      ...
      # print(paste0("Avant: ",tab_allModels[1,"General_rcp26_H0_initDate_AnneeProjModeleMedian_"]))
      ...
      # print(paste0("Apres: ",tab_allModels[1,"General_rcp26_H0_initDate_AnneeProjModeleMedian_"]))
      # print(aaaaa)
      
      # Init delai #
      initDelai_median_ <- format(median(as.Date(unlist(tab_initDate_HER_generale_[which(tab_initDate_HER_generale_$Year >= min(year(date_intervalle_)) &
                                                                                           tab_initDate_HER_generale_$Year <= max(year(date_intervalle_))),
                                                                                   2:ncol(tab_initDate_HER_generale_)] - 
                                                          tab_initDate_HER_generale_[which(tab_initDate_HER_generale_$Year >= min(year(c("1976-01-01","2005-12-31"))) &
                                                                                             tab_initDate_HER_generale_$Year <= max(year(c("1976-01-01","2005-12-31")))),
                                                                                     2:ncol(tab_initDate_HER_generale_)])), na.rm = T),"%d/%m")
      tab_allModels[which(tab_allModels$HER == HER_h_),paste0("General_",str_before_first(str_after_last(nom_categorieSimu_,"_"),"/"),"_",horizon_,"_initDelai_AnneeProjModeleMedian_")] = initDelai_median_
      
      
      # initDate_median_ <- lapply(tab_initDate_HER_generale_[which(tab_initDate_HER_generale_$Year >= min(year(date_intervalle_)) &
      #                                                               tab_initDate_HER_generale_$Year <= max(year(date_intervalle_))),
      #                                                       2:ncol(tab_initDate_HER_generale_)], function(x) round(median(x,na.rm=T),1))
      ...
      # Finish delai #
      finishDelai_median_ <- format(median(as.Date(unlist(tab_finishDate_HER_generale_[which(tab_finishDate_HER_generale_$Year >= min(year(date_intervalle_)) &
                                                                                               tab_finishDate_HER_generale_$Year <= max(year(date_intervalle_))),
                                                                                   2:ncol(tab_finishDate_HER_generale_)] - 
                                                            tab_finishDate_HER_generale_[which(tab_finishDate_HER_generale_$Year >= min(year(c("1976-01-01","2005-12-31"))) &
                                                                                             tab_finishDate_HER_generale_$Year <= max(year(c("1976-01-01","2005-12-31")))),
                                                                                     2:ncol(tab_finishDate_HER_generale_)])), na.rm = T),"%d/%m")
      tab_allModels[which(tab_allModels$HER == HER_h_),paste0("General_",str_before_first(str_after_last(nom_categorieSimu_,"_"),"/"),"_",horizon_,"_finishDelai_AnneeProjModeleMedian_")] = finishDelai_median_
