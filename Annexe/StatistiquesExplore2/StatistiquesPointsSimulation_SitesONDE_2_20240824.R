...
median(c(tab_CTRIP$SurfaceTopo_ChoixDefinitifPointSimu,
         ...
         tab_SMASH$SurfaceTopo_ChoixDefinitifPointSimu)) # 178
quantile(c(tab_CTRIP$SurfaceTopo_ChoixDefinitifPointSimu,
           tab_GRSD$SurfaceTopo_ChoixDefinitifPointSimu,
           tab_J2000$SurfaceTopo_ChoixDefinitifPointSimu,
           tab_ORCHIDEE$SurfaceTopo_ChoixDefinitifPointSimu,
           tab_SMASH$SurfaceTopo_ChoixDefinitifPointSimu), probs = 0.25) # 95
quantile(c(tab_CTRIP$SurfaceTopo_ChoixDefinitifPointSimu,
           tab_GRSD$SurfaceTopo_ChoixDefinitifPointSimu,
           tab_J2000$SurfaceTopo_ChoixDefinitifPointSimu,
           tab_ORCHIDEE$SurfaceTopo_ChoixDefinitifPointSimu,
           tab_SMASH$SurfaceTopo_ChoixDefinitifPointSimu), probs = 0.75) # 400
...
median(na.omit(c(tab_CTRIP$Altitude,
                 ...
                 tab_SMASH$Altitude))) # 178
quantile(na.omit(c(tab_CTRIP$Altitude,
                   tab_GRSD$Altitude,
                   tab_J2000$Altitude,
                   tab_ORCHIDEE$Altitude,
                   tab_SMASH$Altitude)), probs = 0.25) # 70
quantile(na.omit(c(tab_CTRIP$Altitude,
                   tab_GRSD$Altitude,
                   tab_J2000$Altitude,
                   tab_ORCHIDEE$Altitude,
                   tab_SMASH$Altitude)), probs = 0.75) # 309

### Obs ###
tab_Obs <- read.table("/home/tjaouen/Documents/Input/HYDRO/EtudeFrance/StationsSelectionnees/SelectionCsv/SelectionCsv_30_PresentMesures_HERh_FltOndeAtStart_JctHER89et92_ValidAnSecInterHum_2012_2022_20231221/Stations_HYDRO_KGESUp0.00_DispSup-1_2.csv",
                      sep = ";", dec = ".", header = T)
dim(tab_Obs) # 1008
median(tab_Obs$SurfaceTopo, na.rm = T) # 173
quantile(tab_Obs$SurfaceTopo, probs = 0.25, na.rm = T) # 85
quantile(tab_Obs$SurfaceTopo, probs = 0.75, na.rm = T) # 396
length(which(is.na(tab_Obs$SurfaceTopo))) # 0

median(tab_Obs$Altitude, na.rm = T) # 170
length(which(is.na(tab_Obs$Altitude))) # 28

### ONDE ###
tab_ONDE <- read.table("/home/tjaouen/Documents/Input/ONDE/Data_Description/DescriptionSites/Surf_ONDE_3_AjoutMetadonneesManquantes_20230912.csv", sep =",", dec=".", header = T)
dim(tab_ONDE)

median(tab_ONDE$Surf_BV,na.rm = T) # 24
quantile(tab_ONDE$Surf_BV, probs = 0.25, na.rm = T) # 12
quantile(tab_ONDE$Surf_BV, probs = 0.75, na.rm = T) # 50
length(which(is.na(tab_ONDE$Surf_BV))) # 107

median(tab_ONDE$altitude,na.rm = T)  # 158.4
quantile(tab_ONDE$altitude,probs = 0.25,na.rm = T) # 81.15
quantile(tab_ONDE$altitude,probs = 0.75,na.rm = T) # 302.75
length(which(is.na(tab_ONDE$altitude))) # 107
...
quantile(na.omit(c(tab_CTRIP$Altitude,
                   ...
                   tab_SMASH$Altitude,
                   tab_Obs$Altitude,
                   tab_ONDE$altitude)), probs = 0.25) # 74.8
quantile(na.omit(c(tab_CTRIP$Altitude,
                   tab_GRSD$Altitude,
                   tab_J2000$Altitude,
                   tab_ORCHIDEE$Altitude,
                   tab_SMASH$Altitude,
                   tab_Obs$Altitude,
                   tab_ONDE$altitude)), probs = 0.75) # 308
