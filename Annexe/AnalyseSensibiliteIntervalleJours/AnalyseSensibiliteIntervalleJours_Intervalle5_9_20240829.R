...
png("/home/tjaouen/Documents/Output/ChangementClimatique2019/EtudeFrance/15_ResultatsModeles_ValidationParAnnees_ParHer/17_PresentMesures_HERh_FltOnde_JctHER_SansIFetExES_CorrOubliHerJointes20212022etFonctionSeuilOnde_SensiIntervJours_2012_2022_20230614/LeaveOneYearOut/Validation_ParAnnees/SensibiliteJour/BarplotKGEdesHER_ParIntervalleTemps_1_20240829_d.png",
    width = 1200, height = 750,
    units = "px", pointsize = 12)
# x11()
p_KGE <- ggplot(df_long, aes(x = variable, y = value)) +
  geom_boxplot() +
  geom_hline(yintercept = median(df_long$value[which(df_long$variable == "[j-6;j]")], na.rm = T), color = "#d95f0e", linetype = "dashed", size = 1) +
  # geom_line(data = means_val_, mapping = aes(x = intervalle, y = minimums, group = 1),
  #           color = "gray38", linetype = "solid", size = 2) +
  # geom_line(data = means_val_, mapping = aes(x = intervalle, y = maximums, group = 1),
  #           color = "gray38", linetype = "solid", size = 2) +
  labs(title = bquote(''),
       x = bquote('Time window of daily discharge'),
       y = "KGE (unitless)") +
  theme(text = element_text(size = 20),
        axis.line.x = element_line(color = "gray38", size = 1),
        axis.line.y = element_line(color = "gray38", size = 1),
        axis.text.x = element_text(size= 24,angle = 35, hjust = 1, vjust = 1,colour = "gray38"),
        axis.text.y = element_text(size= 24, colour = "gray38"),
        axis.title.x = element_text(size= 30, colour = "gray38"),
        axis.title.y = element_text(size= 30, colour = "gray38"),
        panel.background = element_blank(),
